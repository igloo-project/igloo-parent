...
public class NotificationBuilder
    implements INotificationBuilderInitState,
        INotificationBuilderBaseState,
        INotificationBuilderBuildState,
        INotificationBuilderBodyState,
        INotificationBuilderContentState,
  ...
  private final Map<NotificationTarget, INotificationRecipient> toByAddress =
      Maps.newLinkedHashMap();

  private final Map<NotificationTarget, INotificationRecipient> ccByAddress =
      Maps.newLinkedHashMap();

  private final Map<NotificationTarget, INotificationRecipient> bccByAddress =
      Maps.newLinkedHashMap();
  ...
  /**
   * This is not a map, since duplicate filenames are allowed. This is not a multimap either, since
   * mutliple attachements with the same name are not related to each other.
   */
  ...
  /**
   * Argument may be either an addr-spec or an address (<i>local-part@domain</i> or <i>Personal
   * &lt;local-part@domain&lt;</i>)
   *
   * @see InternetAddress
   */
   ...
   *
   * <p>i.e. arguments must be <i>local-part@domain</i> addresses and NOT <i>Personal
   * &lt;local-part@domain&lt;</i>
   */
  @Override
  ...
  @Override
  public INotificationBuilderBuildState to(
  ...
  @Override
  public INotificationBuilderBuildState cc(
      INotificationRecipient ccFirst, INotificationRecipient... ccOthers) {
  ...
  @Override
  public INotificationBuilderBuildState bcc(
      INotificationRecipient bccFirst, INotificationRecipient... bccOthers) {
  ...
  public INotificationBuilderBuildState exceptAddress(Collection<String> except) {
    recipientsToIgnore.addAll(
        except.stream().map(ADDRESS_TO_TARGET_FUNCTION).collect(Collectors.toList()));
    return this;
  }
  ...
  @Override
  public INotificationBuilderBuildState except(
      INotificationRecipient exceptFirst, INotificationRecipient... exceptOthers) {
    return except(Lists.asList(exceptFirst, exceptOthers));
  }

  @Override
  public INotificationBuilderBuildState except(
      Collection<? extends INotificationRecipient> except) {
    recipientsToIgnore.addAll(
        except.stream()
            .map(I_NOTIFICATION_RECIPIENT_TO_TARGET_FUNCTION)
  ...
  public INotificationBuilderTemplateState template(String templateKey) {
    ...
    this.templateContentDescriptor =
        new FreemarkerTemplateNotificationContentDescriptorImpl(
            ...
            templateKey,
            Collections.unmodifiableCollection(attachments),
            getDefaultLocale());
  ...
  @Override
  public INotificationBuilderContentState content(
      INotificationContentDescriptor contentDescriptor) {
  ...
  public void send() throws ServiceException {
    ...
    for (Entry<INotificationContentDescriptor, Targets> entry : groupTargetsByContext()) {
      ...
      if (targets.to.isEmpty()
          && targets.cc.isEmpty()
          && targets.bcc.isEmpty()) { // Multimap.get(unknown key) returns an empty collection
        continue;
      }
      ...
      Address sender;
      try {
        // Step 1 : Build message. Effective recipients could be different than expected ones, we
        // store.
        ...
        throw new ServiceException(
            String.format(
                "Error building the MIME message (to:%s, cc:%s, bcc:%s)",
      ...
      try {
        ...
        throw new ServiceException(
            String.format(
                ...
                sender,
                Arrays.toString(replyTo),
                targets.to,
                targets.cc,
                targets.bcc,
                Arrays.toString(effectiveTo),
                Arrays.toString(effectiveCc),
                Arrays.toString(effectiveBcc)),
            e);
      }
    }
  }
  ...
  private void addAddresses(
      Map<NotificationTarget, INotificationRecipient> recipientsByTarget,
  ...
  private void addAddress(
      Map<NotificationTarget, INotificationRecipient> recipientsByTarget, String address) {
  ...
  private void addRecipient(
    ...
    if ((recipient.isNotificationEnabled() || bypassDisabledRecipients)
      ...
      for (String redirectionEmail :
          propertyService.get(NOTIFICATION_MAIL_DISABLED_RECIPIENT_FALLBACK)) {
        addRecipientUnsafe(
            recipientsByTarget,
            NotificationTarget.of(redirectionEmail),
            new SimpleRecipient(recipient, redirectionEmail));
      }
    }
  }

  private void addRecipientUnsafe(
      Map<NotificationTarget, INotificationRecipient> recipientsByTarget,
  ...
  private MimeMessage buildMessage(
      INotificationContentDescriptor contentDescriptor, Targets recipients, String encoding)
      throws NotificationContentRenderingException, MessagingException {
    ...
    MimeMessageHelper helper =
        new MimeMessageHelper(message, isMultipartNeeded(bodyPlainText, bodyHtmlText), encoding);
    ...
    String textBodyPrefix =
        getBodyPrefix(recipients.to, recipients.cc, recipients.bcc, encoding, MailFormat.TEXT);
    String htmlBodyPrefix =
        getBodyPrefix(recipients.to, recipients.cc, recipients.bcc, encoding, MailFormat.HTML);
  ...
  private boolean isMultipartNeeded(String bodyPlainText, String bodyHtmlText) {
    boolean multipleBodies =
        StringUtils.hasText(bodyPlainText) && StringUtils.hasText(bodyHtmlText);
    ...
    switch (behavior) {
      ...
      default:
        throw new IllegalStateException(
            String.format(
                "Unknown value %s for %s", behavior, MailSenderBehavior.class.getSimpleName()));
    }
  }
  ...
  protected Collection<NotificationTarget> getNotificationFilterEmails() {
    return propertyService.get(NOTIFICATION_MAIL_FILTER_EMAILS).stream()
        .map(ADDRESS_TO_TARGET_FUNCTION)
        .collect(Collectors.toList());
  }
  ...
  private String getBodyPrefix(
      Collection<NotificationTarget> to,
      Collection<NotificationTarget> cc,
      Collection<NotificationTarget> bcc,
      String encoding,
  ...
  private String renderAddressesForDebug(
      Collection<NotificationTarget> addresses, MailFormat mailFormat) {
    String debug;
