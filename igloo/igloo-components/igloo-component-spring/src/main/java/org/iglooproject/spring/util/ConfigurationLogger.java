...
/**
 * <p>This Spring listener allows to log PropertyId-s values on refresh event. It is triggered on by a root context
 * event.</p>
 * 
 * <p>Logged properties are controlled by <i>propertyNamesForInfoLogLevel</i> attribute, that list PropertyId-s keys to
 * log. These string values must match a registered PropertyId key.</p>
 * 
 * <p>Logged values are sent to {@link org.iglooproject.spring.util.ConfigurationLogger} with an INFO level,
 * and according to <i>propertyNamesForInfoLogLevel</i> list order.</p>
 * 
 * <p>If {@link org.iglooproject.spring.util.ConfigurationLogger} is set to TRACE level, all registered PropertyId-s
 * are logged in a second phase.</p>
 * 
 * <p>logPattern attribute allows to control the way key/value are logged.</p>
 * 
 * @see PropertyId
...
public class ConfigurationLogger implements ApplicationListener<ContextRefreshedEvent> {
	...
	private IConfigurablePropertyService getPropertyServiceName(ApplicationContext context) {
		...
		if (propertyServiceNames.length > 1) {
			if (LOGGER.isWarnEnabled()) { // NOSONAR (squid:S1066)
				LOGGER.warn(String.format("Multiple %1$s found. We only log the configuration of the first instance.",
	...
	/**
	 * Log configured PropertyId-s values, according to configuration order, then, if TRACE mode enabled, log all
	 * PropertyId-s values.
	 */
	private void logConfigurationPropertiesIds(IConfigurablePropertyService propertyService) {
		...
		// iterate on propertyIdsKeysForInfoLogLevel so that properties are logged according to configuration order.
