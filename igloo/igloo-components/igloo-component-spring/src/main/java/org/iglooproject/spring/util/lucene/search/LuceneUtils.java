...
public final class LuceneUtils {
  ...
  public static RawLuceneQuery toFilterRangeQuery(
      String field, Number min, Number max, boolean minInclusive, boolean maxInclusive) {
    if (min == null && max == null) {
      throw new IllegalArgumentException(
          String.format("Either min (%f) or max (%f) is needed", min, max));
    }
    ...
    if (booleanQuery.clauses().size() > 0) {
      ...
      if (booleanQuerySb.length() > 0) {
        if (booleanQuery.clauses().size() > 1
            ...
            || booleanQuerySb.charAt(0) == '+'
            || (booleanQuery.clauses().size() == 1
                && (booleanQuery.clauses().get(0).getQuery() instanceof RawLuceneQuery))) {
  ...
  private static String formatRawLuceneQuery(RawLuceneQuery simpleQuery) {
    ...
    if (StringUtils.hasText(simpleQuery.getQuery())) {
      sb.append("(").append(simpleQuery.getQuery()).append(")");
    }
    return sb.toString();
  }
  ...
  private static String formatNumericRangeQuery(
      NumericRangeQuery<? extends Number> numericRangeQuery) {
    return toFilterRangeQuery(
            numericRangeQuery.getField(),
            numericRangeQuery.getMin(),
            numericRangeQuery.getMax(),
            numericRangeQuery.includesMin(),
            numericRangeQuery.includesMax())
        .getQuery();
  }
