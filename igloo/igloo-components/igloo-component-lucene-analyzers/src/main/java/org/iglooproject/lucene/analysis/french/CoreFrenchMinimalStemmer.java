...
/**
 * Stemmer adapté depuis le FrenchMinimalStemmer qui a les particularités suivantes : - on ne
 * désactive le stemming que si la chaîne fait moins de 4 caractères et pas 6 (typiquement, problème
 * sur stage et stages, noir et noire) - on enlève la suppression des doublements de lettres en fin
 * de mot pour éviter marmot == marmotte - on arrête le stemming si le mot devient trop court de
 * manière à éviter le phénomène stage/stages sur des mots très courts
 */
public class CoreFrenchMinimalStemmer {
   ...
   *
   * <p>/!\ certaines règles dépendent de ce choix de longueur : il faut donc faire attention à ne
   * pas le descendre ou alors il faut intégrer des checks en plus ci-dessous quand on remonte dans
   * les index
   */
