...
/**
 * This class reads provided {@link StorageEvent} collections to apply appropriate transaction
 * completion action on storage.
 *
 * <p>All file storageOperations are delegated to {@link StorageOperations}. All meaningful logs are
 * triggered by {@link StorageOperations} instance.
 *
...
public class StorageTransactionHandler {
  ...
  public void onRollback(List<StorageEvent> events) {
    ...
    events.stream()
        .filter(StorageTransactionHandler::isAdd)
        .forEach(t -> operations.removePhysicalFile("[rollback/add]", t.getId(), t.getPath()));
  }

  public void onCommit(List<StorageEvent> events) {
    // Remove physical deleted file on commit
    events.stream()
        .filter(StorageTransactionHandler::isDelete)
        .forEach(t -> operations.removePhysicalFile("[commit/delete]", t.getId(), t.getPath()));
  }
