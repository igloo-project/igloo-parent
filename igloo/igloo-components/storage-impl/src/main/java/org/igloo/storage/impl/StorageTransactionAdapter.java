...
/**
 * Implements Spring {@link TransactionSynchronization} contract. This instance must be added in
 * {@link TransactionSynchronization} context and {@link #addEvent(Long, StorageEventType, Path)}
 * must be called for each {@link Fichier} transaction-involving operation.
 *
 * <p>After transaction status switch, storageOperations are delegated to {@link
 * StorageTransactionHandler}.
 *
...
public class StorageTransactionAdapter implements TransactionSynchronization {
  ...
  public StorageTransactionAdapter(
      Integer order,
      StorageTransactionHandler handler,
      IStorageTransactionResourceManager resourceManager) {
    this.order = order;
