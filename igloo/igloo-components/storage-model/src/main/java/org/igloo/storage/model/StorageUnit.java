...
/**
 * A {@link StorageUnit} is a physical device that can store files. {@link StorageUnit}
 * implementation must provide listing API to allow consistency jobs to check the storage status
 * (missing or orphan files/{@link Fichier}). Performance can be ensured by splitting {@link
 * StorageUnit} before listing can be problematic (duration, memory).
 */
@Entity
@Bindable
public class StorageUnit extends GenericEntity<Long, StorageUnit> {
  ...
  /**
   * Checksum-enabled check delay for {@link StorageUnit}. If null application settings is applied.
   * Used only if {@link #checkType} allows checksum check.
   */
  @Basic(optional = true)
  private Duration checkChecksumDelay;

  /**
   * Split condition, by total file size, before a storage unit split. New storage unit inherits all
   * settings.
   *
   * <p>If null, file size does not trigger storage unit split.
   */
  @Basic(optional = true)
  private Long splitSize;

  /**
   * Split condition, by duration, before a storage unit split. New storage unit inherits all
   * settings.
   *
   * <p>If null, storage unit age does not trigger storage unit split.
   */
  @Basic(optional = true)
