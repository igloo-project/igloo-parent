...
import org.igloo.monitoring.service.PerfData;
...
class TestPage {
  ...
  @RegisterExtension public WicketTesterExtension scope = new WicketTesterExtension();
  ...
  void testPageOk() {
    ...
    assertThat(tester.getLastResponse().getHeader("Content-Type"))
  ...
  void testPageWarning() {
    ...
    ResourceReference resourceReference =
        resourceReference(HealthStatus.WARNING, "Some problems here !", null);
    tester.startResourceReference(resourceReference);
    assertThat(tester.getLastResponseAsString())
        .isEqualTo("WARNING metric.name - Some problems here !");
    assertThat(tester.getLastResponse().getHeader("Content-Type"))
        .isEqualTo("text/plain; charset=UTF-8");
    assertThat(tester.getLastResponse().getHeader("Cache-Control")).isEqualTo("no-cache");
  ...
  void testPageCritical() {
    WicketTester tester = scope.getTester();
    ResourceReference resourceReference =
        resourceReference(HealthStatus.CRITICAL, "Big trouble :-/", null);
    tester.startResourceReference(resourceReference);
    assertThat(tester.getLastResponseAsString())
        .isEqualTo("CRITICAL metric.name - Big trouble :-/");
    assertThat(tester.getLastResponse().getHeader("Content-Type"))
        .isEqualTo("text/plain; charset=UTF-8");
    assertThat(tester.getLastResponse().getHeader("Cache-Control")).isEqualTo("no-cache");
  ...
  void testPageUnknown() {
    WicketTester tester = scope.getTester();
    ResourceReference resourceReference =
        resourceReference(HealthStatus.UNKNOWN, "Status not known", null);
    tester.startResourceReference(resourceReference);
    assertThat(tester.getLastResponseAsString())
        .isEqualTo("UNKNOWN metric.name - Status not known");
    assertThat(tester.getLastResponse().getHeader("Content-Type"))
        .isEqualTo("text/plain; charset=UTF-8");
    assertThat(tester.getLastResponse().getHeader("Cache-Control")).isEqualTo("no-cache");
  ...
  void testSpring() {
    ...
    when(service1.getHealthLookup())
        .thenReturn(new HealthLookup("metric.name1", HealthStatus.OK, "All Right", null));
    when(service2.getHealthLookup())
        .thenReturn(new HealthLookup("metric.name2", HealthStatus.WARNING, "Some problems", null));
  ...
  private ResourceReference resourceReference(
      HealthStatus status, String message, List<PerfData> perfDatas) {
    IHealthMetricService service1 = mock(IHealthMetricService.class);
    when(service1.getHealthLookup())
        .thenReturn(new HealthLookup("metric.name", status, message, perfDatas));
    IHealthService service = HealthService.of("metric.name", service1);
    SerializableSupplier<IResource> resourceSupplier =
        () ->
            new MonitoringResource() {
