...
public class SimpleMicrometerHealthMetricService implements IHealthMetricService {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(SimpleMicrometerHealthMetricService.class);
  ...
  /**
   * With this constructor, tag-filtered metric must be unique.
   *
   * @see SimpleMicrometerHealthMetricService#SimpleMicrometerHealthService(MeterRegistry, String,
   *     Predicate, Collector, Function, Function, Function)
   * @param registry
   * @param metricName
   * @param filters
   ...
   */
  public SimpleMicrometerHealthMetricService(
      MeterRegistry registry,
      String metricName,
      Tags filters,
      Function<Number, HealthStatus> statusFunction,
      Function<Number, String> messageFunction) {
    this(registry, metricName, filters, null, statusFunction, messageFunction);
  }

  /**
   * Metric filtering is tag-based.
   *
   * @see SimpleMicrometerHealthMetricService#SimpleMicrometerHealthService(MeterRegistry, String,
   *     Predicate, Collector, Function, Function, Function)
   */
  public SimpleMicrometerHealthMetricService(
      ...
      String metricName,
      Tags filters,
      Collector<Gauge, ?, ? extends Number> reducer,
      Function<Number, HealthStatus> statusFunction,
      Function<Number, String> messageFunction) {
    this(
        registry,
  ...
  public SimpleMicrometerHealthMetricService(
      ...
      String metricName,
      String metricCheck,
      String metricCheckTagName,
      Predicate<Gauge> metricFilter,
      Collector<Gauge, ?, ? extends Number> reducer,
      Function<Number, HealthStatus> statusFunction,
      Function<Number, String> messageFunction) {
    this(
  ...
  /**
   * This service can provides a {@link HealthLookup} object from a metric coordinate (name and
   * tags), a reducer (how to collect multiple metrics to provide one number) and a status function
   * (how to map a number to a status).
   *
   * <p>Metric filtering is done by a predicate.
   *
   * @param registry a micrometer registry to lookup metrics from.
   * @param metricName a metric name for micrometer lookup.
   * @param metricCheck a metric that allows to check if metricName is meant to be available. May be
   *     null.
   ...
   * @param reducer a function to build a unique number result from all matching metrics. If null,
   *     metric uniqueness is enforced.
   ...
   * @param perfDataClassifier if perfData is needed, this classifier is used to split all matching
   *     metrics; each classifier is used for a perfData line. If null, only one perfData line is
   *     generated.
   */
  public SimpleMicrometerHealthMetricService(
      MeterRegistry registry,
      ...
      Function<Number, HealthStatus> statusFunction,
      Function<Number, String> messageFunction,
      Function<Gauge, String> perfDataClassifier) {
    if (metricCheck != null) {
      Objects.requireNonNull(
          metricCheckTagName, "metricCheckTagName is required if metricCheck is not null.");
    }
  ...
  private void checkAvailability() throws LookupResultException {
    if (metricCheck != null) {
      ...
      if (gauge == null) {
        throw unknownStatus(
            metricName, String.format("No computation found for metric %s", metricName));
      }
    }
  }
  ...
  private String lookupBaseUnit() {
    ...
    try {
      baseUnit =
          Optional.ofNullable(registry.get(metricName))
              .map(m -> m.gauge().getId().getBaseUnit())
              .orElse("");
   ...
   */
  private static LookupResultException unknownStatus(String metricName, String message)
      throws LookupResultException {
    throw new LookupResultException(
        new HealthLookup(metricName, HealthStatus.UNKNOWN, message, null));
  }
  ...
  public static Predicate<Gauge> tagFilter(Tags filters) {
    return g ->
        filters.stream().allMatch(t -> Objects.equals(t.getValue(), g.getId().getTag(t.getKey())));
  }
