...
public abstract class AbstractModalPopupPanel<O> extends GenericPanel<O>
    implements IModalPopupPanel, IMarkupResourceStreamProvider {
  ...
  public AbstractModalPopupPanel(String id, IModel<? extends O> model) {
    ...
    add(
        container
            .add(dialog.add(new ClassAttributeAppender(getModalDialogCssClassModel())))
  ...
  public IResourceStream getMarkupResourceStream(
      final MarkupContainer container, Class<?> containerClass) {
    if (AbstractModalPopupPanel.class.equals(containerClass)) {
      ...
      Class<?> bootstrapOverrideContainerClass =
          BootstrapRequestCycle.getSettings().modalMarkupClass();
      String path = bootstrapOverrideContainerClass.getName().replace('.', '/');
      IResourceStream resourceStream =
          locator.locate(bootstrapOverrideContainerClass, path, null, null, null, "html", false);
      return new MarkupResourceStream(
          resourceStream,
          new ContainerInfo(bootstrapOverrideContainerClass, container),
          bootstrapOverrideContainerClass);
    } else {
