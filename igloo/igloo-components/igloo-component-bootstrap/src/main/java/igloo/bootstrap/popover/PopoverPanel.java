...
public class PopoverPanel extends Panel {
  ...
  public PopoverPanel(String wicketId, Popover popover) {
    ...
    add(titleComponent, contentComponent);

    add(
        link.add(
                new EnclosureContainer("icon")
                    .condition(
                        Condition.predicate(asModel(popover.iconCssClass()), Predicates2.hasText()))
                    .add(new AttributeModifier("class", asModel(popover.iconCssClass()))),
                new CoreLabel("label", asModel(popover.label()))
                    .hideIfEmpty()
                    .add(
                        Condition.predicate(asModel(popover.showLabel()), Predicates2.isTrue())
                            .thenShow()))
            .add(
    ...
    add(Condition.componentVisible(contentComponent).thenShowInternal());
  }
