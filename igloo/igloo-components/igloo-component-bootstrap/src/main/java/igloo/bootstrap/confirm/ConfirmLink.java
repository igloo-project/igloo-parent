...
public abstract class ConfirmLink<O> extends Link<O> {
  ...
  public void renderHead(IHeaderResponse response) {
    ...
    Event confirmEvent =
        new Event(BootstrapConfirmEvent.CONFIRM) {
          ...
          public JsScope callback() {
            return JsScopeEvent.quickScope(
                new JsStatement()
                    .append("window.location = " + JsUtils.quotes(getURL(), true))
                    .append(";")
                    .append("event.preventDefault();"));
          }
        };
    response.render(
        OnDomReadyHeaderItem.forScript(new JsStatement().$(this).chain(confirmEvent).render(true)));
  }
}
