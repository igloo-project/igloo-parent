...
public class AjaxConfirmLinkBuilder<O> extends AbstractConfirmLinkBuilder<AjaxConfirmLink<O>, O>
    implements IConfirmLinkBuilder<AjaxConfirmLink<O>, O> {
  ...
  public AjaxConfirmLink<O> create(String wicketId, IModel<O> model) {
    if (onAjaxClick == null) {
      throw new IllegalStateException(
          String.format(
              "%s must be used with a %s", getClass().getName(), IAjaxAction.class.getName()));
    }
    AjaxConfirmLink<O> ajaxConfirmLink =
        new FunctionalAjaxConfirmLink<>(
            wicketId,
            model,
            form,
            titleModelFactory,
