...
class TestJuiceInliner {
  ...
  void testSuccess() throws MalformedURLException {
    ...
    server.when(request().withPath("/")).respond(response().withStatusCode(200).withBody("html"));
  ...
  void testRetry_success() throws MalformedURLException {
    ...
    server.when(request().withPath("/"), Times.once()).respond(response().withStatusCode(500));
    server.when(request().withPath("/"), Times.once()).respond(response().withStatusCode(200));

    inliner.postProcessJuice("html", "css");

    server.verify(request().withPath("/"), VerificationTimes.exactly(2));
  }
  ...
  void testRetry_failure() throws MalformedURLException {
    ...
    Assertions.assertThatCode(() -> inliner.postProcessJuice("html", "css"))
  ...
  void testAuthentication_empty() throws MalformedURLException {
    ...
    server.when(request().withPath("/"), Times.once()).respond(response().withStatusCode(200));
    inliner.postProcessJuice("html", "css");
    server.verify(request().withPath("/").withHeader("Authorization"), VerificationTimes.never());
  }
  ...
  void testAuthentication_notEmpty() throws MalformedURLException {
    ...
    inliner.postProcessJuice("html", "css");
    server.verify(
        request().withPath("/").withHeader("Authorization", "Bearer %s".formatted(token)),
        VerificationTimes.once());
  }
  ...
  void testTimeout_honored_callFailed() throws MalformedURLException {
    URL url = new URL("http://localhost:%d".formatted(server.getPort()));
    String token = "toto";
    IJuiceInliner inliner =
        JuiceInliner.builder()
            .inlinerUrl(url)
            .retryMaxAttempts(5)
            // keep a 200ms delay
            .requestTimeout(Duration.ofMillis(1200))
            .build();
    server
    ...
    server.verify(request().withPath("/"), VerificationTimes.once());
  }
  ...
  void testTimeout_honored_callSuccess() throws MalformedURLException {
    ...
    server.verify(request().withPath("/"), VerificationTimes.exactly(5));
  }
}
