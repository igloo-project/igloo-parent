...
public interface IJuiceInliner {
	...
	default String postProcessJuice(String html, String extraCss) {
		try {
			...
			Builder httpRequestBuilder = HttpRequest
				.newBuilder(inlinerUrl().toURI())
			...
			var retryConfigBuilder = RetryConfig.<HttpResponse<String>>custom()
			...
			Retry retry =  Retry.of(
				"juice-css",
				retryConfigBuilderHandler().apply(retryConfigBuilder).build()
			);
			
			HttpResponse<String> response = Decorators.ofCallable(() -> httpClient().send(request, BodyHandlers.ofString()))
