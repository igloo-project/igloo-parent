...
public abstract class AbstractJQPlotDataAdapter<S, K, V, TK>
    extends AbstractJQPlotDataProvider<S, K, V> implements IJQPlotDataAdapter<S, K, V, TK> {
  ...
  @Override
  public final Collection<? extends AbstractSeries<TK, V, ? extends SeriesEntry<TK, V>>>
      getObject() {
    return getObject(Session.get().getLocale());
  }

  protected abstract Collection<? extends AbstractSeries<TK, V, ? extends SeriesEntry<TK, V>>>
      getObject(Locale locale);
  ...
  protected void setObject(Collection<? extends AbstractSeries<?, ?, ?>> object, Locale locale) {
    throw new UnsupportedOperationException(
        "Cannot set the value of a " + Classes.simpleName(getClass()));
  }
