 ...
 * <p>This is only useful if your data references keys from a continuous domain (i.e. with a high or infinite
 * number of values in any given range), such as floating point numbers, or integers on extensive ranges.
 * <p>If your data keys are only "discrete" keys (for example integers on small ranges), use
 * a {@link JQPlotDiscreteKeysDataAdapter}.
 * 
...
public class JQPlotContinuousNumberKeysDataAdapter<S, K extends Number, V extends Number>
	 ...
	 * (this file is located in the same package as {@link JQPlotDateAxisRendererResourceReference}).
	 */
	public JQPlotContinuousNumberKeysDataAdapter(IModel<Table<S, K, V>> tableModel, String formatString) {
	 ...
	 * (this file is located in the same package as {@link JQPlotDateAxisRendererResourceReference}).
	 */
	public JQPlotContinuousNumberKeysDataAdapter(IJQPlotDataProvider<S, K, V> dataProvider, String formatString) {
	...
	public void configure(PlotOptions options, Map<? extends S, PlotSeries> seriesMap, Map<? extends K, PlotTick> keysMap, Locale locale) {
		options.getAxes().getXaxis()
				.setRenderer((String)null); // Default renderer
