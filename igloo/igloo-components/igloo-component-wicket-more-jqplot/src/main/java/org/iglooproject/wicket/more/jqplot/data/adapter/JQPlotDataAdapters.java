...
public final class JQPlotDataAdapters {
  ...
  public static <S, K, V extends Number, TK, SE extends SeriesEntry<TK, V>>
      IJQPlotDataAdapter<S, K, V, TK> addPercentLabels(
          final IJQPlotDataAdapter<S, K, V, TK> adapter,
          final Renderer<? super Double> percentRenderer) {
  ...
  private static <K, V extends Number, TK>
      Collection<? extends AbstractSeries<TK, V, ? extends SeriesEntry<TK, V>>> addPercentLabels(
          Collection<? extends AbstractSeries<TK, V, ? extends SeriesEntry<TK, V>>> delegateObject,
          Renderer<? super Double> percentRenderer,
          Locale locale) {
    ...
    Renderer<SeriesEntry<TK, V>> entryRenderer =
        percentRenderer.onResultOf(
  ...
  private static <K, V extends Number, E extends SeriesEntry<K, V>>
      Collection<LabelledSeries<K, V>> addLabels(
          Collection<? extends AbstractSeries<? extends K, V, ? extends E>> input,
          Renderer<? super E> renderer,
          Locale locale) {
    ...
    for (Series<?, ?, ? extends E> series : input) {
      ...
      for (E entry : series.getData()) {
        copiedSeries.addEntry(
            new LabelledSeriesEntry<K, V>(
                entry.getKey(), entry.getValue(), renderer.render(entry, locale)));
      }
    }
    return seriesList;
  }

  public static <S, K, V extends Number & Comparable<V>, TK, SE extends SeriesEntry<TK, V>>
      IJQPlotDataAdapter<S, K, V, TK> fix(final IJQPlotDataAdapter<S, K, V, TK> adapter) {
    return new ForwardingJQPlotDataAdapter<S, K, V, TK>(adapter) {
      private static final long serialVersionUID = 1L;
      ...
      public void afterConfigure(
        ...
        // To avoid the problems related to data too low: jqPlot duplicates the values on the Y axis
        // when
        ...
        if ("%d".equals(formatString) || "%'d".equals(formatString)) {
          ...
          if ((max - min) <= 5.0) {
            options
                ...
                .getYaxis()
                .setMin(
                    0.0 <= min && max <= 5.0 || fillToZero != null && fillToZero
                        ? 0
                        : (int) min - 2)
                .setMax((int) max + 2)
                .setNumberTicks(5);
          } else {
            options.getAxes().getYaxis().setMin(null).setMax(null).setNumberTicks(null);
          }
        }
      ...
      protected Collection<? extends AbstractSeries<TK, V, ? extends SeriesEntry<TK, V>>>
        ...
        // The only way to make it accept the fact that we want to display a series (for instance in
        // the legend) even
