 ...
 *   <li>It includes a built-in placeholder for cases when there's no data to be shown (see {@link
 *       #setNonEmptyDataPredicate(SerializablePredicate2)})
 * </ul>
 *
...
public abstract class JQPlotPanel<S, K, V extends Number & Comparable<V>, TK> extends Panel {
  ...
  @SpringBean private DefaultJQPlotRendererOptionsFactory optionsFactory;

  @SpringBean private IJQPlotConfigurer<Object, Object> defaultjqPlotConfigurer;

  /*
   * Also contains the default configurer (in first position) and the dataAdapter (in second position).
   */
  private final Collection<IJQPlotConfigurer<? super S, ? super K>> jqPlotConfigurers =
      Lists.newLinkedList();
  ...
  @SafeVarargs
  public final JQPlotPanel<S, K, V, TK> add(
      IJQPlotConfigurer<? super S, ? super K> configurer,
  ...
  protected void onInitialize() {
    ...
    add(
        new PlaceholderContainer("jqPlotPlaceholder")
            .condition(Condition.componentVisible(jqPlot)));
    ...
    jqPlot.add(
        new Behavior() {
          ...
          public void renderHead(Component component, IHeaderResponse response) {
            response.render(
                JavaScriptHeaderItem.forReference(JQPlotAutoresizeJavascriptReference.get()));
          }
        });
    ...
    jqPlot.add(
        new Condition() {
          ...
          public boolean applies() {
             ...
             * would be incorrect).
             */
            return nonEmptyDataPredicate.test(
                Collections.unmodifiableCollection(dataAdapter.getValues()));
          }
        }.thenShow());
  }
  ...
  public JQPlotPanel<S, K, V, TK> setNonEmptyDataPredicate(
      SerializablePredicate2<Collection<V>> nonEmptyDataPredicate) {
