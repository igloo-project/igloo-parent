 ...
 *
 * <p>This adapter assigns an index to each data point so that all charts (especially bar charts)
 * will render correctly.
 *
 * <p>This is only useful if your data references keys from a discrete domain (i.e. with a small,
 * finite number of values in a given range). This is the case when your keys are labels
 * ("male"/"female"), but also when they are ranges (e.g. the first day of each week at midnight,
 * which represents the whole week).
 *
 * <p>If your data keys are only "almost-continuous" keys (such as floating-point numbers, or dates
 * precise to the second, use a {@link AbstractJQPlotContinuousKeysDataAdapter} instead.
 *
 ...
 */
public class JQPlotDiscreteKeysDataAdapter<S, K, V extends Number>
    extends AbstractMissingValuesJQPlotDataAdapter<S, K, V, Integer> {
  ...
  public JQPlotDiscreteKeysDataAdapter(
      ...
      IRenderer<? super K> keyRenderer) {
    this(
        new JQPlotTableDataProvider<>(tableModel),
        seriesModel,
        keysModel,
        missingValueReplacementModel,
        keyRenderer);
  }
  ...
  public void configure(
      ...
      Locale locale) {
    options.getAxes().getXaxis().setRenderer(JQPlotCategoryAxisRenderer.get());
