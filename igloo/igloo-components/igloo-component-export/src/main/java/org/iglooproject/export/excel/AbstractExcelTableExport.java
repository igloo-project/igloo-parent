...
/**
 * Classe abstraite permettant de construire des tableaux Excel.
 *
 * @author Open Wide
 */
public abstract class AbstractExcelTableExport extends AbstractExcelExport {
   ...
   *
   * <p>J'aurai bien mis en octets aussi, malheureusement Excel n'accepte visiblement que deux
   * conditions... pas trois.
   */
  private String fileSizeDataFormat =
  ...
  /**
   * Retourne le style de la ligne en tenant compte des alternances de couleurs pour les lignes
   * paires/impaires.
   *
   ...
   */
  protected void addHeadersToSheet(
      Sheet sheet, int rowIndex, Collection<ColumnInformation> columnInfos) {
    int columnIndex = 0;
   ...
   * @param sheet feuille de calcul
   * @param rowIndex numéro de la ligne
   * @param columnInfos RangeMap contenant les informations de colonnes (valeurs) et les index sur
   *     auxquelles s'appliquent ces colonnes (clés). Les "colonnes" s'étendant sur plus d'un index
   *     seront automatiquement fusionnées.
   */
  protected void addHeadersToSheet(
      Sheet sheet, int rowIndex, RangeMap<Integer, ColumnInformation> columnInfos) {
    Row rowHeader = sheet.createRow(rowIndex);
    for (Map.Entry<Range<Integer>, ColumnInformation> entry :
        columnInfos.asMapOfRanges().entrySet()) {
      Range<Integer> range = entry.getKey();
      ColumnInformation columnInformation = entry.getValue();

      addHeaderCell(
          rowHeader, range.lowerEndpoint(), getColumnLabel(columnInformation.getHeaderKey()));
  ...
  /**
   * Finalise la création de la feuille de calcul, notamment en demandant le redimensionnement
  ...
  protected void finalizeSheet(Sheet sheet, List<String> headers, boolean landscapePrintSetup) {
    ...
    for (int i = 0; i < nbColumns; i++) {
      ...
      sheet.setColumnWidth(
          i, columnWidth < ABSOLUTE_MAX_COLUMN_WIDTH ? columnWidth : ABSOLUTE_MAX_COLUMN_WIDTH);
    }
   ...
   * @param landscapePrintSetup définit si la feuille est imprimée en paysage ou non
   */
  protected void finalizeSheet(
      Sheet sheet, Collection<ColumnInformation> columnInfos, boolean landscapePrintSetup) {
   ...
   */
  protected void finalizeSheet(
      Sheet sheet, RangeMap<Integer, ColumnInformation> columnInfos, boolean landscapePrintSetup) {
    for (Map.Entry<Range<Integer>, ColumnInformation> entry :
        columnInfos.asMapOfRanges().entrySet()) {
      ColumnInformation columnInformation = entry.getValue();
      Range<Integer> range = entry.getKey();
      ...
      if (columnInformation.getColumnWidth() != -1) {
        ...
        // On prend en compte le fait que la "colonne" peut s'étendre en fait sur plusieurs colonnes
        // (fusion de cellules au niveau du header)
        ...
        // On redimmensionne les colonnes une à une en prennant leur taille actuelle et en les
        // augmentant un petit peu
        for (int columnIndex = beginIndex; columnIndex <= endIndex; ++columnIndex) {
   ...
   *
   * <p>Dans POI, les régions fusionnées ne sont pas prises en compte dans le autoSizeColumn. Quand
   * on fusionne des cellules sur une même colonne, on corrige la taille de cette colonne si
   * nécessaire.
   *
   * @param sheet feuille de calcul
  ...
  /**
   * Peut être surchargé pour permettre, par exemple, de considérer la "headerKey" comme le libellé
   * déjà localisé.
   */
