...
public class PersonXSSFLocalDateTimeExport extends AbstractExcelTableExport {

  public PersonXSSFLocalDateTimeExport() {
  ...
  public XSSFWorkbook generate(List<PersonLocalDateTime> persons, List<String> columns) {
    ...
    for (PersonLocalDateTime person : persons) {
  ...
  private void addCell(
      XSSFRow currentRow, int columnIndex, PersonLocalDateTime person, String column) {
    if ("username".equals(column)) {
      ...
      addLocalDateCell(currentRow, columnIndex, person.getBirthDate().toLocalDate());
