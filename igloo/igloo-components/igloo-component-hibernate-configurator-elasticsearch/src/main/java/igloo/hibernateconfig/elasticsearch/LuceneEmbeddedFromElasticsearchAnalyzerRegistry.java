...
public class LuceneEmbeddedFromElasticsearchAnalyzerRegistry
    implements LuceneEmbeddedAnalyzerRegistry, ClientSideAnalyzerProvider {
  ...
  public LuceneEmbeddedFromElasticsearchAnalyzerRegistry(
  ...
  public Analyzer getAnalyzer(
    ...
    ScopedElasticsearchAnalyzerReference scopedAnalyzer =
        (ScopedElasticsearchAnalyzerReference) searchIntegrator.getAnalyzerReference(indexedType);
    try {
      // these properties are package protected ! Use a bypass !
      ElasticsearchAnalyzerReference globalAnalyzer =
          (ElasticsearchAnalyzerReference)
              FieldUtils.getField(
                      ScopedElasticsearchAnalyzerReference.class, "globalAnalyzerReference", true)
                  .get(scopedAnalyzer);
      @SuppressWarnings("unchecked")
      Map<String, ElasticsearchAnalyzerReference> analyzers =
          (Map<String, ElasticsearchAnalyzerReference>)
              FieldUtils.getField(
                      ScopedElasticsearchAnalyzerReference.class, "scopedAnalyzerReferences", true)
                  .get(scopedAnalyzer);
      ...
      for (Entry<String, ElasticsearchAnalyzerReference> analyzer : analyzers.entrySet()) {
        luceneAnalyzers.put(
            analyzer.getKey(),
            new SimpleLuceneAnalyzerReference(
                getAnalyzer(analyzer.getValue().getAnalyzerName(null))));
      }
      LuceneAnalyzerReference luceneGlobalAnalyzer =
          new SimpleLuceneAnalyzerReference(getAnalyzer(globalAnalyzer.getAnalyzerName(null)));
      return new ScopedLuceneAnalyzerReference.Builder(luceneGlobalAnalyzer, luceneAnalyzers)
          .build()
          .getAnalyzer();
  ...
  @PostConstruct
  private void init() {
    LuceneAnalysisDefinitionRegistryBuilderImpl builder =
        new LuceneAnalysisDefinitionRegistryBuilderImpl();
    ...
    ServiceManager serviceManager =
        ContextHelper.getSearchIntegratorBySF(entityManagerFactory.unwrap(SessionFactory.class))
            .getServiceManager();
    ...
    for (Entry<String, AnalyzerDef> entry : registry.getAnalyzerDefinitions().entrySet()) {
      try {
        ...
        throw new IllegalStateException(
            String.format("Error building analyzer %s", entry.getKey()));
      }
    }
  }
  ...
  private Analyzer buildAnalyzer(AnalyzerDef analyzerDef, ResourceLoader resourceLoader)
      throws IOException {
    TokenizerDef tokenizer = analyzerDef.tokenizer();
    TokenizerFactory tokenizerFactory =
        buildAnalysisComponent(
            TokenizerFactory.class, tokenizer.factory(), tokenizer.params(), resourceLoader);

    return buildAnalyzer(
        tokenizerFactory, analyzerDef.charFilters(), analyzerDef.filters(), resourceLoader);
  }

  private Analyzer buildAnalyzer(
      TokenizerFactory tokenizerFactory,
      CharFilterDef[] charFilterDefs,
      TokenFilterDef[] filterDefs,
    ...
    for (int index = 0; index < tokenFiltersLength; index++) {
      ...
      filters[index] =
          buildAnalysisComponent(
              TokenFilterFactory.class, filterDef.factory(), filterDef.params(), resourceLoader);
    }
  ...
  private <T> T buildAnalysisComponent(
      Class<T> expectedFactoryClass,
      Class<? extends T> factoryClass,
      Parameter[] parameters,
      ResourceLoader resourceLoader)
      throws IOException {
    final Map<String, String> tokenMapsOfParameters =
        getMapOfParameters(parameters, luceneMatchVersion);
    T tokenizerFactory =
        instanceFromClass(
            expectedFactoryClass, factoryClass, "Tokenizer factory", tokenMapsOfParameters);
    injectResourceLoader(tokenizerFactory, tokenMapsOfParameters, resourceLoader);
    return tokenizerFactory;
  }

  private void injectResourceLoader(
      Object processor, Map<String, String> mapOfParameters, ResourceLoader resourceLoader)
      throws IOException {
  ...
  private static Map<String, String> getMapOfParameters(
      Parameter[] params, String luceneMatchVersion) {
