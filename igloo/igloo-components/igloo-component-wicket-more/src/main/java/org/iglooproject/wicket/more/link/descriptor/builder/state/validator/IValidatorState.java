 ...
 *
 * <p>"Validation" here means any type of condition, which among others may be parameter-based
 * (check that a parameter is in a certain state), session-based (check that the user has required
 * roles, or both (check that the user has a certain permission on a parameter).
 *
 * <p><strong>Note:</strong> if you're planning on adding parameter-based validation, please
 * consider declaring your model parameters using {@link
 * IMappableParameterDeclarationState#model(Class)}, mapping those declared parameters using one of
 * the methods defined in {@link IChosenParameterState} or {@link IOneChosenParameterState} and
 * adding validation using the same interfaces. This will allow you to build a {@link
 * ILinkDescriptorMapper link descriptor mapper}, suitable for use in declarative builders such as
 * {@link org.iglooproject.wicket.more.markup.repeater.table.builder.DataTableBuilder}.
 */
public interface IValidatorState<TSelf extends IValidatorState<TSelf>> {
   ...
   *
   * <p><strong>Note:</strong> if your validator depends on one of the parameters, and you're
   * building a {@link ILinkDescriptorMapper link descriptor mapper}, please consider calling {@link
   ...
   * that will pass validation if and only if the given condition is <code>true</code>, and raise an
   * error otherwise.
   *
   * <p><strong>Note:</strong> if your condition depends on one of the parameters, and you're
   * building a {@link ILinkDescriptorMapper link descriptor mapper}, please consider calling {@link
   * IChosenParameterState#validator(org.iglooproject.wicket.more.markup.html.factory.IDetachableFactory)}
   * instead.
   *
   ...
   * object contained in the given model, and raise an error otherwise.
   *
   * <p><strong>Note:</strong> if the given model is one of the link parameters, and you're building
   * a {@link ILinkDescriptorMapper link descriptor mapper}, please consider calling {@link
   * IOneChosenParameterState#permission(String)} instead.
   *
   ...
   * given permissions on the object contained in the given model, and raise an error otherwise.
   *
   * <p><strong>Note:</strong> if the given model is one of the link parameters, and you're building
   * a {@link ILinkDescriptorMapper link descriptor mapper}, please consider calling {@link
   * IOneChosenParameterState#permission(String, String...)} instead.
   *
  ...
  /**
   * Shorthand for <code>
   * permission(BindingModel.of(model, binding), firstPermissionName, otherPermissionNames)</code>
   *
   * @see #permission(IModel, String, String...)
   */
  <R, T> TSelf permission(
      IModel<R> model,
