...
abstract class AbstractMainStateImpl<
  ...
  protected static <T> IDetachableFactory<Tuple, IModel<T>> constantModelFactory(
      IModel<T> earlyTargetDefinitionTargetModel) {
  ...
  @Override
  public <T> IAddedParameterMappingState<TSelf> map(
      String name, IModel<T> valueModel, Class<T> valueType) {
    Args.notNull(name, "name");
    Args.notNull(valueModel, "valueModel");
    Args.notNull(valueType, "valueType");

    return map(
        new SimpleLinkParameterMappingEntry<T>(
            name,
            valueModel,
            LinkParameterTypeInformation.valueOf(valueType).getTypeDescriptorSupplier()));
  }

  @Override
  @SuppressWarnings("rawtypes")
  public <RawC extends Collection, C extends RawC, T>
      IAddedParameterMappingState<TSelf> mapCollection(
          String parameterName,
          IModel<C> valueModel,
          Class<RawC> rawCollectionType,
          Class<T> elementType) {
    return mapCollection(
        parameterName, valueModel, rawCollectionType, TypeDescriptor.valueOf(elementType));
  }
  ...
  public <RawC extends Collection, C extends RawC, T>
    ...
    return map(
        new CollectionLinkParameterMappingEntry<>(
            parameterName,
            valueModel,
  ...
  protected final <TTuple extends Tuple> IAddedParameterMappingState<TSelf> doMap(
      final ILinkParameterMappingEntryFactory<TTuple> factory, List<Integer> parameterIndices) {
    LinkParameterMappingEntryBuilder<TTuple> builder =
        new LinkParameterMappingEntryBuilder<>(factory);
  ...
  private static class ConstantLinkParameterMappingEntryFactory
      implements ILinkParameterMappingEntryFactory<Tuple> {
    private static final long serialVersionUID = 1L;

    private final ILinkParameterMappingEntry parameterMappingEntry;

    protected ConstantLinkParameterMappingEntryFactory(
        ILinkParameterMappingEntry parameterMappingEntry) {
      super();
  ...
  @Override
  public <T> IAddedParameterMappingState<TSelf> renderInUrl(
      String parameterName, IModel<T> valueModel) {
    return map(new InjectOnlyLinkParameterMappingEntry<>(parameterName, valueModel));
  }

  @Override
  public <R, T> IAddedParameterMappingState<TSelf> renderInUrl(
      String parameterName, IModel<R> rootModel, BindingRoot<R, T> binding) {
    return map(
        new InjectOnlyLinkParameterMappingEntry<>(
            parameterName, BindingModel.of(rootModel, binding)));
  }

  protected final <TTuple extends Tuple> TSelf doValidator(
      final ILinkParameterValidatorFactory<TTuple> factory, List<Integer> parameterIndices) {
  ...
  public TSelf validator(ILinkParameterValidator validator) {
    ...
    return doValidator(
        new ConstantLinkParameterValidatorFactory(validator), ImmutableList.<Integer>of());
  }

  private static class ConstantLinkParameterValidatorFactory
      implements ILinkParameterValidatorFactory<Tuple> {
    private static final long serialVersionUID = 1L;
  ...
  public TSelf permission(IModel<?> model, String permissionName) {
    return validator(
        new ConditionLinkParameterValidator(Condition.permission(model, permissionName)));
  }

  @Override
  public TSelf permission(
      IModel<?> model, String firstPermissionName, String... otherPermissionNames) {
    return validator(
        new ConditionLinkParameterValidator(
            anyPermission(model, firstPermissionName, otherPermissionNames)));
  }
  ...
  protected final <TTarget, TLinkDescriptor>
      IBuilderMapperLinkDescriptorFactory<TLinkDescriptor> mapperLinkDescriptorFactory(
