...
public class BodyBreadCrumbPanel extends GenericPanel<List<BreadCrumbElement>> {
  ...
  public BodyBreadCrumbPanel(
    ...
    super(
        id,
        new BreadCrumbElementListConcatModel(
            prependedBreadCrumbElementsModel,
            breadCrumbElementsModel,
            numberOfElementsToSubstract));
  }
  ...
  protected void onInitialize() {
    ...
    add(
        new BreadCrumbListView(
            ...
            getModel(),
            BreadCrumbMarkupTagRenderingBehavior.HTML_BODY,
            dividerModel));

    add(Condition.collectionModelNotEmpty(getModel()).thenShowInternal());

    add(
        new WebMarkupContainer("trailingLi") {
