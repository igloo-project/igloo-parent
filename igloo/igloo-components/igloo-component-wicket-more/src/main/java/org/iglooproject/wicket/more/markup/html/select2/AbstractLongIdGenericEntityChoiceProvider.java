...
public abstract class AbstractLongIdGenericEntityChoiceProvider<T extends GenericEntity<Long, ?>>
    extends AbstractChoiceRendererChoiceProvider<T> {
  ...
  public AbstractLongIdGenericEntityChoiceProvider(
      Class<T> clazz, IConverter<? super T> converter) {
    this(clazz, new GenericEntityConverterChoiceRenderer<>(converter));
  }

  public AbstractLongIdGenericEntityChoiceProvider(
      Class<T> clazz, IChoiceRenderer<? super T> choiceRenderer) {
  ...
  public Collection<T> toChoices(Collection<String> ids) {
    return ids.stream()
        .map(id -> entityConverter.convertToObject(id, null /* unused */))
