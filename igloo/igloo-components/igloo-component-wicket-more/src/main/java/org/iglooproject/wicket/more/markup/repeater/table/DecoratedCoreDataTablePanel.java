...
public class DecoratedCoreDataTablePanel<T, S extends ISort<?>> extends Panel
  ...
  public DecoratedCoreDataTablePanel(
    ...
    add(
        new EnclosureContainer("headingAddInContainer")
            .condition(
                Condition.visible(headingMainAddinWrapper)
                    .or(
                        Condition.visible(headingRightAddinWrapper)
                            .or(Condition.visible(headingLeftAddinWrapper))))
            .add(
        ...
        new EnclosureContainer("footerAddInContainer")
            .condition(
                Condition.visible(footerMainAddinWrapper)
                    .or(
                        Condition.visible(footerRightAddinWrapper)
                            .or(Condition.visible(footerLeftAddinWrapper))))
            .add(
  ...
  protected CoreDataTable<T, S> newDataTable(
      String id,
      IDataTableFactory<T, S> factory,
      Map<IColumn<T, S>, Condition> columns,
      ISequenceProvider<T> sequenceProvider,
  ...
  public static class CountAddInComponentFactory implements IComponentFactory<Component> {
    ...
    public CountAddInComponentFactory(
        ISequenceProvider<?> sequenceProvider, String countResourceKey) {
      super();
      this.sequenceProvider = sequenceProvider;
    ...
    @Override
    public Component create(String wicketId) {
      IModel<Integer> countModel =
          new PropertyModel<>(
              sequenceProvider, DataProviderBindings.iBindableDataProvider().size().getPath());
      return new CountLabel(wicketId, countResourceKey, countModel);
    }
  }

  public static class AjaxPagerAddInComponentFactory
      implements IOneParameterComponentFactory<Component, DecoratedCoreDataTablePanel<?, ?>> {
    private static final long serialVersionUID = 7358590231263113101L;
  ...
  public static class PagerAddInComponentFactory
      implements IOneParameterComponentFactory<Component, IPageable> {
    private static final long serialVersionUID = 7358590231263113101L;
