 ...
 * 
 * Les pourcentages affichés entre 0 et 100 par exemple correspondent en BigDecimal à des valeurs :
 * - entre 0 et 1 si modelIsRatio est true
 * - entre 0 et 100 si modelIsRatio est false
 * 
 * Le scale d'un BigDecimal correspond au nombre de chiffres après la virgule.
 * La precision d'un BigDecimal correspond au nombre total de chiffre.
 * 
 * Rmq : l'éventuel 0 devant la virgule ne compte pas dans la precision, au contraire des 0 à droite.
 * Ainsi, 0.504400 aurait une precision de 6 (et un scale de 6).
 * 
 * On attribue un scale fixe et une precision maximale autorisée aux instances de BigDecimal manipulées.
 */
public class PercentageBigDecimalConverter implements IConverter<BigDecimal> {
	...
	/**
	 * Les # après la virgule sont importants car la valeur sera tronquée.
	 * Les # avant la virgule n'ont pas d'importance.
	 */
	...
	public PercentageBigDecimalConverter(int scale, RoundingMode roundingMode, int maxPrecision, boolean modelIsRatio) {
		this(scale, roundingMode, maxPrecision, modelIsRatio, false);
	}

	public PercentageBigDecimalConverter(int scale, RoundingMode roundingMode, int maxPrecision,
			boolean modelIsRatio, boolean displayPercentSymbol) {
		super();
		if (maxPrecision < scale) {
			throw new IllegalArgumentException("The maximum precision must be equal or superior to the scale.");
		}
		this.scale = scale;
	...
	public BigDecimal convertToObject(String value, Locale locale) {
		...
		if (bigDecimal.precision() > maxPrecision) {
			...
			e.setConverter(this);
			e.setVariable("maxBeforePoint", modelIsRatio ? (maxPrecision - scale + 2) : (maxPrecision - scale));
	...
	private DecimalFormatSymbols getDecimalFormatSymbols() {
		return new DecimalFormatSymbols(CoreWicketApplication.get().getNumberFormatLocale());
	}
