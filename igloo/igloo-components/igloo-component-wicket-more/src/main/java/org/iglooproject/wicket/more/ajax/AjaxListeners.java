...
public final class AjaxListeners {
  ...
  public static void add(
      AjaxRequestTarget target, Iterable<? extends AjaxRequestTarget.IListener> listeners) {
  ...
  public static void add(
      AjaxRequestTarget target, Map<Collection<IListener>, Condition> listeners) {
  ...
  public static void addChildrenIfVisible(
      final AjaxRequestTarget target, MarkupContainer parent, Class<?> childCriteria) {
    ...
    Visits.visitChildren(
        ...
        VisitFilters.every(
            VisitFilters.including(childCriteria), VisitFilters.renderedComponents()));
  }
  ...
  public static AjaxRequestTarget.IListener refreshPage() {
    return new SerializableListener() {
      ...
      public void onBeforeRespond(Map<String, Component> map, AjaxRequestTarget target) {
        // target.add(target.getPage()); // Won't work, since the handler already decided started to
        // respond without redirecting when this method is called
  ...
  /**
   * Adds or remove items of the given refreshing view using javascript, without refreshing any of
   * the existing items that were not removed nor added during this request.
   *
   * <p><strong>WARNING:</strong> the repeater must ensure that its items are reused after a
   * refresh. This means in particular that callers should make sure to call {@link
   * RefreshingView#setItemReuseStrategy(org.apache.wicket.markup.repeater.IItemReuseStrategy)} on
   * their {@link RefreshingView} with something like {@link
   * ReuseIfModelsEqualStrategy#getInstance()} as a parameter, or to call {@link
   * ListView#setReuseItems(boolean)} on their {@link ListView}.
   *
   * <p><strong>WARNING:</strong> removing markup of removed items will only work for {@link
   * ListView}s and {@link RefreshingView}. Instances of {@link RepeatingView} (or subclasses) do
   * not allow detection of the removed items.
   *
   * <p><strong>WARNING:</strong> added items will be added as last child of the repeater's parent.
   * If it's not what you want, you may simply wrap the repeater in a {@link WebMarkupContainer}
   * whose single child is the repeater.
   */
  public static AjaxRequestTarget.IListener refreshNewAndRemovedItems(
      final IRefreshableOnDemandRepeater repeater) {
    if (!repeater.getParent().getOutputMarkupId()) {
      LOGGER.warn(
          "Trying to update new and removed items on a repeater whose parent does not"
              + " output its markup id. This is likely to fail on the client side. Repeater: {}",
          repeater);
    }
    ...
    public void onBeforeRespond(Map<String, Component> map, AjaxRequestTarget target) {
      ...
      for (Component removedItem : Sets.difference(itemsBefore, itemsAfter)) {
        if (!removedItem.getOutputMarkupId()) {
          LOGGER.warn(
              "Trying to remove a repeater item that does not"
        ...
        target.prependJavaScript(
            String.format("Wicket.$('%s').remove();", removedItem.getMarkupId()));
      }
  ...
  /**
   * @return A listener that will trigger the refresh of every component in the page of the given
   *     class(es).
   */
  @SafeVarargs
  public static AjaxRequestTarget.IListener refresh(
  ...
  public static AjaxRequestTarget.IListener refresh(final IVisitFilter... additiveFilters) {
    return new SerializableListener() {
      ...
      public void onBeforeRespond(Map<String, Component> map, AjaxRequestTarget target) {
        Visits.visitChildren(
            target.getPage(), Visitors.addToTarget(target), VisitFilters.every(additiveFilters));
      }
    };
  }

  @SafeVarargs
  public static AjaxRequestTarget.IListener refreshChildren(
      final MarkupContainer parent,
      Class<? extends Component> first,
      Class<? extends Component>... rest) {
    return refreshChildren(parent, VisitFilters.including(first, rest));
  }

  public static AjaxRequestTarget.IListener refreshChildren(
      final MarkupContainer parent, IVisitFilter... additiveFilters) {
    ...
    return new SerializableListener() {
      ...
      @Override
      public void onBeforeRespond(Map<String, Component> map, AjaxRequestTarget target) {
        Visits.visitChildren(parent, Visitors.addToTarget(target), filter);
      }
    };
  }
  ...
  /**
   * @return A listener that will trigger the refresh of every component in the page of the given
   *     class(es), <strong>except <code>self</code> and components in {@link IAjaxModalPopupPanel
   *     modals}</strong>.
   */
  @SafeVarargs
  public static <T extends Component> AjaxRequestTarget.IListener refreshOthersNotInAjaxModals(
      final T self, Class<? extends T> first, Class<? extends T>... rest) {
  ...
  public static ImmutableMap.Builder<Collection<AjaxRequestTarget.IListener>, Condition>
      chainCondition() {
