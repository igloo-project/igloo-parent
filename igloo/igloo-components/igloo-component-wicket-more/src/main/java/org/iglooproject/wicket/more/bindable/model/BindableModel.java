...
public class BindableModel<E> implements IBindableModel<E> {
  ...
  private static enum BindableModelWrappingFunction
      implements SerializableFunction2<IModel, IBindableModel> {
    INSTANCE;
  ...
  public static <T> SerializableFunction2<? super T, ? extends IBindableModel<T>> factory(
      SerializableFunction2<? super T, ? extends IModel<T>> function) {
   ...
   *   <li>Value: the {@link BindableModel} representing the property (which might be a {@link
   *       BindableCollectionModel} or a {@link BindableMapModel}.
  ...
  @Override
  public <T> IBindableModel<T> bindWithCache(
      BindingRoot<? super E, T> binding, IModel<T> workingCopyProposal) {
    ...
    if (propertyModel.hasCache()) {
      ...
      WorkingCopyModel<T> workingCopyModel =
          WorkingCopyModel.of(propertyModel.getDelegateModel(), workingCopyProposal);
  ...
  /**
   * Records the impacting paths, i.e. the paths such that when we read the cache on them
   * (respectively, write), then the given model should be read (respectively, written), too.
   *
  ...
  private <T, C extends Collection<T>> IBindableCollectionModel<T, C> bindCollectionWithCache(
    ...
    if (propertyModel == null) {
      BindableCollectionModel<T, C> collectionPropertyModel =
          new BindableCollectionModel<>(
      ...
      @SuppressWarnings("unchecked") // Generic parameters are known, by construction.
      IBindableCollectionModel<T, C> collectionPropertyModel =
          (IBindableCollectionModel<T, C>) propertyModel;
      return collectionPropertyModel;
    }
  }
  ...
  private static IllegalStateException newNotCollectionModelException(FieldPath path) {
    return new IllegalStateException(
        "The model bound to property '"
            + path
            + "' was registered using bind() or bindWithCache(). Thus, it"
  ...
  @Override
  public <T, C extends Collection<T>> IBindableCollectionModel<T, C> bindCollectionAlreadyAdded(
      BindingRoot<? super E, C> binding) {
    FieldPath path = FieldPath.fromBinding(binding);
    return bindCollectionAlreadyAdded(path);
  }

  private <T, C extends Collection<T>> IBindableCollectionModel<T, C> bindCollectionAlreadyAdded(
      FieldPath path) {
    BindableModel<?> owner = getOrCreateSimpleModel(path.parent().get());
    if (owner != this) {
    ...
    if (propertyModel == null) {
      throw new NoSuchModelException(
          "No collection model was added for path '"
              + path
  ...
  private <K, V, M extends Map<K, V>> IBindableMapModel<K, V, M> bindMapWithCache(
    ...
    if (propertyModel == null) {
      BindableMapModel<K, V, M> mapPropertyModel =
          new BindableMapModel<>(
  ...
  @Override
  public <K, V, M extends Map<K, V>> IBindableMapModel<K, V, M> bindMapAlreadyAdded(
      BindingRoot<? super E, M> binding) {
    FieldPath path = FieldPath.fromBinding(binding);
    return bindMapAlreadyAdded(path);
  }

  private <K, V, M extends Map<K, V>> IBindableMapModel<K, V, M> bindMapAlreadyAdded(
      FieldPath path) {
    BindableModel<?> owner = getOrCreateSimpleModel(path.parent().get());
    if (owner != this) {
    ...
    if (propertyModel == null) {
      throw new NoSuchModelException(
          "No map model was added for path '"
              + path
   ...
   */
  private <T> BindableModel<T> getOrCreateSimpleModel(
      BindingRoot<? super E, T> binding, FieldPath path) {
  ...
  private <T> BindableModel<T> getOrCreateSimpleModel(FieldPath path) {
    if (path.isRoot()) {
      ...
      return getOrCreateSimpleModel(parentPath).getOrCreateSimpleModel(relativePropertyPath);
    }
  }
  ...
  public final void writeAll() {
    write();
    if (propertyModels != null
        // Avoid an error in Wicket property resolver ("Runtime exception when trying to set a value
        // on a null object.")
        && getObject() != null) {
  ...
  public final <T2> void readAllUnder(BindingRoot<? super E, T2> binding) {
    if (propertyModelsByImpactingPaths != null) {
      for (BindableModel<?> propertyModel :
          propertyModelsByImpactingPaths.get(FieldPath.fromBinding(binding))) {
        propertyModel.readAll();
      }
    }
  }
