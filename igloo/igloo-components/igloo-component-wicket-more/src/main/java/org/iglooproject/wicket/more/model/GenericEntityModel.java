...
public class GenericEntityModel<
  ...
  static {
    ...
    if (EXTENDED_DEBUG_INFO) {
      LOGGER.warn(
          "Extended debug info for GenericEntityModel is enabled."
              + " This may cause a significant performance hit.");
    }
  }

  @SuppressWarnings(
      "unchecked") // SerializableModelFactory works for any T extending GenericEntity<?, ?>
  public static <E extends GenericEntity<?, ?>>
      SerializableFunction2<E, GenericEntityModel<?, E>> factory() {
  ...
  /**
   * L'objectif est ici de stocker les entités qui n'ont pas encore été persistées en base
   * (typiquement, quand on fait la création).
   */
  private E notYetPersistedEntity;

  public static <K extends Serializable & Comparable<K>, E extends GenericEntity<K, ?>>
      GenericEntityModel<K, E> of(E entity) {
  ...
  public void setObject(E entity) {
    ...
    updateSerializableData(); // Useful to keep equals() and getId() up-to-date and for
  ...
  /**
   * Updates the serializable data (id, clazz, notYetPersistedEntity) according to the attached
   * object's value.
   *
  ...
  /**
   * If the entity has been persisted since this model has been detached, then fix the serializable
  ...
  private void writeObject(ObjectOutputStream out) throws IOException {
    if (attached) {
      LOGGER.warn(
          ...
          persistedEntityReference,
          notYetPersistedEntity);
      if (EXTENDED_DEBUG_INFO) {
        LOGGER.debug(
