...
public final class BuilderMapperLinkDescriptorFactory<TTarget, TLinkDescriptor>
    ...
    switch (size) {
      ...
      default:
        throw new IllegalStateException(
            "Only Unit, Pair, Triplet and Quartet parameters are supported for ILinkParameterMappingEntryFactory");
    }
  }
  ...
  public final TLinkDescriptor create(Tuple parameters) {
    ...
    IModel<? extends TTarget> target =
        doCreate(
            (IDetachableFactory<? extends Tuple, ? extends IModel<? extends TTarget>>)
                targetFactory.getValue0(),
            parameters,
            targetFactory.getValue1());

    for (Map.Entry<LinkParameterMappingEntryBuilder<?>, List<Integer>> entry :
        mappingEntryBuilders.entrySet()) {
    ...
    for (Map.Entry<ILinkParameterValidatorFactory<?>, List<Integer>> entry :
  ...
  private static <T extends Tuple, R> R doCreate(
      IDetachableFactory<T, R> factory, Tuple parameters, List<Integer> indices) {
