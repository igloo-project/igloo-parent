...
public class ReadOnlyCollectionModel<T, C extends Collection<T>>
  ...
  protected ReadOnlyCollectionModel(
      IModel<? extends C> readModel,
      SerializableFunction2<? super T, ? extends IModel<T>> keyFactory) {
    super();
  ...
  public final Iterator<IModel<T>> iterator(long offset, long limit) {
    // Transform must be the last operation, in order to let collectionModelIterator() optimize
    // stuff
