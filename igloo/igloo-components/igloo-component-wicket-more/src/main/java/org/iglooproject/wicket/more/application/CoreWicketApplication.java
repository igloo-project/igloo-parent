...
public abstract class CoreWicketApplication extends WebApplication
    implements ICoreApplication, IBootstrapApplication {
  ...
  @Autowired protected ICachedScssService scssService;

  /**
   * Initialized with an empty list to circumvent {@link NullPointerException} when no autowirable
   * bean are available.
   */
  ...
  public static CoreWicketApplication get() {
    ...
    throw new WicketRuntimeException(
        "There is no CoreWicketApplication attached to current thread "
  ...
  public void init() {
    ...
    // Custom comparator to have a specific order for some header items, fallback to default
    // comparator.
    ...
    getStoreSettings()
        .setMaxSizePerSession(
            Bytes.megabytes(propertyService.get(WICKET_DISK_DATA_STORE_MAX_SIZE_PER_SESSION)));
    ...
    if (StringUtils.hasText(wicketDiskDataStorePath)) {
      try {
        ...
        LOGGER.error(
            String.format(
                "Unable to define a specific path (%1$s) for wicket data store. Using the default one.",
                wicketDiskDataStorePath),
            e);
      }
    }
  ...
  private void initBootstrap(ApplicationContext applicationContext) {
    Map<String, IBootstrapProvider> providers =
        applicationContext.getBeansOfType(IBootstrapProvider.class);
    IBootstrapProvider bootstrap4Provider =
        providers.values().stream()
            .filter(p -> BootstrapVersion.BOOTSTRAP_4.equals(p.getVersion()))
            .findFirst()
            .orElse(null);
    IBootstrapProvider bootstrap5Provider =
        providers.values().stream()
            .filter(p -> BootstrapVersion.BOOTSTRAP_5.equals(p.getVersion()))
            .findFirst()
            .orElse(null);
    bootstrapSettings =
        new BootstrapSettings(BootstrapVersion.BOOTSTRAP_5, bootstrap4Provider, bootstrap5Provider);
  }
  ...
  protected void addResourceReplacements() {
    addResourceReplacement(
        org.wicketstuff.wiquery.ui.JQueryUIJavaScriptResourceReference.get(),
        JQueryUIJavaScriptResourceReference.get());
    addResourceReplacement(
        org.wicketstuff.wiquery.ui.themes.WiQueryCoreThemeResourceReference.get(),
        JQueryUiCssResourceReference.get());
  }
  ...
  protected void updateResourceBundles() {
    ...
    modules.stream().forEach(module -> module.updateResourceBundles(getResourceBundles()));
  }
  ...
  protected void registerImportScopes() {
    ...
    modules.stream().forEach(module -> module.registerImportScopes(scssService));
  }

  protected void updateResourceSettings() {
    modules.stream().forEach(module -> module.updateResourceSettings(getResourceSettings()));
  }
  ...
  public final <T extends Page> void mountUnversionedPage(
      final String path, final Class<T> pageClass) {
    mount(new NoVersionMountedMapper(path, pageClass));
  }

  public final <T extends Page> void mountParameterizedPage(
      final String path, final Class<T> pageClass) {
  ...
  protected void preloadStyleSheets(ResourceReference... resourcesReferences) {
    for (ResourceReference resourceReference : resourcesReferences) {
      LOGGER.info(
          ...
          resourceReference.getScope().getName(),
  ...
  /**
   * Overriden to integrate configurationType via bean injection and properties file in place of
   * web.xml or system properties definition
   */
  @Override
  public RuntimeConfigurationType getConfigurationType() {
    return RuntimeConfigurationType.valueOf(
        propertyService.get(CONFIGURATION_TYPE).toUpperCase(Locale.ROOT));
  }
