...
/**
 * A {@link Image} whose targeted {@link ResourceReference} and {@link PageParameters} may change
 * during the page life cycle (for instance on an Ajax refresh).
 *
 ...
 * {@link ResourceReference} is invalid, then a {@link LinkInvalidTargetRuntimeException} will be
 * thrown. <br>
 * This is an expected behavior: you should either ensure that your target and parameters are always
 * valid, or that this image is hidden when they are not. <br>
 * The latter can be obtained by either using {@link #hideIfInvalid()}, or adding custom {@link
 * Behavior behaviors} that would use the {@link #setVisibilityAllowed(boolean)} method.
 *
...
public class DynamicImage extends Image {
   ...
   *
   * <p>Default behavior is throwing a {@link LinkInvalidTargetRuntimeException} or a {@link
  ...
  protected void onComponentTag(ComponentTag tag) {
    ...
    if (resourceReference == null) {
      throw new IllegalStateException(
          "The target ResourceReference of an image of type "
              + getClass()
              + " was null when trying to render the url.");
    }
  ...
  /**
   * Sets whether Wicket should automatically add a timestamp parameter so as to bypass browser
   * cache. <strong>Warning:</strong> use with caution, since this bypasses browser cache
   * completely. If there are multiple references to the same image in a single page, the browser
   * will have to download the image once for each reference.
   *
   * @param shouldAddAntiCacheParameterCondition The condition on which to add the anticache
   *     parameter automatically.
   */
  public DynamicImage setShouldAddAntiCacheParameterCondition(
      Condition shouldAddAntiCacheParameterCondition) {
