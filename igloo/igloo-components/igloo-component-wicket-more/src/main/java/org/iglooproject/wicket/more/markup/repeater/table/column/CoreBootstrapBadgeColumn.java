...
public class CoreBootstrapBadgeColumn<T, S extends ISort<?>, C> extends AbstractCoreColumn<T, S> {
  ...
  public CoreBootstrapBadgeColumn(
    ...
    cellItem.add(
        new CoreBootstrapBadgeLinkColumnPanel<T, ISort<?>, C>(componentId, rowModel) {
          ...
          public Component getBootstrapBadge(String wicketId, IModel<T> rowModel) {
            return BootstrapRequestCycle.getSettings()
                .badgeSupplier(wicketId, modelFactory.create(rowModel), renderer)
                .get()
                .badgePill(badgePill)
                .showIcon(showIcon)
                .showLabel(showLabel)
          ...
          public MarkupContainer getSideLink(String wicketId, IModel<T> rowModel) {
            ...
            true) // Delete useless <div> under <td>. If it could be usefull, give the ability to
        // configure it
        );
  }
  ...
  public ILinkDescriptorMapper<? extends ILinkGenerator, ? super IModel<T>>
      getLinkGeneratorMapper() {
    return linkGeneratorMapper;
  }

  public CoreBootstrapBadgeColumn<T, S, C> setLinkGeneratorMapper(
      ILinkDescriptorMapper<? extends ILinkGenerator, ? super IModel<T>> linkGeneratorFactory) {
    if (sideLinkGeneratorMapper != null) {
  ...
  public ILinkDescriptorMapper<? extends ILinkGenerator, ? super IModel<T>>
      getSideLinkGeneratorMapper() {
    return sideLinkGeneratorMapper;
  }

  public CoreBootstrapBadgeColumn<T, S, C> setSideLinkGeneratorMapper(
      ILinkDescriptorMapper<? extends ILinkGenerator, ? super IModel<T>> sideLinkGeneratorFactory) {
    if (linkGeneratorMapper != null) {
      throw new IllegalStateException("link and side link cannot be both set.");
    }
