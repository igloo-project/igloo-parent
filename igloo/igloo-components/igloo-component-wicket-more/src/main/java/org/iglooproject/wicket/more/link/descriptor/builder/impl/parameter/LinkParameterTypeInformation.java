...
public class LinkParameterTypeInformation<T> {
  ...
  /**
   * We must wrap the typedescriptor in a supplier, since it is not serializable, and clients are
   * serializable (so they cannot hold references to non-serializable objects).
   */
  ...
  public static <T> LinkParameterTypeInformation<T> valueOf(final Class<T> clazz) {
    return new LinkParameterTypeInformation<>(() -> TypeDescriptor.valueOf(clazz), null);
  }

  public static <T extends Collection<TElement>, TElement>
      LinkParameterTypeInformation<T> collection(
          final Class<? super T> clazz, final Class<TElement> elementType) {
    return new LinkParameterTypeInformation<>(
        () -> TypeDescriptor.collection(clazz, TypeDescriptor.valueOf(elementType)), null);
  }
  ...
  public static <T extends Collection<?>> LinkParameterTypeInformation<T> collection(
    ...
    return new LinkParameterTypeInformation<>(
        () -> TypeDescriptor.collection(clazz, elementType), null);
  }
  ...
  public static <T extends Collection<?>> LinkParameterTypeInformation<T> collection(
    ...
    return new LinkParameterTypeInformation<>(
        () -> TypeDescriptor.collection(clazz, elementType), emptyValueSupplier);
  }

  private LinkParameterTypeInformation(
