...
public abstract class AbstractActionColumnElementBuilder<T, L extends AbstractLink, F extends AbstractActionColumnElementBuilder<T, L, F>>
    ...
    private final List<IDetachableFactory<? super IModel<? extends T>, ? extends Behavior>> cssClassBehaviorFactories = Lists
	    .newArrayList();

    private final List<IDetachableFactory<? super IModel<? extends T>, ? extends Behavior>> behaviorFactories = Lists
	    .newArrayList();
	...
	link.add(getIconComponent("icon", rendererModel), getLabelComponent("label", rendererModel)).add(
		BootstrapColorBehavior.btn(rendererModel.getColorModel()),
		new AttributeModifier("title", showTooltipCondition.negate().then(Models.placeholder())
	...
	placeholder.condition(showPlaceholderCondition.negate()).add(getIconComponent("icon", rendererModel),
	...
	return new WebMarkupContainer(id).add(new ClassAttributeAppender(iconCssClassModel))
		.add(Condition.predicate(iconCssClassModel, Predicates2.hasText()).and(showIconCondition).thenShow());
    }

    private Component getLabelComponent(String id, IBootstrapRendererModel rendererModel) {
	return new CoreLabel(id, rendererModel)
		.add(Condition.predicate(rendererModel, Predicates2.hasText()).and(showLabelCondition).thenShow());
    }
    ...
    public F withClass(
	    Collection<? extends IDetachableFactory<? super IModel<? extends T>, ? extends IModel<? extends String>>> valueModelFactories) {
	this.cssClassBehaviorFactories.addAll(valueModelFactories.stream()
		// https://bugs.openjdk.java.net/browse/JDK-8212750 -> JDK-11 le type de f est
		// nécessaire dans la déclaration
		// corrigé dans JDK-12
		.map((IDetachableFactory<? super IModel<? extends T>, ? extends IModel<? extends String>> f) -> ((IDetachableFactory<IModel<? extends T>, Behavior>) itemModel -> new ClassAttributeAppender(
			f.create(itemModel))))
    ...
    public F withClass(
	    IDetachableFactory<? super IModel<? extends T>, ? extends IModel<? extends String>> valueModelFactory) {
	...
	Lists.asList(Objects.requireNonNull(firstValue), otherValues).stream().map(Model::of).forEach(this::withClass);
	return thisAsF();
    }

    public F add(
	    Collection<? extends IDetachableFactory<? super IModel<? extends T>, ? extends Behavior>> behaviorFactories) {
	...
	return add(Lists.asList(Objects.requireNonNull(firstBehavior), otherBehaviors).stream()
		.map(DetachableFactories::constant).collect(ImmutableList.toImmutableList()));
    }
