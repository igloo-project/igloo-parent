...
public class RangeModel<C extends Comparable<?>> implements IModel<Range<C>> {
  ...
  /**
   * Returns a point, represented by a closed range model in which the lower and the upper bounds
   * are equal.
   */
  ...
  /**
   * Returns a closed range model bounded by the values contained in the given {@code
   * lowerBoundModel} and {@code upperBoundModel}.
   *
   * @throws IllegalArgumentException if the value of {@code lowerBoundModel} is greater than the
   *     value of {@code upperBoundModel}
   */
  public static <C extends Comparable<?>> RangeModel<C> closed(
      IModel<? extends C> lowerBoundModel, IModel<? extends C> upperBoundModel) {
    return new RangeModel<>(lowerBoundModel, upperBoundModel);
  }

  /**
   * Returns a right-closed left-unbounded range model bounded by the value contained in the given
   * {@code upperBoundModel}.
   */
  public static <C extends Comparable<?>> RangeModel<C> atMost(
      IModel<? extends C> upperBoundModel) {
    return new RangeModel<>(null, upperBoundModel);
  }

  /**
   * Returns a left-closed right-unbounded range model bounded by the value contained in the given
   * {@code lowerBoundModel}.
   */
  public static <C extends Comparable<?>> RangeModel<C> atLeast(
      IModel<? extends C> lowerBoundModel) {
   ...
   *
   * @param lowerBoundModel the lower bound of this range, or null for negative infinity
   * @param upperBoundModel the upper bound of this range, or null for positive infinity
   */
