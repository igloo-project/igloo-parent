...
public final class InlineEnclosureComponentHandler extends AbstractMarkupFilter implements IComponentResolver {
	...
	public final static String INLINE_ENCLOSURE_COMPONENT_ATTRIBUTE_NAME = "enclosure-component";
		...
		if (enclosureAttr != null) {
			if (!tag.isOpen()) {
				throw new WicketParseException(
						"Close or open-close tags don't make sense for " + getInlineEnclosureComponentAttributeName(null) + ". Tag:",
						tag);
			}
			...
			if (Strings.isEmpty(tag.getId())) {
				...
				if (Strings.isEmpty(htmlId)) {
					String id = getWicketNamespace() + "_" + INLINE_ENCLOSURE_COMPONENT_ID_PREFIX + getRequestUniqueId();
					tag.setId(id);
				} else {
					tag.setId(htmlId);
				}
