 ...
 * <ul>
 *   <li>
 *       <pre>FileUploadForm.uploadFailed</pre>
 *       when the upload failed
 *   <li>
 *       <pre>FileUploadForm.uploadTooLarge</pre>
 *       when the uploaded file is too large (model: "maxSize" and "actualSize")
 *       <ul>
 */
public class FileUploadForm<T> extends Form<T> {
  ...
  @Override
  protected void onFileUploadException(
      final FileUploadException e, final Map<String, Object> model) {
    onFileUploadException(this, e, model);
  }

  public static void onFileUploadException(
      Form<?> form, FileUploadException e, Map<String, Object> model) {
    if (e instanceof FileUploadSizeException) {
      ...
      String msg =
          form.getString(
              FileUploadForm.class.getSimpleName() + '.' + UPLOAD_TOO_LARGE_RESOURCE_KEY,
              Model.ofMap(model),
              defaultValue);
      form.error(msg);
    } else {
      ...
      String msg =
          form.getString(
              FileUploadForm.class.getSimpleName() + '.' + UPLOAD_FAILED_RESOURCE_KEY,
              Model.ofMap(model),
              defaultValue);
      form.error(msg);
    }
