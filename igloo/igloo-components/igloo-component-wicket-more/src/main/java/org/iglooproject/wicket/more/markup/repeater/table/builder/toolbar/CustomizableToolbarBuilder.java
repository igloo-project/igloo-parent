...
public class CustomizableToolbarBuilder<T, S extends ISort<?>>
    implements IToolbarElementState<T, S> {
  ...
  private abstract class CustomizableToolbarBuilderWrapper implements IToolbarElementState<T, S> {
    ...
    @Override
    public IAddedToolbarCoreElementState<T, S> addComponent(
        IOneParameterComponentFactory<Component, CoreDataTable<T, S>> delegateFactory) {
  ...
  private class AddedToolbarLabelElementState
      extends AddedToolbarElementState<IAddedToolbarLabelElementState<T, S>>
  ...
  private class AddedToolbarLabelCoreElementState
      extends AddedToolbarElementState<IAddedToolbarCoreElementState<T, S>>
      implements IAddedToolbarCoreElementState<T, S> {

    private final CustomizableToolbarElementBuilder<T, S> elementBuilder;

    public AddedToolbarLabelCoreElementState(
        CustomizableToolbarElementBuilder<T, S> elementBuilder) {
      super();
  ...
  public IAddedToolbarLabelElementState<T, S> addLabel(final IModel<String> model) {
    CustomizableToolbarElementBuilder<T, S> elementBuilder =
        new CustomizableToolbarElementBuilder<>(
  ...
  private Integer getPreviousColspan(final int currentIndex) {
    ...
    for (CustomizableToolbarElementBuilder<T, S> builder :
        FluentIterable.from(builders).limit(currentIndex)) {
