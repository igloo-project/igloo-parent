...
public class ConditionFormModelValidator implements IFormModelValidator {
  ...
  public ConditionFormModelValidator(
      String errorRessourceKey, Condition condition, FormComponent<?>... formComponents) {
    this(
        errorRessourceKey,
        condition,
        ImmutableList.<FormComponent<?>>builder().add(formComponents).build());
  }

  public ConditionFormModelValidator(
      String errorRessourceKey, Condition condition, Collection<FormComponent<?>> formComponents) {
    this(new ResourceModel(errorRessourceKey), condition, formComponents);
  }

  public ConditionFormModelValidator(
      IModel<String> errorMessageModel, Condition condition, FormComponent<?>... formComponents) {
    this(
        errorMessageModel,
        condition,
        ImmutableList.<FormComponent<?>>builder().add(formComponents).build());
  ...
  public ConditionFormModelValidator(
      IModel<String> errorMessageModel,
      Condition condition,
  ...
  public void validate(Form<?> form) {
    if (!condition.applies()) {
      (formComponentOnError != null ? formComponentOnError : form)
          .error(Models.wrap(errorMessageModel, form).getObject());
    }
  }
