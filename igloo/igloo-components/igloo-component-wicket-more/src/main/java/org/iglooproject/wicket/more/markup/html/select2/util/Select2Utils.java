...
public final class Select2Utils {
	...
	private static final String DEFAULT_MATCHER = "function(term, text) { return Igloo.StringUtils.removeAccents('' + text).indexOf(Igloo.StringUtils.removeAccents('' + term))>=0; }";
	
	private static final String DEFAULT_FORMAT_RESULT = "function(result, container, query) {" +
			"var markup=[];" +
			"var text = result.text;" +
			"var term = query.term;" +
			"var match=Igloo.StringUtils.removeAccents('' + text).indexOf(Igloo.StringUtils.removeAccents('' + term));" +
			"var tl=term.length;" +
			"if (match<0) {" +
				"return text;" +
			"}" +
