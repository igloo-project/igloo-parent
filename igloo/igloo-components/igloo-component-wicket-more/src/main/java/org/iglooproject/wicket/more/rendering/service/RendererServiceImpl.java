...
public class RendererServiceImpl implements IRendererService, IRendererRegistry {
  ...
  public final <T> Renderer<? super T> findRenderer(
      Class<?> rootType, FieldPath path, Class<T> valueType) {
    Triplet<Class<?>, FieldPath, Class<?>> parameters =
        Triplet.<Class<?>, FieldPath, Class<?>>with(rootType, path, valueType);
  ...
  @SuppressWarnings("unchecked")
  private <T> Renderer<? super T> findRendererNoCache(
      Class<?> rootType, FieldPath path, Class<T> valueType) {
    Renderer<? super T> renderer =
        (Renderer<? super T>) renderers.get(Triplet.with(rootType, path, valueType));
    if (renderer != null) {
      return renderer;
    }
  ...
  /**
   * Wrapper which makes sure that a renderer will have access to all the elements provided by
   * Wicket (especially the Localizer) when it's executed.
   */
  ...
  @Override
  public final <R, T> void registerRenderer(
  ...
  @Override
  public final <T> void registerRenderer(
      Class<?> rootType, Class<T> valueType, Renderer<? super T> renderer) {
  ...
  @Override
  public final <R, T> void registerRenderer(
      Class<R> rootType, FieldPath path, Renderer<? super T> renderer) {
  ...
  public final String localize(
      final Enum<?> enumValue, final String prefix, final String suffix, final Locale locale) {
    try (ITearDownHandle handle = context().open()) {
