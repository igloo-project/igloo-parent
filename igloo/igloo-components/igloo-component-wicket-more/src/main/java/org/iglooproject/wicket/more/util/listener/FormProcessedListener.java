...
public final class FormProcessedListener
    implements IComponentInstantiationListener, IComponentOnAfterRenderListener {

  private static final FormProcessedListener INSTANCE = new FormProcessedListener();

  private static final MetaDataKey<Serializable> IS_PROCESSED =
  ...
  private static final IFormValidator PROCESSING_LISTENER =
      new IFormValidator() {
        private static final long serialVersionUID = 1L;
   ...
   *
   * <p>There is a slight difference between a form being submitted and a form being processed: when
   * a button submits a nested form, it actually submits the whole form (all the data is transfered
   * to the server) but only triggers the processing of the nested form (data submitted for the root
   * form, for instance, is ignored).
   *
   * <p>We cannot rely on the isSubmitted() method to determine if a form has been processed,
   * because it may return true for every sub-form of a root form, even though only one sub-form has
   * been processed, due to the behavior of AjaxFormSubmitBehavior that always call <code>
   * form.getRootForm().onFormSubmitted()</code> and not <code>form.onFormSubmitted()</code>.
   *
