...
public class UpdateOnChangeAjaxEventBehavior extends Behavior {
  ...
  public UpdateOnChangeAjaxEventBehavior onChange(
      final Condition condition, IListener listener, IListener... otherListeners) {
  ...
  public UpdateOnChangeAjaxEventBehavior onChange(
      Map<? extends Collection<? extends IListener>, ? extends Condition> listeners) {
    for (Entry<? extends Collection<? extends IListener>, ? extends Condition> listener :
        listeners.entrySet()) {
  ...
  public UpdateOnChangeAjaxEventBehavior onChange(
      Condition condition, Iterable<? extends IListener> listeners) {
  ...
  protected void onChange(AjaxRequestTarget target) {
    ...
    if (select2Cache) {
      target.appendJavaScript(
          new JsStatement().$(formComponent).chain("select2", "'focus'").render());
    }
