...
public class UpdateOnChangeAjaxEventBehavior extends Behavior {
	...
	private final Map<AbstractListener, Condition> onChangeListeners = Maps.newLinkedHashMap();
	...
	public UpdateOnChangeAjaxEventBehavior onChange(AbstractListener listener) {
		return onChange(Condition.alwaysTrue(), listener);
	}

	public UpdateOnChangeAjaxEventBehavior onChange(Condition condition, AbstractListener listener) {
		this.onChangeListeners.put(listener, condition);
		return this;
	}

	public UpdateOnChangeAjaxEventBehavior onChange(AbstractListener listener, AbstractListener ... otherListeners) {
		return onChange(Condition.alwaysTrue(), listener, otherListeners);
	}

	public UpdateOnChangeAjaxEventBehavior onChange(final Condition condition, AbstractListener listener, AbstractListener ... otherListeners) {
		return onChange(condition, Lists.asList(listener, otherListeners));
	}

	public UpdateOnChangeAjaxEventBehavior onChange(Iterable<? extends AbstractListener> listeners) {
		return onChange(Condition.alwaysTrue(), listeners);
	}

	public UpdateOnChangeAjaxEventBehavior onChange(Map<? extends Collection<? extends AbstractListener>, ? extends Condition> listeners) {
		for (Entry<? extends Collection<? extends AbstractListener>, ? extends Condition> listener : listeners.entrySet()) {
			onChange(listener.getValue(), listener.getKey());
		}
		return this;
	}

	public UpdateOnChangeAjaxEventBehavior onChange(Condition condition, Iterable<? extends AbstractListener> listeners) {
	...
	protected void onChange(AjaxRequestTarget target) {
		...
		for (Entry<AbstractListener, Condition> onChangeListener : onChangeListeners.entrySet()) {
