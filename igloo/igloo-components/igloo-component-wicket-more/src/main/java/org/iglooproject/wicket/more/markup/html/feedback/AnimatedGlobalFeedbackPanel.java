...
public class AnimatedGlobalFeedbackPanel extends GlobalFeedbackPanel {
  ...
  /**
   * @param id Id wicket.
   * @param autohideDelay Délai de fermeture automatique, en secondes. Si < 0 : le feedback ne se
   *     cache pas automatiquement
   */
  public AnimatedGlobalFeedbackPanel(
      String id, Integer autohideDelayValue, TimeUnit autohideDelayUnit) {
    super(id);
    this.variation = BootstrapRequestCycle.getVariation();
    setOutputMarkupId(true);

    this.autohideDelayValue =
        autohideDelayValue != null
            ? autohideDelayValue
            : propertyService.get(GLOBAL_FEEDBACK_AUTOHIDE_DELAY_VALUE);
    this.autohideDelayUnit =
        autohideDelayUnit != null
            ? autohideDelayUnit
            : propertyService.get(GLOBAL_FEEDBACK_AUTOHIDE_DELAY_UNIT);
  ...
  private static class CloseAlertBehavior extends Behavior {
    ...
    public void renderHead(Component component, IHeaderResponse response) {
      ...
      response.render(
          OnDomReadyHeaderItem.forScript(new JsQuery(component).$().chain(event).render()));
    }
  }
  ...
  public FeedbackPanel getFeedbackPanel(String id, int level, MarkupContainer component) {
    return new ErrorLevelFeedbackPanel(id, level) {
      ...
      protected void onConfigure() {
        // cf WICKET-4536 : ceci dit, même après mise à jour, on a encore eu des cas avec le
        // problème
