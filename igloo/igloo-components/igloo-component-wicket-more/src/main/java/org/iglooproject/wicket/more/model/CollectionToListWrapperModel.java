...
/**
 * A wrapper on Collection models to display them in Wicket components, such as ListView.
 *
 * <p>The returned list is {@link Collections#unmodifiableList(List) not modifiable}.
 *
 * <p>WARNING : depending on the implementation of the underlying collection, the index of items in
 * the returned list may not be the same on each call to getObject() if the collection has been
 * reloaded, even with the same content. To avoid any problem, either use {@link SortedSet sorted
 * collections} or, if using this model in a {@link ListView}, override {@link
 * ListView#getListItemModel()}.
 *
 * <p>As this model brutally copies the elements to a new list from the underlying collection, it
 * may expose performance issues if used extensively or on very large collections.
 */
public class CollectionToListWrapperModel<T> implements IModel<List<T>> {
  ...
  public static <T> CollectionToListWrapperModel<T> of(
