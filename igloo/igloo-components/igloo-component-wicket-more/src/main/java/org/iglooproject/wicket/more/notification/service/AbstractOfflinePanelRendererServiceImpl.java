...
/**
 * <p>
 * This is a common utility for "offline" page generation, for notifications or
 * queued pdf rendering for example.
 * </p>
 * 
 * <p>
 * Existing implementations are:
 * <ul>
 * <li>Extend {@link AbstractNotificationContentDescriptorFactory}, if you need
 * to render e-mail notifications.
 * <li>Extend {@link AbstractWicketRendererServiceImpl}, if you only need to
 * render components/strings
 * </ul>
 * </p>
 */
abstract class AbstractOfflinePanelRendererServiceImpl {
    ...
    protected final String renderComponent(final IOfflineComponentProvider<? extends Component> offlineComponent,
	    Locale locale) {
     ...
     * @param variation        A string identifier that will be passed to
     *                         {@link #postProcessHtml(Component, Locale, String, String)}.
     * @return The component returned by <code>offlineComponent</code>, rendered in
     *         HTML with the given <code>locale</code>
     */
    protected String renderComponent(final IOfflineComponentProvider<? extends Component> offlineComponent,
	    final Locale locale, final String variation) {
	Args.notNull(offlineComponent, "offlineComponent");
	try (ITearDownHandle handle = context(locale).open()) {
	    RequestCycle.get().setMetaData(OfflineComponentClassMetadataKey.INSTANCE,
		    offlineComponent.getComponentClass());
	    Component component = offlineComponent.getComponent();
	    Args.notNull(component, "component");
	    return postProcessHtml(component, ComponentRenderer.renderComponent(component).toString(), locale,
		    variation);
	}
    }
     ...
     *         HTML with the given <code>locale</code>
     */
    protected String renderPage(final IOfflineComponentProvider<? extends Page> offlineComponent, final Locale locale,
    ...
    /**
     * Override this method if you need to customize your rendering context. A
     * common usage is to bind context with a customized {@link WebApplication}.
     */
    ...
    /**
     * Alternative renderPage for
     * {@link ComponentRenderer#renderPage(java.util.function.Supplier)}.
     * 
     * We need to mimic mix renderComponent behavior with renderPage arguments;
     * renderComponent overrides only response, as we already perform all the other
     * initialization.
     * 
     * This method is made from
     * {@link ComponentRenderer#renderComponent(java.util.function.Supplier)} for
     * context initialization code and
     * {@link ComponentRenderer#renderPage(java.util.function.Supplier)} for
     * rendering
     */
	...
	try {
		...
		LOGGER.warn("Component class {} does not match advertised class {}", component.getClass().getName(),
