...
public abstract class AbstractAjaxInputPrerequisiteBehavior<T> extends Behavior {
  ...
  private final IFormComponentChangeObserver observer =
      new IFormComponentChangeObserver() {
  ...
  private SerializablePredicate2<? super T> resetAttachedFormComponentsPredicate =
      Predicates2.alwaysFalse();

  /**
   * @deprecated This should disappear soon, along with the {@link
   *     #cleanDefaultModelObject(Component)} method.
   */
  @Deprecated private boolean resetAttachedModelOnConfigure = true;
  ...
  public void detach(Component component) {
    ...
    Detachables.detach(
        defaultWhenPrerequisiteInvisibleCondition, forceSetUpConditon, forceTakeDownConditon);
  }

  /**
   * Sets whether the attached component should be set up (when condition is <code>true</code>) or
   * taken down (when condition is <code>false</code>) when the prerequisiteField is invisible.
   *
   * <p>Default is <code>Condition.alwaysTrue()</code>.
   */
  public AbstractAjaxInputPrerequisiteBehavior<T> setDefaultWhenPrerequisiteInvisible(
  ...
  /**
   * Sets whether the attached component should be set up (<code>true</code>) or taken down (<code>
   * false</code>) when the prerequisiteField is invisible.
   *
  ...
  /**
   * Sets whether wicket validation ({@link FormComponent#validate()} method) is to be taken into
   * account before trying to apply the {@code #setObjectValidPredicate(Predicate)
   * objectValidPredicate}.
   *
   * <p><strong>WARNING:</strong> The wicket validation relies solely on input, and not on the model
   * object. Thus any wicket validation will systematically deem a field invalid when there is no
   * input. In particular, this means that the prerequisite field will systematically be deemed
   * invalid on the first page rendering, which means this feature is not suitable for "editing"
   * forms, where the form is filled before the first rendering.
   *
   * @deprecated Avoid using this, for the reasons mentioned above.
   */
  @Deprecated
  public AbstractAjaxInputPrerequisiteBehavior<T> setUseWicketValidation(
      boolean validatePrerequisiteInput) {
    this.useWicketValidation = validatePrerequisiteInput;
    return this;
  }

  /**
   * Sets whether the prerequisite field model is to be updated when the prerequisite field input
   * changes.
   */
  public AbstractAjaxInputPrerequisiteBehavior<T> setUpdatePrerequisiteModel(
      boolean updatePrerequisiteModel) {
   ...
   */
  @Deprecated
  public AbstractAjaxInputPrerequisiteBehavior<T> setResetAttachedModel(
      boolean resetAttachedModel) {
    this.resetAttachedModelPredicate =
        resetAttachedModel ? Predicates2.alwaysTrue() : Predicates2.alwaysFalse();
    return this;
  }
   ...
   */
  @Deprecated
  public AbstractAjaxInputPrerequisiteBehavior<T> setResetAttachedModelIfInvalid(
      boolean resetAttachedModel) {
   ...
   */
  @Deprecated
  public AbstractAjaxInputPrerequisiteBehavior<T> setResetAttachedModelPredicate(
      SerializablePredicate2<? super T> resetAttachedModelPredicate) {
    this.resetAttachedModelPredicate = resetAttachedModelPredicate;
    return this;
  }

  /**
   * Sets whether the attached component and all its children are to be reset if they are
   * FormComponents, i.e. their model are to be set to null and their convertedInput are to be
   * cleared, when the prerequisite model changes.
   *
   * <p>Note that FormComponents children will not be reset (allowing the use of
   * FormComponentPanels, which should handle input clearing of their children themselves).
   */
   ...
   * Sets whether the attached component and all its children are to be reset if they are
   * FormComponents, i.e. their model are to be set to null and their convertedInput are to be
   * cleared, when the prerequisite model changes <em>and its new object is invalid</em>.
   *
   * <p>Note that FormComponents children will not be reset (allowing the use of
   * FormComponentPanels, which should handle input clearing of their children themselves).
   */
   ...
   * Sets whether the attached component and all its children are to be reset if they are
   * FormComponents, i.e. their model are to be set to null and their convertedInput are to be
   * cleared, when the prerequisite model changes and the given predicate does not apply to the
   * prerequisite model value anymore.
   */
  public AbstractAjaxInputPrerequisiteBehavior<T> setResetAttachedFormComponentsPredicate(
      SerializablePredicate2<? super T> resetAttachedFormComponentsPredicate) {
    this.resetAttachedFormComponentsPredicate = resetAttachedModelPredicate;
    return this;
  }

  /**
   * Sets whether the attached component's model are to be set to null when calling onConfigure if
   * the prerequisite model is invalid.
   *
   ...
   */
  public AbstractAjaxInputPrerequisiteBehavior<T> setObjectValidPredicate(
      SerializablePredicate2<? super T> objectValidPredicate) {
    this.objectValidPredicate = objectValidPredicate;
    return this;
  }

  /**
   * Sets the condition under which the attached component will be "set up" regardless of the value
   * of the prerequisite field.
   */
  ...
  /**
   * Sets the condition under which the attached component will be "taken down" regardless of the
   * value of the prerequisite field.
   */
  ...
  /**
   * Sets whether the ajax refresh should target the attached component's parent instead of the
   * component itself.
   *
   * <p>This is useful for components that use javascript to generate siblings in the DOM tree, such
   * as Select2.
   */
  ...
  public final void bind(Component component) {
    if (attachedComponents.isEmpty()) {
      // Make sure that the attached component will be updated when the content of the
      // prerequisiteField changes.
  ...
  private void onPrerequisiteFieldChange(AjaxRequestTarget target) {
    ...
    for (Component attachedComponent : attachedComponents) {
      ...
      if (hasReset) {
        ...
        FormComponentChangeAjaxEventBehavior behavior =
            FormComponentChangeAjaxEventBehavior.getExisting(attachedComponent);
        if (behavior != null) {
    ...
    onPrerequisiteFieldChange(
        target, prerequisiteField, Collections.unmodifiableCollection(attachedComponents));
  }
  ...
  // Visits.visit is screwed: it does not accept Components, but only Iterable<Component>, on
  // contrary to Visits.visitPostOrder
  private static final <T extends Component, R> R visit(
      Component component, IVisitor<T, R> visitor, IVisitFilter filter) {
    return Visits.visitChildren(Collections.<Component>singleton(component), visitor, filter);
  }

  private IVisitor<FormComponent<?>, Void> resetFormComponentsVisitor =
      new ResetFormComponentsVisitor();

  private class ResetFormComponentsVisitor
      implements IVisitor<FormComponent<?>, Void>, Serializable {
  ...
  /**
   * Ajax call triggered by a change on the prerequisite field. Called after adding the attached
   * components to the target, and before generating the response.
   *
   * @deprecated Use {@link #onChange(IListener)} instead.
   */
  @Deprecated
  protected void onPrerequisiteFieldChange(
      AjaxRequestTarget target,
      FormComponent<T> prerequisiteField,
      Collection<Component> attachedComponents) {}

  private boolean hasPrerequisiteFieldInputChanged() {
    return prerequisiteField.isEnabledInHierarchy()
        && prerequisiteField.isVisibleInHierarchy()
        && (prerequisiteField.getForm().isSubmitted()
            || FormComponentChangeAjaxEventBehavior.getExisting(prerequisiteField)
                .isBeingSubmitted());
  }
    ...
    if (prerequisiteField.isVisibleInHierarchy()) {
      if (forceTakeDownConditon != null && forceTakeDownConditon.applies()) {
        ...
        if (hasPrerequisiteFieldInputChanged()) {
          // The prerequisiteField input has changed : the rendering of the attached component was
          // triggered either by our
          // InputPrerequisiteAjaxEventBehavior or by a form submit.
          // We will decide whether the attached component should be set up or taken down based on
          // the prerequisiteField's input.
          prerequisiteField.validate(); // Performs input conversion
          updateModelIfNecessary(prerequisiteField);

          if (isConvertedInputSatisfyingRequirements(
              prerequisiteField, prerequisiteField.getConvertedInput())) {
            setUpAttachedComponent(component);
          } else {
          ...
          // Clearing the input seems useless here, and may harm if the input is used when rendering
          // the form component.
          //				prerequisiteField.clearInput();
        } else {
          if (useWicketValidation) {
            // TODO YRO : see if anyone is still using this, and drop it if not. This is dangerous
            // since [...]
            // it may result in using wicket to validate input even though the form was not
            // submitted.
            prerequisiteField.validate(); // Performs input conversion
          }

          if (isCurrentModelSatisfyingRequirements(
              prerequisiteField, prerequisiteField.getModel())) {
            setUpAttachedComponent(component);
          } else {
            if (resetAttachedModelOnConfigure) {
              cleanDefaultModelObject(component);
        ...
        if (useWicketValidation) {
          // We need to clear the message that may have been added during the validation, since they
          // are not relevant to the user (no form was submitted)
          prerequisiteField.getFeedbackMessages().clear();
        }
      }
    } else {
  ...
  protected final boolean isConvertedInputSatisfyingRequirements(
      FormComponent<T> prerequisiteField, T convertedInput) {
    return (!useWicketValidation || prerequisiteField.isValid()) && isObjectValid(convertedInput);
  }

  protected final boolean isCurrentModelSatisfyingRequirements(
      FormComponent<T> prerequisiteField, IModel<T> currentModel) {
    return (!useWicketValidation || prerequisiteField.isValid())
        && isObjectValid(currentModel.getObject());
  }
