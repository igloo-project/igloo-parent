...
public abstract class IndexedItemListView<T>
		extends AbstractPageableView<T> { // Does not implement IGenericComponent<C> in order to allow using IModel<? extends List<T>>, not only IModel<List<T>>
	...
	/**
	 * Useful when using a {@link ReuseIfModelsEqualStrategy} : removes the item from the underlying model AND the view
	 * and reorganizes the other items so that their respective indices are up-to-date.
	 */
	...
	/**
	 * Useful when using a {@link ReuseIfModelsEqualStrategy} : removes the item from the view (not the model) and reorganizes
	 * the other items so that their respective indices are up-to-date.
	 */
