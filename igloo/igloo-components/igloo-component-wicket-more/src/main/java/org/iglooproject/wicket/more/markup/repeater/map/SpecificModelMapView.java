...
public abstract class SpecificModelMapView<K, V, MK extends IModel<K>, MV extends IModel<V>>
  ...
  public SpecificModelMapView(
      String id, IItemModelAwareMapModel<K, V, ?, ? extends MK, ? extends MV> collectionModel) {
  ...
  @Override
  @SuppressWarnings(
      "unchecked") // The model is known to be a IItemModelAwareCollectionModel, see constructor
  ...
  protected SpecificModelItem newItem(String id, int index, IModel<K> model) {
    return new SpecificModelItem(
        id, index, (MK) model, getModel().valueModelForProvidedKeyModel(model));
  }
