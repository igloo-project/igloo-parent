...
/**
 * A panel that uses bindable models for sub-components models, in order to leverage caching
 * capabilities, and resets those caches automatically when necessary.
 *
 * <p>This type of panel is especially useful inside editing popup windows, where:
 *
 * <ul>
 *   <li>the root model may change just before opening the popup window, and thus the caches may
 *       have to be reset automatically on show.
...
public class AbstractCachingPanel<T> extends GenericPanel<T>
  ...
  protected final <T2> IBindableModel<T2> bindWithCache(
      BindingRoot<T, T2> binding, IModel<T2> workingCopyProposal) {
