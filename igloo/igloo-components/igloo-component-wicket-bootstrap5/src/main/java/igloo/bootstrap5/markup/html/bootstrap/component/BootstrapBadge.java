package igloo.bootstrap5.markup.html.bootstrap.component;
...
public class BootstrapBadge<T> extends GenericPanel<T>
    implements IBootstrapBadge<T, BootstrapBadge<T>> {
  ...
  public BootstrapBadge(String id, IModel<T> model, final IBootstrapRenderer<? super T> renderer) {
    ...
    add(
        ...
        new EnclosureContainer("separator")
            .condition(Condition.and(Condition.visible(icon), Condition.componentVisible(label))));
    ...
    add(Condition.or(Condition.visible(icon), Condition.visible(label)).thenShowInternal());
  }
