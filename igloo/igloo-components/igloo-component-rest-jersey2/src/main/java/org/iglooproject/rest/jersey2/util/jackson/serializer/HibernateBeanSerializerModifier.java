...
public class HibernateBeanSerializerModifier extends BeanSerializerModifier {
  ...
  public static class BeanSerializerWrapper extends BeanSerializer {
    ...
    private <T> T unwrapBean(T bean) {
      ...
      // Pour serializeWithType : absolument nécessaire, car la sérialisation du type ne prend pas
      // du tout
      // en compte ce qu'on a fait dans serializerFactory.
      // pour les IObjetTouristiqueLinkedReference, on n'unwrappe pas le bean car on le type
      // statiquement et on
    ...
    protected void serializeFieldsFiltered(
        Object bean, JsonGenerator jgen, SerializerProvider provider)
