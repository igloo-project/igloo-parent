...
public class DifferenceIntrospectorContext {
  ...
  private DifferenceIntrospectorContext(
      DifferenceField field, Set<Class<?>> visitedTypes, int depth) {
    this.field = field;
    this.visitedTypes = Collections.unmodifiableSet(visitedTypes);
    this.depth = depth;
  }

  public DifferenceIntrospectorContext toChild(
      DifferenceField parentField, String name, BindingRoot<?, ?> binding, Class<?> type) {
    return toChild(parentField, name, binding, type, null);
  }

  public DifferenceIntrospectorContext toChild(
      DifferenceField parentField,
      String name,
      BindingRoot<?, ?> binding,
      Class<?> type,
      BindingRoot<?, ?> containedBinding) {
    DifferenceField childField;
    childField =
        new DifferenceField(
            DifferenceIntrospectorContext.childPath(parentField, name),
            binding,
            type,
            DifferenceMode.SIMPLE,
            parentField,
            containedBinding,
            null,
            null);
  ...
  public static FieldPath childPath(DifferenceField field, String name) {
    if (field.isContainer()) {
      return field
          .getPath()
          .append(FieldPath.of(FieldPathComponent.ITEM, new FieldPathPropertyComponent(name)));
    } else {
