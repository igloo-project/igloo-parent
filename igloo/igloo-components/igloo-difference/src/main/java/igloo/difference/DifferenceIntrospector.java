 ...
 *
 * <ul>
 *   <li>To check difference algorithm behavior, and to detect behavior changes when compared
 *       objects model is changed
 *   <li>To configure an {@link AbstractConfiguredDifferenceServiceImpl} with the help of {@link
 *       DifferenceConfigurer}
 * </ul>
 */
public class DifferenceIntrospector {
  ...
  private final Map<FieldPath, AbstractContainerDifferStrategy<?, ?>> containerDifferStrategies =
      new HashMap<>();

  /** Initializers. */
  private final ListMultimap<FieldPath, IProxyInitializer<?>> initializers =
      LinkedListMultimap.create();

  /**
   * Provides container item binding to use to explore paths to collection. i.e. <code>
   * path names -> Collection&lt;String&gt; -&gt; new StringBinding()</code>.
   */
  private final Map<FieldPath, BindingRoot<?, ?>> bindings = new HashMap<>();

  public DifferenceIntrospector(
      BindingRoot<?, ?> rootBinding, Set<Class<?>> additionalSimpleTypes) {
   ...
   *
   * @return an object describing a difference configuration (list of each fields, with associated
   *     difference behavior).
   */
  ...
  private void visitBinding(DifferenceFields fields, DifferenceIntrospectorContext context) {
    ...
    AbstractContainerDifferStrategy<?, ?> containerDifferStrategy =
        containerDifferStrategies.get(context.getField().getPath());
    if (context.getDepth() > 20) {
      // stop on maxDepth overflow
      throw new IllegalStateException(
          "Max depth overflow on field %s".formatted(context.getField()));
    }
    if (getShallowPaths().contains(context.getField().getPath())) {
      ...
      fields
          ...
          .add(
              context
                  .getField()
                  .copy(DifferenceMode.SHALLOW, parentInitializers, containerDifferStrategy));
      return;
    } else {
      fields
          .getFieldList()
          .add(
              context
                  .getField()
                  .copy(DifferenceMode.DEEP, parentInitializers, containerDifferStrategy));
    }
    // else introspect field
    @SuppressWarnings("unchecked")
    List<BindingRoot<?, ?>> unsafeCast =
        (List<BindingRoot<?, ?>>)
            (Object) context.getField().getIntrospectionBinding().getChildBindings();
    for (BindingRoot<?, ?> b : unsafeCast) {
      FieldPath childPath =
          DifferenceIntrospectorContext.childPath(context.getField(), b.getName());
      ...
      if (childType == null) {
        throw new IllegalStateException(
            "Unknown type for %s. Add DifferenceInstrospector.addBinding(FieldPath.fromString(\"%s\"), new CONTAINED_TYPEBinding());"
                .formatted(childPath, childPath));
      }
      DifferenceIntrospectorContext childContext =
          context.toChild(
      ...
      if (context.getVisitedTypes().contains(childContext.getField().getType())) {
        LOGGER.debug(
            "Type {} visited twice on field {}. This may be a infinite loop",
            context.getField().getPath().append(FieldPath.fromString(b.getName())),
            b.getClass());
      }

      AbstractContainerDifferStrategy<?, ?> childDifferStrategy =
          childContext.getField().getContainerDifferStrategy();
      if (isSimpleType(childContext.getField().getType())
          || Enum.class.isAssignableFrom(childContext.getField().getType())) {
        if (childDifferStrategy != null) {
          LOGGER.warn("Container differ strategy ignored on {} (it is a simple type)", childPath);
        }
        fields
            .getFieldList()
            .add(childContext.getField().copy(DifferenceMode.SIMPLE, childInitializers, null));
      } else {
        // introspect
  ...
  @Nullable
  private BindingRoot<?, ?> getContainedBinding(
      DifferenceFields fields, FieldPath path, BindingRoot<?, ?> binding) {
   ...
   */
  @Nonnull
  private BindingRoot<?, ?> getBinding(
      DifferenceFields fields, FieldPath path, BindingRoot<?, ?> binding) {
    if (getBindings().containsKey(path)) {
      return getBindings().get(path);
    } else {
  ...
  public DifferenceIntrospector addContainerDifferStrategy(
      Binding<?> binding, AbstractContainerDifferStrategy<?, ?> containerDifferStrategy) {
    addContainerDifferStrategy(FieldPath.fromBinding(binding), containerDifferStrategy);
    return this;
  }

  public DifferenceIntrospector addContainerDifferStrategy(
      FieldPath fieldPath, AbstractContainerDifferStrategy<?, ?> containerDifferStrategy) {
  ...
  public DifferenceIntrospector addInitializer(
      Binding<?> binding, IProxyInitializer<?> initializer) {
  ...
  public Map<FieldPath, BindingRoot<?, ?>>
      getBindings() { // NOSONAR we cannot use stronger generics
    return bindings;
  }
  ...
  public static class DifferenceFieldComparator
      implements Comparator<DifferenceField> { // NOSONAR singleton is assumed
  ...
  public static class FieldPathComparator
      implements Comparator<FieldPath> { // NOSONAR singleton is assumed
    ...
    public int compare(FieldPath path1, FieldPath path2) {
      ...
      if (path1FirstComponent.equals(path2FirstComponent)) {
        ...
        return compare(
            path1.relativeTo(path1FirstComponent).get(),
            path2.relativeTo(path1FirstComponent).get());
      } else {
