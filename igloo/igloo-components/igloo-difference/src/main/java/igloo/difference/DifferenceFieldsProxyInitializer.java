...
public class DifferenceFieldsProxyInitializer implements IProxyInitializer<Object> {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(DifferenceFieldsProxyInitializer.class);
	...
	public void initialize(Object rootValue) {
		...
		Preconditions.checkArgument(skippedRootField.getPath().isRoot(), "First field must be root field");
		fieldList.remove(0);
	...
	private void initialize(List<DifferenceField> remainingFields, DifferenceInitializerContext context) {
		while (!remainingFields.isEmpty()) {
			...
			if (field.isContainer()) {
				...
				if (initializedValue instanceof Collection<?> collection) {
					...
					throw new IllegalStateException("Unexpected type %s".formatted(initializedValue.getClass().getName()));
				}
			} else {
				// initialize property
				Object initializedValue = initialize(field, context.peekRootValue());
	...
	private void initializeItem(DifferenceField field, Object initializedValue) {
		if (field.getInitializer() != null && initializedValue != null) {
			...
			IProxyInitializer<Object> unsafeInitializer = (IProxyInitializer<Object>) field.getInitializer();
			unsafeInitializer.initialize(initializedValue);
		}
	}

	private List<DifferenceField> collectChildFields(DifferenceField currentField, List<DifferenceField> remainingField) {
