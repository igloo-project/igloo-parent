...
public enum HistoryLogSort implements ISort<Function<SearchSortFactory, SortFinalStep>> {
  ID {
    ...
    public List<Function<SearchSortFactory, SortFinalStep>> getSortFields(SortOrder sortOrder) {
      return List.of(f -> f.field(AbstractHistoryLog.ID).order(toSortOrder(this, sortOrder)));
    }
  ...
  DATE {
    @Override
    public List<Function<SearchSortFactory, SortFinalStep>> getSortFields(SortOrder sortOrder) {
      return List.of(f -> f.field(AbstractHistoryLog.DATE).order(toSortOrder(this, sortOrder)));
    }
