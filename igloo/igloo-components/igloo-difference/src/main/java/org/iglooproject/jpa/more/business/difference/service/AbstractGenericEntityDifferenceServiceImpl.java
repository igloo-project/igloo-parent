...
public abstract class AbstractGenericEntityDifferenceServiceImpl<T extends GenericEntity<?, ?>>
  ...
  protected void initialize() {
    ...
    Iterable<? extends ICoreBinding<? extends T, ?>> simpleFieldsBindingsList =
  ...
  protected abstract Iterable<? extends ICoreBinding<? extends T, ?>>
      getSimpleInitializationFieldsBindings();
  ...
  protected final ObjectDifferBuilder initializeMinimalDiffer(ObjectDifferBuilder builder) {
    ...
    // We make sure, that if no nodes have been specified as included, all the other nodes won't be
    // considered
    ...
    for (BindingRoot<? super T, ?> binding : getMinimalDifferenceFieldsBindings()) {
      ...
      // However we don't use the category system here nor the NodePathInclusionResolver because it
      // would include
      // all the children of the parent (the categories are inherited by the children and the
  ...
  public <HD extends AbstractHistoryDifference<HD, ?>> List<HD> toHistoryDifferences(
    ...
    // A "leaf" is either a node without children or a node which is an element of a Collection or a
    // Map.
    rootDifference
        .getDiffNode()
        .visitChildren(
            new Visitor() {
              ...
              public void node(DiffNode node, Visit visit) {
                ...
                Collection<IHistoryDifferenceFactory<T>> specificFactories =
                    specificHistoryDifferenceFactories.get(FieldPath.of(pathComponents));
