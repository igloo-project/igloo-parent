...
/**
 * <p>Page listing all Spring-managed {@link CacheManager} / {@link Cache}.</p>
 * 
 * <p>For each {@link CacheManager}:
 * <ul>
 * <li>a button allows to reset all caches</li>
 * <li>all caches are listed</li>
 * </ul>
 * </p>
 * 
 * <p>For each {@link Cache}
 * <ul>
 * <li>statistics (hit/miss/hit ratio, current and max size) are displayed</li>
 * <li>a form allows to modify max size (only if cache is configured with a max size)</li>
 * <li>a button allows to reset the cache</li>
 * </ul>
 * </p>
 */
		...
		add(
			new ListView<CacheManagerWrapper>("cacheManagers", cacheManagerListModel) {
				...
				protected void populateItem(final ListItem<CacheManagerWrapper> item) {
					...
					IModel<String> cacheManagerNameModel = cacheManagerModel.map(CacheManagerWrapper::getName);
					
					IModel<String> filterModel = new Model<>();
					
					WebMarkupContainer cachesContainer = new WebMarkupContainer("cachesContainer");
					IModel<List<Cache>> cacheListModel = () -> item.getModelObject().getCacheManager()
							.getCacheNames().stream().map(n -> item.getModelObject().getCacheManager().getCache(n))
					...
					item.add(
						...
						AjaxConfirmLink.<CacheManagerWrapper>build()
							...
							.content(
								new StringResourceModel("console.maintenance.caches.cacheManager.purge.confirm")
							...
							.add(
								new ListView<Cache>("caches", cacheListModel) {
									...
									protected void populateItem(final ListItem<Cache> item) {
										...
										IModel<ICacheWrapper> cacheModel = () -> ICacheWrapper.wrap(item.getModelObject());
										...
										item.add(
											...
											new CoreLabel("evictionCount", BindingModel.of(cacheModel, CacheBindings.cache().cacheEvictions())),
											new CoreLabel("cacheHits", BindingModel.of(cacheModel, CacheBindings.cache().cacheHits())),
											new CoreLabel("cacheMisses", BindingModel.of(cacheModel, CacheBindings.cache().cacheMisses())),
											...
											new CoreLabel("memoryStoreObjectCount", BindingModel.of(cacheModel, CacheBindings.cache().currentSize())),
											new CoreLabel("maxElementsInMemory", BindingModel.of(cacheModel, CacheBindings.cache().maxSize())),
											new CoreLabel("cacheFillRatio",
												CoreRenderers.percent().asModel(
													PercentageFloatToBigDecimalModel.of(
														BindingModel.of(cacheModel, CacheBindings.cache().cacheFillRatio())
													)
												)
											)
										);
										...
										ConsoleMaintenanceJcacheCacheEditPopup editPopup = new ConsoleMaintenanceJcacheCacheEditPopup("editPopup", cacheModel, item);
										item.add(
											editPopup,
											new BlankLink("edit")
												.add(
										...
										item.add(
											AjaxConfirmLink.<Cache>build()
												.title(new ResourceModel("common.action.confirm.title"))
												.content(
													new StringResourceModel("console.maintenance.caches.cacheManager.cache.clear.confirm.content")
