...
public class ConsoleMaintenanceJcacheCacheEditPopup
    extends AbstractAjaxModalPopupPanel<ICacheWrapper> {

  private static final long serialVersionUID = 2798613803910080178L;

  private static final Logger LOGGER =
      LoggerFactory.getLogger(ConsoleMaintenanceJcacheCacheEditPopup.class);

  private Component parent;

  private Form<Cache> form;

  public ConsoleMaintenanceJcacheCacheEditPopup(
      String id, IModel<ICacheWrapper> cacheModel, Component parent) {
    super(id, cacheModel);
    this.parent = parent;
  }
  ...
  protected Component createBody(String wicketId) {
    ...
    form.add(
        new TextField<>(
                "maxElementsInMemory", BindingModel.of(getModel(), CacheBindings.cache().maxSize()))
            .setRequired(true)
            .setLabel(
                new ResourceModel(
                    "console.maintenance.caches.cacheManager.cache.maxElementsInMemory")));
        ...
        new AjaxButton("save", form) {
          ...
          protected void onSubmit(AjaxRequestTarget target) {
            try {
              ...
              LOGGER.error(
                  "Error modifying cache {}.",
                  ConsoleMaintenanceJcacheCacheEditPopup.this.getModelObject().getName(),
                  e);
