...
/**
 * <p>Configure {@link Cacheable} annotation, spring caching subsystem and register a cache registry needed by cache
 * console page.</p>
 * 
 ...
 * cachine) initialized by Hibernate and exposed as a bean for monitoring purpose.</p>
 * 
...
// needed to ensure that cacheManager is already defined; else @Autowired @Qualifier("cacheManager") #cacheManager may fail. 
...
public class CacheAutoConfiguration implements CachingConfigurer {
	...
	public ICacheManagerRegistry cacheManagerRegistry(Map<String, CacheManager> cms) {
		if (LOGGER.isInfoEnabled()) {
			LOGGER.info("Registering cache managers: {}", cms.keySet().stream().collect(Collectors.joining(", ")));
		}
