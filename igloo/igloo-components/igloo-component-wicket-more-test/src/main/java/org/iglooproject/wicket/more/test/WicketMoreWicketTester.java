...
public class WicketMoreWicketTester extends CoreWicketTester {
   ...
   */
  public <P extends Page> void assertDynamicBookmarkablePageLinkEnabled(
      String path, Class<P> pageClass) {
    DynamicBookmarkablePageLink linkComponent =
        assertEnabled(path, DynamicBookmarkablePageLink.class);
   ...
   */
  public <P extends Page> void assertDynamicBookmarkablePageLinkDisabled(
      String path, Class<P> pageClass) {
    DynamicBookmarkablePageLink linkComponent =
        assertDisabled(path, DynamicBookmarkablePageLink.class);
    assertThat(linkComponent.isLinkedPageAccessible(pageClass)).isTrue();
  }

  /**
   * Assert a Component is visible, of type popupClass (extends AbstractModalPopupPanel) and that
   * the internal component are invisible
   */
  public void assertModalBeforeOpening(
      String modalPath,
      String modalTitle,
      Class<?> popupClass,
    ...
    bodyFirstChildElementsId.stream()
        .forEach(id -> assertInvisible(modalPath + ":container:dialog:body:" + id));
    // Footer elements should be invisible
    footerFirstChildElementsId.stream()
        .forEach(id -> assertInvisible(modalPath + ":container:dialog:footer:" + id));
  }
  ...
  public void assertTooltip(String path, String expectedValue) {
    ...
    TagTester tagTester =
        TagTester.createTagByAttribute(
            getLastResponse().getDocument(), "id", component.getMarkupId());
