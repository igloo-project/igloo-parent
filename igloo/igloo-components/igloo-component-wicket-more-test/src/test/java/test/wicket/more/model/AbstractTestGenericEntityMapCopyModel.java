...
public abstract class AbstractTestGenericEntityMapCopyModel<K, V, M extends Map<K, V>>
  ...
  protected static final Equivalence<Map<?, ?>> ORDERED_MAP_EQUIVALENCE =
      new Equivalence<Map<?, ?>>() {
        @Override
        protected boolean doEquivalent(Map<?, ?> a, Map<?, ?> b) {
          return Lists.newArrayList(a.entrySet())
              .equals(
                  Lists.newArrayList(
                      b.entrySet())); // SortedMap.equals won't work on cloned transient instances
        }
  ...
  public void testNotAttached(
      SerializableSupplier2<? extends M> mapSupplier, Equivalence<? super M> equivalence) {
