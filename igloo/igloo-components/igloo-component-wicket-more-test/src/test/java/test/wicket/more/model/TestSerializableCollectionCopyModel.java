...
class TestSerializableCollectionCopyModel<C extends Collection<ValueEnum>>
	...
	protected ICollectionModel<ValueEnum, C> createModel(SerializableSupplier2<? extends C> collectionSupplier) {
		return CollectionCopyModel.custom(collectionSupplier, Models.<ValueEnum>serializableModelFactory());
	}

	protected C createCollection(SerializableSupplier2<? extends C> collectionSupplier, ValueEnum... items) {
	...
	@ParameterizedTest
	@MethodSource("data")
	void testNull(SerializableSupplier2<? extends C> collectionSupplier, Equivalence<? super C> equivalence) {
	...
	@ParameterizedTest
	@MethodSource("data")
	void testNonNull(SerializableSupplier2<? extends C> collectionSupplier, Equivalence<? super C> equivalence) {
	...
	@ParameterizedTest
	@MethodSource("data")
	void testCollectionCopy(SerializableSupplier2<? extends C> collectionSupplier, Equivalence<? super C> equivalence) throws Exception {
		C collection = createCollection(collectionSupplier, ValueEnum.VALUE1, ValueEnum.VALUE2);
	...
	@ParameterizedTest
	@MethodSource("data")
	void testGetObjectAdd(SerializableSupplier2<? extends C> collectionSupplier, Equivalence<? super C> equivalence) throws Exception {
		IModel<C> model = createModel(collectionSupplier);
		model.setObject(createCollection(collectionSupplier, ValueEnum.VALUE1, ValueEnum.VALUE2));
		C modelObject = model.getObject();
		
		modelObject.add(ValueEnum.VALUE3);
		assertThat(model.getObject(), isEquivalent(equivalence, createCollection(collectionSupplier, ValueEnum.VALUE1, ValueEnum.VALUE2, ValueEnum.VALUE3)));
		
		model = serializeAndDeserialize(model);
		modelObject = model.getObject();
		assertThat(modelObject).isNotNull();
		assertThat(modelObject).hasSize(3);
		assertThat(modelObject, isEquivalent(equivalence, createCollection(collectionSupplier, ValueEnum.VALUE1, ValueEnum.VALUE2, ValueEnum.VALUE3)));
	}

	@ParameterizedTest
	@MethodSource("data")
	void testGetObjectRemove(SerializableSupplier2<? extends C> collectionSupplier, Equivalence<? super C> equivalence) throws Exception {
		IModel<C> model = createModel(collectionSupplier);
		model.setObject(createCollection(collectionSupplier, ValueEnum.VALUE1, ValueEnum.VALUE2));
		C modelObject = model.getObject();
		
		modelObject.remove(ValueEnum.VALUE2);
		assertThat(model.getObject(), isEquivalent(equivalence, createCollection(collectionSupplier, ValueEnum.VALUE1)));
		
		model = serializeAndDeserialize(model);
		modelObject = model.getObject();
		assertThat(modelObject).isNotNull();
		assertThat(modelObject).hasSize(1);
		assertThat(modelObject, isEquivalent(equivalence, createCollection(collectionSupplier, ValueEnum.VALUE1)));
	}
