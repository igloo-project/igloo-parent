...
class PercentageTests extends AbstractWicketMoreTestCase {
	...
	void percentageTest() {
		...
		
		{
			...
			assertEquals("51%", s);
			assertEquals("51%", CoreRenderers.percent("#0%", RoundingMode.HALF_UP).render(new BigDecimal("0.51256"), Locale.FRENCH));
		}
		
		{
			...
			assertEquals("51,256%", CoreRenderers.percent("#0.######%", RoundingMode.HALF_UP).render(new BigDecimal("0.51256"), Locale.FRENCH));
		}
