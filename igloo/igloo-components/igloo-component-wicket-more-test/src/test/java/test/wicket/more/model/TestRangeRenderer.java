...
public class TestRangeRenderer extends AbstractWicketMoreTestCase {
  ...
  /**
   * JUnit non-regression tests on Range renderers.
   *
   * <p><b>Tests:</b>
   *
   * <ul>
   ...
   *   <li>Whether the four tags which may be found within a string designated by a resource key are
   *       correctly replaced
   * </ul>
   */
  @Test
  ...
  private void testWithUnits() {
    ...
    expected =
        Localizer.get()
            .getString(KEY_MONEY_SOLO, null)
            .replace(LOWER_UNIT_TAG, UPPER_UNIT_TAG)
            .replace(
                UPPER_UNIT_TAG,
                Renderer.count(KEY_COUNT_BASE).render(lowerBoundModel.getObject(), Locale.FRANCE));
    assertThat(
            Renderer.range(KEY_MONEY_BASE, Renderer.<Integer>count(KEY_COUNT_BASE))
                .asModel(moneyRangeModel)
                .getObject())
        .isEqualTo(expected);
    // Testing .both suffix
    ...
    expected =
        Localizer.get()
            .getString(KEY_MONEY_BOTH, null)
            .replace(
                LOWER_UNIT_TAG,
                Renderer.count(KEY_COUNT_BASE).render(lowerBoundModel.getObject(), Locale.FRANCE))
            .replace(
                UPPER_UNIT_TAG,
                Renderer.count(KEY_COUNT_BASE).render(upperBoundModel.getObject(), Locale.FRANCE));
    assertThat(
            Renderer.range(KEY_MONEY_BASE, Renderer.<Integer>count(KEY_COUNT_BASE))
                .asModel(moneyRangeModel)
                .getObject())
  ...
  private void testWithoutUnits() {
    ...
    expected =
        Localizer.get()
            .getString(KEY_DATES_SOLO, null)
            .replace(LOWER_TAG, UPPER_TAG)
            .replace(
                UPPER_TAG,
                Renderer.fromDatePattern(DatePattern.SHORT_DATE)
                    .render(lowerBoundModel.getObject(), Locale.FRANCE));
    assertThat(
            Renderer.range(KEY_DATES_BASE, Renderer.fromDatePattern(DatePattern.SHORT_DATE))
                .asModel(dateRangeModel)
                .getObject())
        .isEqualTo(expected);
    // Testing .both suffix
    ...
    expected =
        Localizer.get()
            ...
            .replace(
                UPPER_TAG,
                Renderer.fromDatePattern(DatePattern.SHORT_DATE)
                    .render(upperBoundModel.getObject(), Locale.FRANCE));
    assertThat(
            Renderer.range(KEY_DATES_BASE, Renderer.fromDatePattern(DatePattern.SHORT_DATE))
                .asModel(dateRangeModel)
                .getObject())
