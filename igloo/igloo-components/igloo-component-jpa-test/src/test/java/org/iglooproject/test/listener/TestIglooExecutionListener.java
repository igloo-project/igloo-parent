...
import org.iglooproject.test.jpa.spring.IglooAnyTestExecutionListener;
import org.iglooproject.test.jpa.spring.IglooClassTestExecutionListener;
import org.iglooproject.test.jpa.spring.IglooExecutionTestExecutionListener;
import org.iglooproject.test.jpa.spring.IglooMethodTestExecutionListener;
  ...
  private static class CheckCalledListenerIglooTestListener implements IIglooTestListener {
    ...
    public void before(IglooTestListenerType type, TestContext context) {
      Assertions.assertThatList(StackWalker.getInstance().walk(f -> f.limit(10).toList()))
          .anyMatch(i -> TestIglooExecutionListener.matchMethodName(i, "before" + methodName));
    }

    @Override
    public void after(IglooTestListenerType type, TestContext context) {
      Assertions.assertThatList(StackWalker.getInstance().walk(f -> f.limit(10).toList()))
          .anyMatch(i -> TestIglooExecutionListener.matchMethodName(i, "after" + methodName));
    }
  }
}
