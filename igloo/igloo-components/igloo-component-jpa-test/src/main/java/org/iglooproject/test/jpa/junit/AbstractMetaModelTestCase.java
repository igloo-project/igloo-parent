...
public abstract class AbstractMetaModelTestCase extends AbstractTestCase {
  ...
  @Autowired protected IJpaConfigurationProvider configurationProvider;

  @Autowired protected MetadataRegistryIntegrator metadataRegistryIntegrator;

  public AbstractMetaModelTestCase() {
    super();
  }

  protected JdbcRelation getRelation(
      Connection connection, String schemaPattern, String relationPattern, String... types) {
    return getRelations(connection, schemaPattern, "%", types).stream()
        .filter(
            i -> i.getTable_name() != null && i.getTable_name().equalsIgnoreCase(relationPattern))
        .findFirst()
        .orElseThrow(
            () ->
                new NoSuchElementException(
                    String.format("Relation %s cannot be found.", relationPattern)));
  }

  protected Collection<JdbcRelation> getRelations(
      Connection connection, String schemaPattern, String relationPattern, String... types) {
    ...
    try {
      ...
      throw new IllegalStateException(
          String.format(
              "error retrieving relation list for <%s-%s-%s>",
  ...
  protected TableInformation getTableInformation(String schema, String relation) {
    ...
    Identifier schemaIdentifier =
        metadataRegistryIntegrator.getMetadata().getDatabase().toIdentifier(schema);
    Identifier relationIdentifier =
        metadataRegistryIntegrator.getMetadata().getDatabase().toIdentifier(relation);
    return getDatabaseInformation()
        .getTableInformation(new Namespace.Name(catalog, schemaIdentifier), relationIdentifier);
  }
  ...
  protected SequenceInformation getSequenceInformation(String schema, String relation) {
    ...
    Identifier relationIdentifier =
        metadataRegistryIntegrator.getMetadata().getDatabase().toIdentifier(relation);
    return getDatabaseInformation()
        .getSequenceInformation(new Namespace.Name(catalog, schemaIdentifier), relationIdentifier);
  }

  protected DatabaseInformation getDatabaseInformation() {
    SessionFactoryImplementor sessionFactory =
        ((SessionImpl) entityManagerUtils.getEntityManager().getDelegate()).getSessionFactory();
    SqlStringGenerationContext sqlStringGenerationContext =
        sessionFactory.getSqlStringGenerationContext();
    ServiceRegistry serviceRegistry = sessionFactory.getServiceRegistry();
    HibernateSchemaManagementTool schemaManagementTool =
    ...
    DdlTransactionIsolator transactionIsolator =
        schemaManagementTool.getDdlTransactionIsolator(jdbcContext);
    return Helper.buildDatabaseInformation(
        serviceRegistry, transactionIsolator, sqlStringGenerationContext, schemaManagementTool);
  }
}
