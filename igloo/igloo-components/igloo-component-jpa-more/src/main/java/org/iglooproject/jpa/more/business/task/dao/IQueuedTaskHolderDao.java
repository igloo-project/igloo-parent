...
public interface IQueuedTaskHolderDao extends IGenericEntityDao<Long, QueuedTaskHolder> {
  ...
  /**
   * Renvoi aléatoirement l'une tâche considérée comme bloquée, c'est à dire dont le temps
   * d'exécution est jugé trop long. La limite du temps d'exécution est fixée par la propriété
   * 'sitra.tasks.executionTimeLimitInSeconds' dans la configuration.
   *
   * @param taskType type de tâche à récupérer
   * @param executionTimeLimitInSeconds le temps d'exécution au-delà duquel la tâche est considrée
   *     comme bloquée.
