...
public class QueuedTaskHolderManagerImpl
  ...
  @Autowired private IQueuedTaskHolderService queuedTaskHolderService;
  ...
  @Autowired private ITransactionSynchronizationTaskManagerService synchronizationManager;
  ...
  private final Multimap<TaskQueue, TaskConsumer> consumersByQueue =
      Multimaps.newListMultimap(
          new HashMap<TaskQueue, Collection<TaskConsumer>>(), Suppliers2.arrayList());
  ...
  private final void initQueues() {
    ...
    for (IQueueId queueId : this.queueIds) {
      ...
      Assert.state(
          !IQueueId.RESERVED_DEFAULT_QUEUE_ID.getUniqueStringId().equals(queueIdAsString),
          "Queue ID '"
              + IQueueId.RESERVED_DEFAULT_QUEUE_ID.getUniqueStringId()
              + "' is reserved for implementation purposes. Please choose another ID.");
      Assert.state(
          !queueIdsAsStrings.contains(queueIdAsString),
          "Queue ID '"
              + queueIdAsString
              + "' was defined more than once. Queue IDs must be unique.");
    ...
    for (IQueueId queueId : queueIds) {
      ...
      if (numberOfThreads < 1) {
        LOGGER.warn(
            "Number of threads for queue '{}' is set to an invalid value ({}); defaulting to 1",
  ...
  /**
   * The length of time the consumer threads will wait before their first access to their task
   * queue.
   */
          ...
          public void run() {
            for (TaskQueue queue : queuesById.values()) {
              try {
                ...
                List<QueuedTaskHolder> tasks =
                    queuedTaskHolderService.getListConsumable(queue.getId());
                ...
                if (!tasks.isEmpty()) {
                  ...
                  // Gets tasks  Id
                  List<Long> taskIds =
                      tasks.stream().map(QueuedTaskHolder::getId).collect(Collectors.toList());
  ...
  /**
   * Warning: this destroy method is called twice, so all the related code has to be written with
   * this constraint
   *
   * @throws Exception
   */
