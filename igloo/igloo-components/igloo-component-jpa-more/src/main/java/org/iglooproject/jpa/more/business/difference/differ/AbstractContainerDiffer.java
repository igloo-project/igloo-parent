...
public abstract class AbstractContainerDiffer implements Differ {
  ...
  private final Map<Predicate2<? super DiffNode>, AbstractContainerDifferStrategy<?, ?>>
      strategies = Maps.newLinkedHashMap();
  private final AbstractContainerDifferStrategy<?, ?> defaultStrategy;

  public AbstractContainerDiffer(
  ...
  protected void addStrategy(
      Predicate2<? super DiffNode> predicate, AbstractContainerDifferStrategy<?, ?> strategy) {
  ...
  public DiffNode compare(DiffNode parentNode, Instances instances) {
    ...
    for (Map.Entry<Predicate2<? super DiffNode>, AbstractContainerDifferStrategy<?, ?>> entry :
        strategies.entrySet()) {
  ...
  protected <TContainer, K> DiffNode compare(
      Instances instances,
      AbstractContainerDifferStrategy<TContainer, K> strategy,
    ...
    if (instances.hasBeenAdded()) {
      ...
      strategy.compareItems(
          differDispatcher, isReturnableResolver, collectionNode, instances, addedItems);
      if (collectionNode.hasChildren()) {
        collectionNode.setState(DiffNode.State.CHANGED);
      } else {
        // No child element, be it before or after the change: we'll say the collection did not
        // change.
      ...
      strategy.compareItems(
          differDispatcher, isReturnableResolver, collectionNode, instances, removedItems);
      if (collectionNode.hasChildren()) {
        collectionNode.setState(DiffNode.State.CHANGED);
      } else {
        // No child element, be it before or after the change: we'll say the collection did not
      ...
      if (comparisonStrategy == null) {
        ...
        strategy.compareKeys(
            differDispatcher, isReturnableResolver, collectionNode, instances, addedKeys);
        strategy.compareKeys(
            differDispatcher, isReturnableResolver, collectionNode, instances, removedKeys);
        strategy.compareKeys(
            differDispatcher, isReturnableResolver, collectionNode, instances, commonKeys);
      } else {
