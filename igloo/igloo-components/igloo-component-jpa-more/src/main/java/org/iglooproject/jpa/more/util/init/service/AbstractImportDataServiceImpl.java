...
public abstract class AbstractImportDataServiceImpl implements IImportDataService {
  ...
  @Autowired private IImportDataDao importDataDao;
  ...
  @Override
  public void importDirectory(String classpathFolder)
      throws ServiceException, SecurityServiceException, IOException, Exception {
    Map<String, Map<String, GenericEntity<Long, ?>>> idsMapping = new HashMap<>();

    importBeforeReferenceData(classpathFolder, idsMapping);

    try (DataSource referenceData =
        getFirstInputStream(classpathFolder, REFERENCE_DATA_FILE_NAMES)) {
  ...
  protected DataSource getFirstInputStream(String classpathFolder, List<String> fileNames) {
    ...
    String classpathFolderAbsolute =
        classpathFolder.endsWith("/") ? classpathFolder : classpathFolder + "/";
    for (String fileName : fileNames) {
      InputStream is =
          Thread.currentThread()
              .getContextClassLoader()
              .getResourceAsStream(classpathFolderAbsolute + fileName);
      if (is != null) {
  ...
  protected void importBeforeReferenceData(
      String classpathFolder, Map<String, Map<String, GenericEntity<Long, ?>>> idsMapping)
      throws ServiceException, SecurityServiceException, FileNotFoundException, IOException {
    // nothing, override if necessary
  }

  protected void importAfterReferenceData(
      String classpathFolder, Map<String, Map<String, GenericEntity<Long, ?>>> idsMapping)
      throws ServiceException, SecurityServiceException, FileNotFoundException, IOException {
    // nothing, override if necessary
  }

  protected void importBeforeBusinessData(
      String classpathFolder, Map<String, Map<String, GenericEntity<Long, ?>>> idsMapping)
      throws ServiceException, SecurityServiceException, FileNotFoundException, IOException {
    // nothing, override if necessary
  }

  protected void importAfterBusinessData(
      String classpathFolder, Map<String, Map<String, GenericEntity<Long, ?>>> idsMapping)
      throws ServiceException, SecurityServiceException, FileNotFoundException, IOException {
    // nothing, override if necessary
  }
  ...
  protected void importReferenceData(
    ...
    for (String packageToScan : getReferenceDataPackagesToScan()) {
      ...
      classes.addAll(
          ReflectionUtils.findAssignableClasses(packageToScan, GenericReferenceData.class));
  ...
  protected abstract void importMainBusinessItems(
      Map<String, Map<String, GenericEntity<Long, ?>>> idsMapping, Workbook workbook);
  ...
  protected <E extends GenericEntity<Long, ?>> void doImportItem(
    ...
    if (sheet != null) {
      GenericEntityConverter converter =
          new GenericEntityConverter(
              importDataDao, workbook, new HashMap<Class<?>, Class<?>>(0), idsMapping);
      ...
      LOGGER.info(
          "Imported " + idsMappingForClass.size() + " objects for class: " + clazz.getSimpleName());
    } else {
  ...
  protected <E extends GenericEntity<Long, ?>> void doFilterLine(
    ...
    if (line.containsKey(PASSWORD_FIELD_NAME)) {
      line.put(
          PASSWORD_HASH_FIELD_NAME,
          passwordEncoder.encode(line.get(PASSWORD_FIELD_NAME).toString()));
    }
  }
