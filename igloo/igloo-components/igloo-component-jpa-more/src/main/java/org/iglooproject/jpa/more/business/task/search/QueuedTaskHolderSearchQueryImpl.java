...
public class QueuedTaskHolderSearchQueryImpl implements IQueuedTaskHolderSearchQuery {
  ...
  @Override
  public List<QueuedTaskHolder> list(
      QueuedTaskHolderSearchQueryData data,
      Map<QueuedTaskHolderSort, SortOrder> sorts,
      Integer offset,
      Integer limit) {
    if (!checkLimit(limit)) {
      return List.of();
  ...
  private BiConsumer<
    ...
    return (f, root) -> {
      ...
      if (data.getName() != null) {
        root.add(
            f.simpleQueryString()
                .field(QueuedTaskHolder.NAME)
                .matching(wildcardTokensOr(data.getName())));
      }
