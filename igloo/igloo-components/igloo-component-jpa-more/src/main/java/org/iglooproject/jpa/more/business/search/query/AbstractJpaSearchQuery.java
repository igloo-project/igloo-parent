...
public abstract class AbstractJpaSearchQuery<T, S extends ISort<OrderSpecifier<?>>>
   ...
   *
   * <ul>
   *   <li>must(matchIfGiven(qCompany.manager.organization, organization))
   *   <li>must(matchIfGiven(qCompany.status, CompanyStatus.ACTIVE))
   * </ul>
   */
  ...
  private JPAQuery<T> getFinalQuery() {
    if (finalJpaQuery == null) {
      ...
      for (OrderSpecifier<?> orderSpecifier :
          SortUtils.getOrderSpecifierWithDefaults(sortMap, defaultSorts)) {
  ...
  // Query factory
  // 	>	Match if given
  protected <P extends Comparable<?>> BooleanExpression matchIfGiven(
      SimpleExpression<P> simpleExpression, P value) {
    if (value != null) {
  ...
  // 	>	Contains if given
  protected <C extends Collection<E>, E, Q extends SimpleExpression<? super E>>
      BooleanExpression containsIfGiven(CollectionPathBase<C, E, Q> collectionPath, E value) {
    if (value != null) {
  ...
  // 	>	Match one if given
  protected <P extends Comparable<?>> BooleanExpression matchOneIfGiven(
      SimpleExpression<P> simpleExpression, Collection<? extends P> possibleValues) {
  ...
  // 	>	Match all if given
  protected <P extends Comparable<?>> BooleanExpression matchAllIfGiven(
      SimpleExpression<P> simpleExpression, Collection<? extends P> possibleValues) {
    if (possibleValues != null && !possibleValues.isEmpty()) {
  ...
  protected <P extends Comparable<?>> BooleanExpression matchRangeIfGiven(
      ComparableExpression<P> comparableExpression, P minValue, P maxValue) {
  ...
  protected <P extends Comparable<?>> BooleanExpression matchExclusiveRangeIfGiven(
      ComparableExpression<P> comparableExpression, P minValue, P maxValue) {
    if (minValue != null && maxValue != null) {
  ...
  protected void setJpaQuery(JPAQuery<T> jpaQuery) {
