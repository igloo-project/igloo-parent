 ...
 *
 * <p>The queue IDs defined in an application must be defined in such a way that, given two non-null
 * <code>IQueueId</code> <code>id1</code> and <code>id2</code>, <code>
 * id1.getUniqueStringId().equals(id2.getUniqueStringId())</code> is true if and only if <code>id1
 * </code> and <code>id2</code> refer to the same queue.
 *
...
public interface IQueueId {
   ...
   *
   * <p><strong>NOTE:</strong> implementors must never return a string equal to {@link
   * #RESERVED_DEFAULT_QUEUE_ID_STRING}.
   *
