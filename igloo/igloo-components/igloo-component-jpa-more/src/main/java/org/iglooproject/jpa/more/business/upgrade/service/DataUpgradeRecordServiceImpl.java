...
public class DataUpgradeRecordServiceImpl extends GenericEntityServiceImpl<Long, DataUpgradeRecord> implements IDataUpgradeRecordService {
	...
	@Override
	public void markAsDone(IDataUpgrade dataUpgrade) throws ServiceException, SecurityServiceException {
		markAs(dataUpgrade, true);
	}

	@Override
	public void markAsToDo(IDataUpgrade dataUpgrade) throws ServiceException, SecurityServiceException {
	...
	@Override
	public void disableAutoPerform(IDataUpgrade dataUpgrade) throws ServiceException, SecurityServiceException {
		DataUpgradeRecord record = getByDataUpgrade(dataUpgrade);
		if (record != null) {
			record.setAutoPerform(false);
			update(record);
		} else {
			LOGGER.warn("Auto perform cannot be disabled because no record has been found for '" + dataUpgrade.getName() + "'");
		}
	}
}
