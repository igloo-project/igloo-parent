...
public enum HibernateCacheRegionFactory {
  ...
  static {
    MAPPING =
        Map.copyOf(
            Arrays.stream(HibernateCacheRegionFactory.values())
                .collect(
                    Collectors.toMap(HibernateCacheRegionFactory::getAlias, Function.identity())));
  }
  ;

  public static HibernateCacheRegionFactory fromString(String value) {
    // null/empty values are mapped to none
    String notNullNotEmptyValue =
        Optional.ofNullable(value).filter(i -> !i.isBlank()).orElse(NONE.getAlias());
    return Optional.ofNullable(MAPPING.get(notNullNotEmptyValue))
        .orElseThrow(
            () ->
                new IllegalStateException(
                    String.format(
                        "Value %s is not a valid HibernateCacheRegionFactory alias", value)));
  }
  ...
  private HibernateCacheRegionFactory(
      String alias, String regionFactoryAlias, String jcacheProvider) {
    this.alias = alias;
