...
import org.iglooproject.wicket.api.Detach;
...
public abstract class Condition implements IModel<Boolean>, IDetachable {
	...
	public static Condition isEmpty(IDataProvider<?> dataProvider) {
		return predicate(BindingModel.of(dataProvider, DataProviderBindings.iBindableDataProvider().size()), Predicates2.equalTo(0L));
	}
	
	public static Condition isNotEmpty(IDataProvider<?> dataProvider) {
		return predicate(BindingModel.of(dataProvider, DataProviderBindings.iBindableDataProvider().size()), Predicates2.equalTo(0L)).negate();
	}
