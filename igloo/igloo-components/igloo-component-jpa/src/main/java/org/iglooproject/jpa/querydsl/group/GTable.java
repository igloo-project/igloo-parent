...
abstract class GTable<R, C, V, T extends Table<R, C, V>>
  ...
  public static <R extends Comparable, C extends Comparable, V>
      GTable<R, C, V, RowSortedTable<R, C, V>> createSorted(QTriplet<R, C, V> expr) {
  ...
  static class Mixin<R, C, V, R2, C2, V2, T extends Table<? super R2, ? super C2, ? super V2>>
    ...
    private class GroupCollectorImpl implements GroupCollector<Triplet<R, C, V>, T> {
      ...
      private final Map<R, GroupCollector<R, R2>> rowCollectors = new LinkedHashMap<>();

      private final Map<C, GroupCollector<C, C2>> columnCollectors = new LinkedHashMap<>();

      private final Table<GroupCollector<R, R2>, GroupCollector<C, C2>, GroupCollector<V, V2>>
          valueCollectors = HashBasedTable.create();
      ...
      @Override
      public T get() {
        for (Map.Entry<GroupCollector<R, R2>, Map<GroupCollector<C, C2>, GroupCollector<V, V2>>>
            rowEntry : valueCollectors.rowMap().entrySet()) {
          ...
          for (Map.Entry<GroupCollector<C, C2>, GroupCollector<V, V2>> columnEntry :
              columnMap.entrySet()) {
    ...
    public Mixin(
        GroupExpression<R, R2> rowExpression,
        GroupExpression<C, C2> columnExpression,
        GroupExpression<V, V2> valueExpression,
