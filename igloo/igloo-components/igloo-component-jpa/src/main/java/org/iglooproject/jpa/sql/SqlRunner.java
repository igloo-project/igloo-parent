...
public class SqlRunner {
	...
	public SqlRunner(EntityManagerFactory entityManagerFactory, MetadataRegistryIntegrator metadataRegistryIntegrator) {
	...
	public void migrationScript(@Nonnull SqlRunner.Action action, @Nullable Path output) {
		var serviceRegistry = entityManagerFactory.unwrap(SessionFactoryImplementor.class).getServiceRegistry();
	...
	private void generateSql(SqlRunner.Action action, SchemaManagementTool schemaTool, Writer writer) {
		...
		if (Action.create.equals(action)) {
			...
			schemaTool.getSchemaCreator(new HashMap<>()).doCreation(metadataRegistryIntegrator.getMetadata(),
					executionOptions, ContributableMatcher.ALL, sourceDescriptor, targetDescriptor);
		} else if (Action.update.equals(action)) {
			schemaTool.getSchemaMigrator(new HashMap<>()).doMigration(metadataRegistryIntegrator.getMetadata(),
					executionOptions, ContributableMatcher.ALL, targetDescriptor);
		}
	...
	/**
	 * Returns an output {@link PrintWriter} targetting provided not-null <code>output</code> file, else stdout.
	 * 
	...
	private PrintWriter writer(@Nullable Path output) throws FileNotFoundException {
		if (output == null) {
			return new PrintWriter(new FilterWriter(CharStreams.asWriter(System.out)) { //NOSONAR, we want to output to stdout
				@Override
