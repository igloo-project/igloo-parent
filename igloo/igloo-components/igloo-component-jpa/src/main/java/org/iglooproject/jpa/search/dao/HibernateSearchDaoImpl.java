...
public class HibernateSearchDaoImpl implements IHibernateSearchDao {
  ...
  @Autowired private IJpaPropertiesProvider jpaPropertiesProvider;
  ...
  public Analyzer getAnalyzer(String analyzerName) {
    if (jpaPropertiesProvider.isHibernateSearchElasticSearchEnabled()) {
      ...
      SearchIntegration integration =
          searchIntegrator.getIntegration(LuceneEmbeddedIndexManagerType.INSTANCE);
      return integration
          .getAnalyzerRegistry()
          .getAnalyzerReference(analyzerName)
          .unwrap(LuceneAnalyzerReference.class)
          .getAnalyzer();
    }
  }
  ...
  public Analyzer getAnalyzer(Class<?> entityType) {
    SearchFactory searchFactory =
        Search.getFullTextEntityManager(getEntityManager()).getSearchFactory();
    ExtendedSearchIntegrator searchIntegrator =
    ...
    if (jpaPropertiesProvider.isHibernateSearchElasticSearchEnabled()) {
      ...
      return Search.getFullTextEntityManager(getEntityManager())
          ...
          .getAnalyzer(indexedType);
    }
  }

  /** Extracted from ConnectedQueryContextBuilder */
  private IndexedTypeIdentifier getIndexBoundType(
      Class<?> entityType, ExtendedSearchIntegrator factory) {
  ...
  protected void reindexClasses(
      FullTextEntityManager fullTextEntityManager, Set<Class<?>> entityClasses)
  ...
  public Set<Class<?>> getIndexedRootEntities(Class<?>... selection) {
    ...
    Set<Class<?>> indexedEntityClasses =
        new TreeSet<>(
            new Comparator<Class<?>>() {
              @Override
              public int compare(Class<?> o1, Class<?> o2) {
                // Legacy. French should not be considered as default locale.
                return GenericEntity.STRING_COLLATOR_FRENCH.compare(
                    o1.getSimpleName(), o2.getSimpleName());
              }
            });
    indexedEntityClasses.addAll(
        getIndexedRootEntities(fullTextEntityManager.getSearchFactory(), selection));
   ...
   */
  protected Set<Class<?>> getIndexedRootEntities(
      SearchFactory searchFactory, Class<?>... selection) {
    ExtendedSearchIntegrator searchIntegrator =
        searchFactory.unwrap(ExtendedSearchIntegrator.class);
    ...
    for (Class<?> entityType : selection) {
      IndexedTypeSet targetedClasses =
          searchIntegrator.getIndexedTypesPolymorphic(IndexedTypeSets.fromClass(entityType));
