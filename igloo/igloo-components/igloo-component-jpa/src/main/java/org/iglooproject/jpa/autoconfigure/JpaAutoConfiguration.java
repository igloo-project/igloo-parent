 ...
 *
 * <ul>
 *   <li>(optional <code>spring.jpa.igloo.component-path.enabled=true</code>, default false)
 *       Customize database naming strategy.
 *   <li>(optional <code>spring.jpa.igloo.old-style-transaction-advisor</code>, default false)
 *       Customize transaction interceptors with the old-style {@link
 *       ITransactionalAspectAwareService} interface and method-name based behaviors.
 *   <li>Add {@link IEntityService} / {@link IEntityDao} / {@link IEntityReferenceQuery} beans.
 *   <li>Add {@link ITransactionScopeIndependantRunnerService} bean that allows to launch {@link
 *       EntityManager}/Transaction isolated jobs.
 *   <li>Add {@link EntityManagerUtils} bean; this bean provides utility methods to lookup
 *       thead-local bound {@link EntityManager}.
 *   <li>Add {@link MetadataRegistryIntegrator} bean
 * </ul>
 *
 * <h2>Database naming strategy</h2>
 *
 * <p>See <code>igloo-component-jpa/component-path.properties</code> resource. Embedded names are
 * included inside property paths (useful to insert a same {@link Embeddable} multiple times) and
 * entity/property names are left untouched (see {@link PhysicalNamingStrategyStandardImpl}).
 *
 * <h2>Old-style transaction interceptor</h2>
 *
 * <p>Based on {@link ITransactionalAspectAwareService} interface. <code>
 * get/is/has/list/search/count/find</code> methods are read-only and other methods are read-write.
 *
 * <h2>MetadataRegistryIntegrator</h2>
 *
 * <p>This bean allows to store {@link Metadata} Hibernate object during {@link
 * EntityManagerFactory} initialization. This object is really helpful to perform some advanced
 * Hibernate customization. This bean does not modify default Hibernate behaviors.
 *
 * <p>Order for transaction management is modified from LOWEST_PRECEDENCE (last to be called) to
 * LOWEST_PRECEDENCE - 1.000.000, to allow to push other interceptors after transaction management.
 * Authorization related orders can be found in AuthorizationInterceptorsOrder and are from 200 to
 * 600.
 */
@Configuration
...
public class JpaAutoConfiguration {
  ...
  @ConditionalOnProperty(
      name = "spring.jpa.igloo.component-path.enabled",
      havingValue = "true",
      matchIfMissing = false)
  @Configuration
  @PropertySource(
      name = IglooPropertySourcePriority.FRAMEWORK,
      value = {"classpath:igloo-component-jpa/component-path.properties"},
      encoding = "UTF-8")
  public static class JpaComponentPathConfiguration {}

  @ConditionalOnProperty(
      name = "spring.jpa.igloo.sequence-naming.enabled",
      havingValue = "true",
      matchIfMissing = false)
  @Configuration
  @PropertySource(
      name = IglooPropertySourcePriority.FRAMEWORK,
      value = {"classpath:igloo-component-jpa/sequence-naming.properties"},
      encoding = "UTF-8")
  public static class SequenceConfiguration {}

  @Bean
  @ConditionalOnProperty(
      name = "spring.jpa.igloo.old-style-transaction-advisor",
      havingValue = "true",
      matchIfMissing = false)
  ...
  @Bean
  public HibernatePropertiesCustomizer hibernateIntegratorProperty(
      MetadataRegistryIntegrator integrator) {
    return p ->
        p.put(
