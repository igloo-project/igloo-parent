...
public final class JpaConfigUtils {
  ...
  /**
   * Construit un {@link LocalContainerEntityManagerFactoryBean} à partir d'un ensemble d'options
   * usuelles.
   */
  public static LocalContainerEntityManagerFactoryBean entityManagerFactory(
      IJpaConfigurationProvider provider) {
    ...
    entityManagerFactoryBean.setPackagesToScan(
        getPackagesToScan(provider.getJpaPackageScanProviders()));
  ...
  public static Properties getJpaProperties(IJpaPropertiesProvider configuration) {
    ...
    properties.setProperty(
        ...
        Boolean.toString(configuration.isCreateEmptyCompositesEnabled()));
    properties.setProperty(
        AvailableSettings.XML_MAPPING_ENABLED,
        Boolean.toString(configuration.isXmlMappingEnabled()));
    ...
    if (defaultBatchSize != null) {
      properties.setProperty(
          AvailableSettings.DEFAULT_BATCH_FETCH_SIZE, Integer.toString(defaultBatchSize));
    ...
    if (!cacheRegionFactory.isDisabled()) {
      properties.setProperty(
          AvailableSettings.CACHE_REGION_FACTORY, cacheRegionFactory.getRegionFactoryAlias());
      if (cacheRegionFactory.isJcache()) {
        properties.setProperty("hibernate.javax.cache.missing_cache_strategy", "create-warn");
        properties.setProperty(
            "hibernate.javax.cache.provider", cacheRegionFactory.getJcacheProvider());
        properties.setProperty("hibernate.javax.cache.uri", configuration.getJcacheConfiguration());
      }

      properties.setProperty(
          AvailableSettings.JPA_SHARED_CACHE_MODE, SharedCacheMode.ENABLE_SELECTIVE.name());
      ...
      if (queryCacheEnabled) {
        throw new IllegalArgumentException(
            "Could not enable query cache without EhCache configuration");
      }
    ...
    Class<? extends Analyzer> hibernateSearchDefaultAnalyzer =
        configuration.getHibernateSearchDefaultAnalyzer();
    if (hibernateSearchDefaultAnalyzer != null) {
      properties.setProperty(
          org.hibernate.search.cfg.Environment.ANALYZER_CLASS,
          hibernateSearchDefaultAnalyzer.getName());
    }
    ...
    if (StringUtils.hasText(hibernateSearchIndexingStrategy)) {
      properties.setProperty(
          org.hibernate.search.cfg.Environment.INDEXING_STRATEGY, hibernateSearchIndexingStrategy);
    }
    ...
    String identifierGeneratorStrategyProviderName =
        identifierGeneratorStategyProvider != null
            ? identifierGeneratorStategyProvider.getName()
    ...
    Class<? extends ImplicitNamingStrategy> implicitNamingStrategy =
        configuration.getImplicitNamingStrategy();
    if (implicitNamingStrategy != null) {
      properties.setProperty(
          AvailableSettings.IMPLICIT_NAMING_STRATEGY, implicitNamingStrategy.getName());
    } else {
      throw new IllegalStateException(
          AvailableSettings.IMPLICIT_NAMING_STRATEGY
              + " may not be null: sensible values are "
    ...
    Class<? extends PhysicalNamingStrategy> physicalNamingStrategy =
        configuration.getPhysicalNamingStrategy();
    if (physicalNamingStrategy != null) {
      properties.setProperty(
          AvailableSettings.PHYSICAL_NAMING_STRATEGY, physicalNamingStrategy.getName());
    } else {
      throw new IllegalStateException(
          AvailableSettings.PHYSICAL_NAMING_STRATEGY
              + " may not be null: sensible values are "
              + PhysicalNamingStrategyStandardImpl.class.getName()
              + " for no filtering of the name "
              + PostgreSQLPhysicalNamingStrategyImpl.class.getName()
              + " to truncate the name to conform with PostgreSQL identifier max length");
    }
    ...
    if (isNewGeneratorMappingsEnabled != null) {
      properties.setProperty(
          ...
          isNewGeneratorMappingsEnabled.toString());
    }

    properties.put(
        EntityManagerFactoryBuilderImpl.INTEGRATOR_PROVIDER, configuration.getIntegratorProvider());
  ...
  public static Advisor defaultTransactionAdvisor(PlatformTransactionManager transactionManager) {
    return defaultTransactionAdvisor(
        transactionManager, Lists.<Class<? extends Exception>>newArrayList());
  }

  public static Advisor defaultTransactionAdvisor(
    ...
    advisor.setAdvice(
        defaultTransactionInterceptor(transactionManager, additionalRollbackRuleExceptions));

    return advisor;
  }

  /** Construit un transactionInterceptor avec une configuration par défaut. */
  public static TransactionInterceptor defaultTransactionInterceptor(
      PlatformTransactionManager transactionManager,
      List<Class<? extends Exception>> additionalRollbackRuleExceptions) {
    ...
    // TODO voir si on ajoute SecurityServiceException.class en fonction de ce que ça donne sur le
    // Wombat
  ...
  public static DataSource dataSource(
      IDatabaseConnectionConfigurationProvider configurationProvider) {
    if (configurationProvider instanceof IDatabaseConnectionJndiConfigurationProvider) {
      String jndiName =
          ((IDatabaseConnectionJndiConfigurationProvider) configurationProvider).getJndiName();
      ...
      try {
        ...
        throw new IllegalStateException(
            String.format("Error during jndi lookup for %s resource", jndiName), e);
      }
      ...
      throw new IllegalStateException(
          String.format(
              "JDBC pool : %s type not handled", configurationProvider.getClass().getName()));
    }
  }
