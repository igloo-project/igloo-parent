...
/**
 * Check that JSR-107 allows to load a separate configuration file by using {@link URI} argument
 * when {@link CacheManager} is created. This is needed so that <code>hibernate.javax.cache.uri
 * </code> property can be used to load a separate root configuration. This is wanted to allow a
 * sensible and specific default configuration for hibernate second-level caching.
 *
 * <p>Also test that basic statistics and configuration methods work as expected.
 */
class TestJcacheCaffeine {
  ...
  void testCustomConfig() {
    CacheManager cacheManager =
        createJsr107CacheManager(URI.create("classpath:caffeine-custom.conf"));
    var cache = createJsr107Cache(cacheManager, "fallback");
    cache.put("key", "value");
    assertThat(cache.get("key")).isEqualTo("value");
  ...
  void testDottedName() {
    CacheManager cacheManager =
        createJsr107CacheManager(URI.create("classpath:caffeine-dotted-name.conf"));
  ...
  private javax.cache.Cache<Object, Object> createJsr107Cache(
  ...
  private Cache<Object, Object> getNativeCache(CacheManager cacheManager, String cacheName) {
    return cacheManager.unwrap(CacheManagerImpl.class).getCache(cacheName).unwrap(Cache.class);
  }
}
