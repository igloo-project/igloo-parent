...
class TestHibernateLambdaSwitch {
  ...
  /**
   * This method allows to check performance overhead introduced by lambda-implementation of {@link
   * GenericEntity#equals(Object)}.
   */
  @Test
  void testEqualsSwitch()
    ...
    newImplementationHibernate =
        doWithClassLoader(
            classLoaderSupplier(true),
            () -> {
              return doEqualsSwitch(
                  nbObjects,
                  randomSeed,
                  nLoop,
                  TestEntity.class.getName(),
                  GenericEntityImplementation.HIBERNATE);
            });

    // second loop : old implementation; equals method overriden in TestEntityHibernate
    oldImplementationHibernate =
        doWithClassLoader(
            classLoaderSupplier(true),
            () -> {
              return doEqualsSwitch(
                  ...
                  randomSeed,
                  nLoop,
                  TestEntityHibernate.class.getName(),
                  GenericEntityImplementation.HIBERNATE);
            });

    // third loop : new implementation without hibernate
    newImplementationSimple =
        doWithClassLoader(
            classLoaderSupplier(false),
            () -> {
              return doEqualsSwitch(
                  nbObjects,
                  randomSeed,
                  nLoop,
                  TestEntity.class.getName(),
                  GenericEntityImplementation.SIMPLE);
            });

    // forth loop : old implementation without hibernate
    oldImplementationSimple =
        doWithClassLoader(
            classLoaderSupplier(false),
            () -> {
              return doEqualsSwitch(
                  ...
                  randomSeed,
                  nLoop,
                  TestEntitySimple.class.getName(),
                  GenericEntityImplementation.SIMPLE);
            });

    // skip first loops for calculations
    Function<List<Long>, Long> avg =
        (newTimings) ->
            newTimings.stream().sorted().skip(skippedLongestLoops).reduce(Long::sum).get()
                / newTimings.size();
    ...
    try {
      ...
      assertThat(newImplementationHibernateTiming)
          .isCloseTo(oldImplementationHibernateTiming, withinPercentage);
    }
    try {
      assertThat(newImplementationSimpleTiming).isLessThan(oldImplementationSimpleTiming);
    } catch (AssertionError e) {
      assertThat(newImplementationSimpleTiming)
          .isCloseTo(oldImplementationSimpleTiming, withinPercentage);
    }
  }

  public List<Long> doEqualsSwitch(
      int nbObjects,
      long randomSeed,
      int nLoop,
      String className,
      GenericEntityImplementation implementation) {
    try {
      // we cannot reference classes by names are we want new classes instances to be loaded by
      // overriden
      // class-loader
  ...
  public Supplier<ClassLoader> classLoaderSupplier(boolean includeHibernate)
      throws ClassNotFoundException, IOException {
    URLClassLoader myClassLoader = (URLClassLoader) Thread.currentThread().getContextClassLoader();
    URL[] urls;
    if (!includeHibernate) {
      urls =
          Arrays.stream(myClassLoader.getURLs())
  ...
  public List<Long> doWithClassLoader(
      Supplier<ClassLoader> classLoaderSupplier, Supplier<List<Long>> r) {
      ...
      if (classLoader instanceof Closeable) {
        try {
          ...
          throw new IllegalStateException(
              String.format("Error closing classLoader %s", classLoader), e);
        }
      }
