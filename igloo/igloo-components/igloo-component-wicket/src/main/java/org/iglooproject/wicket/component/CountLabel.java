...
/**
 * Displays a label with up to three different variations based on whether a
 * numeric model is zero, one, or greater than one. This is incredibly useful
 * for printing count messages where the phrasing needs to change according to
 * singular and plural rules, like this:
 * <ul>
 ...
 * </ul>
 * <p>
 * This class is a simple Label wrapper around a CountMessageModel, which does
 * most of the work. Refer to the class documentation of
...
public class CountLabel extends Label {
	...
	/**
	 * Constructs a label that will a display zero, singular or plural variation
	 * based on the value of the specified {@code count} model.
	 * 
	 * @param id
	 *            The wicket:id of the label component. This is will also be
	 *            used as the message key to look up the message strings in the
	 *            localized properties file.
	 * 
	 * @param count
	 *            A number that will be used to choose either the
	 *            <code>id.zero</code>, <code>id.one</code> or
	 *            <code>id.many</code> string from the properties file. It will
	 *            also be made available to the string as an interpolated
