...
/**
 * An improved and simplified version of the BindingModel implementation of wicket-bindgen.
 *
 * <p>It is based on PropertyModel instead of using bindgen features because bindgen behavior is not
 * the same as Wicket one. Typically, when an object is null in the path, Wicket tries to
 * instantiate using the default constructor which allows to have cost().quantity() working even if
 * cost property is null.
 *
 * @author igor.vaynberg
 * @author Open Wide
 * @param <R> type of object that is the root of the expression
 * @param <T> type of object returned by the expression
 */
public class BindingModel<R, T> extends AbstractPropertyModel<T> {
  ...
  public static final <T, U> IDetachableFactory<IModel<T>, BindingModel<T, U>> factory(
      BindingRoot<? super T, ? extends U> binding) {
  ...
  /**
   * Convenience method to convert a {@link BindingRoot} into a {@link BindingModel}
   *
   * @param <T>
   * @param binding
