...
/**
 * An abstract base for implementations of {@link IMapModel} whose content is to be "cloned" (i.e.
 * copied to a new map) each time {@link #setObject(Map)} is called.
 *
 * <p>This is typically what you want when editing a map in a form.
 *
 * <p>Content is stored as a map of element models. Thus, subclasses of this one are guaranteed to
 * always return the same model for a given element (key or value), up to this element's removal
 * from the map.
 *
 ...
 */
abstract class AbstractMapCopyModel<
        K, V, M extends Map<K, V>, MK extends IModel<K>, MV extends IModel<V>>
    extends LoadableDetachableModel<M> implements IItemModelAwareMapModel<K, V, M, MK, MV> {
  ...
  private void updateModels() {
    ...
    for (Map.Entry<MK, MV> entry : modelMap.entrySet()) {
      ...
      Object previousModelsForThisKey =
          oldModels.put(key, Pair.with(entry.getKey(), entry.getValue()));
   ...
   * getObject()</code> will not return <code>null</code>, but <em>an empty map</em>.
   */
  @Override
  ...
  public final MV valueModelForProvidedKeyModel(final IModel<K> keyModel) {
    // We don't need to update the modelMap here, as the keyModel is supposed to have been provided
    // by this object
