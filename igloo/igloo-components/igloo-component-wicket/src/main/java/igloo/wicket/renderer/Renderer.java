...
import igloo.wicket.component.AbstractCoreLabel;
...
public abstract class Renderer<T> implements IConverter<T>, IRenderer<T> {
  ...
  /**
   * Utility method that can be used when a resource key value is needed in the render()
   * implementation.
   */
  ...
  /**
   * @deprecated Implemented to satisfy the {@link IConverter} interface; use {@link #render(Object,
   *     Locale)} instead.
   */
  @Override
  @Deprecated
  ...
  private static final Renderer<Object> NULL_RENDERER =
      new ConstantRenderer<Object>(null) {
  ...
  private static final Renderer<Object> BLANK_RENDERER =
      new ConstantRenderer<Object>("") {
        private static final long serialVersionUID = 5711969182760960576L;
  ...
  public Renderer<T> withDefault(
      SerializablePredicate2<? super T> validValuePredicate, T defaultValue) {
    return onResultOf(
        Functions2.defaultValue(validValuePredicate, Functions2.constant(defaultValue)));
  }

  public Renderer<T> withDefault(
      SerializablePredicate2<? super T> validValuePredicate, IRenderer<? super T> defaultRenderer) {
  ...
  private static class DefaultingRenderer<T> extends Renderer<T> {
    ...
    public DefaultingRenderer(
        SerializablePredicate2<? super T> validValuePredicate,
        IRenderer<? super T> validValueDelegate,
  ...
  @SafeVarargs
  public final Renderer<T> append(
      SerializableFunction2<? super Locale, ? extends Joiner> joinerFunction,
      Renderer<? super T>... appendedRenderers) {
  ...
  private static class JoiningRenderer<T> extends Renderer<T> {
    ...
    public JoiningRenderer(
        SerializableFunction2<? super Locale, ? extends Joiner> joinerFunction,
        Iterable<? extends Renderer<? super T>> renderers) {
      super();
    ...
    public String render(final T value, final Locale locale) {
      ...
      return joinerFunction
          .apply(locale)
          .join(Streams.stream(renderers).map(input -> input.render(value, locale)).iterator());
    }
  }
  ...
  private static class ByFunctionRenderer<F, T> extends Renderer<F> {
    ...
    public ByFunctionRenderer(
        SerializableFunction2<? super F, ? extends T> function, Renderer<T> delegate) {
      super();
  ...
  private static class ComposeRenderer<T> extends Renderer<T> {
    ...
    public ComposeRenderer(
        SerializableFunction2<? super String, ? extends String> function, Renderer<T> delegate) {
      super();
      this.function = function;
      this.delegate = delegate;
    }
  ...
  private static class ResourceKeyWithRenderedParameterRenderer<T> extends Renderer<T> {
    ...
    public ResourceKeyWithRenderedParameterRenderer(
        String resourceKey, Renderer<T> parameterRenderer) {
      super();
  ...
  private static final Renderer<Object> STRING_VALUE_RENDERER =
      new Renderer<Object>() {
        private static final long serialVersionUID = 5711969182760960576L;
  ...
  public static <T extends Number> Renderer<T> fromNumberFormat(
      SerializableFunction2<? super Locale, ? extends NumberFormat> formatFunction) {
  ...
  public static <T extends Date> Renderer<T> fromDateFormat(
      SerializableFunction2<? super Locale, ? extends DateFormat> formatFunction) {
    return fromFormat(formatFunction);
  }
  ...
  protected static <T> Renderer<T> fromFormat(
      SerializableFunction2<? super Locale, ? extends Format> formatFunction) {
  ...
  public static <T> Renderer<Iterable<? extends T>> fromJoiner(
      SerializableFunction2<? super Locale, ? extends Joiner> joinerFunction,
  ...
  private static class IterableJoinerRenderer<T> extends Renderer<Iterable<? extends T>> {
    ...
    public String render(Iterable<? extends T> value, Locale locale) {
      ...
      return joinerFunction
          .apply(locale)
          .join(Streams.stream(value).map(itemRenderer.asFunction(locale)).iterator());
    }
  }

  public static Renderer<Map<?, ?>> fromMapJoiner(
      SerializableFunction2<? super Locale, ? extends MapJoiner> joinerFunction) {
  ...
  public static <T extends Number> Renderer<T> count(
      String resourceKeyPrefix, Renderer<? super T> numberRenderer) {
  ...
  /**
   * A simplified version of the {@link #range(String, Renderer, Renderer)} function.
   *
   * <p>Returns a static renderer for {@link Range} elements in which every value is rendered the
   * same way - either with or without units.
   *
   * <p>See {@link #range(String, Renderer, Renderer)} for more information regarding the syntax,
   * the context or the prerequisites of {@link Range}-rendering functions.
   *
   * @param rangeResourceKeyPrefix the prefix of the translation key pointing to the range-based
   *     display text patterns
   * @param renderer a renderer used to display the value of a given {@link Range} bound
   */
  public static <C extends Comparable<?>> Renderer<Range<C>> range(
      String rangeResourceKeyPrefix, Renderer<C> renderer) {
    return range(rangeResourceKeyPrefix, renderer, renderer);
  }

  /**
   * Returns a static renderer for {@link Range} type elements in which values may have units.
   *
   * <p>This function uses two renderers, one for the rendering of a plain value and another for the
   * rendering of a value with its unit.
   *
   * <p>Knowing whether to render the first or last element with or without its unit is left to the
   * <i>resource</i>, as it may differ from one interval to another and may need to be determined
   * dynamically.
   *
   * <p><b>Note:</b> There are four different string variables recognised within a resource:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p><b>Note:</b> There are several resource key suffixes dynamically read:
   *
   * <ul>
   ...
   *   <li><b>&#60key&#62.solo:</b> read if the range possesses a lower and an upper bound which
   *       have the same value
   * </ul>
   *
   * <p><b>Example:</b> The resources:
   *
   * <ul>
   *   <li><b>base.key.upto=</b><i>Up to ${endUnit}</i> may be rendered as <i>Up to 1 mile</i> or
   *       <i>Up to 3 miles</i>.
   *   <li><b>base.key.both=</b><i>From ${start} to ${endUnit}</i> may be rendered as <i>From 10 to
   *       15 people</i>.
   *   <li><b>base.key.from=</b><i>Starting at ${startUnit}</i> may be rendered as <i>Starting at
   *       $4</i>.
   *   <li><b>base.key.solo=</b><i>Exactly ${startUnit}</i> may be rendered as <i>Exactly 5
   *       camels.</i>.
   *   <li><b>base.key.solo=</b><i>Closing at ${end}</i> may be rendered as <i>Closing at 5:00pm</i>
   *       using a specific Date Renderer.
   * </ul>
   *
   * @param rangeResourceKey the prefix of the translation key pointing to the range-based display
   *     text patterns
   * @param valueRenderer <br>
   *     a renderer used to display the plain value of a given {@link Range} bound
   * @param valueAndUnitRenderer a renderer used to display the value and the unit associated with a
   *     given {@link Range} bound
   * @see {@link #count(String)}, {@link #count(String, Renderer)} for rendering any countable value
   *     with its unit
   * @see {@link #fromDatePattern(IDatePattern)}, {@link #fromDateFormat(DateFormat)} for fully
   *     rendering dates as without-unit-values without bothering with custom units
   * @throws MissingResourceException if the resource key formed by associating the base key and one
   *     of the suffixes cannot be found
   */
  public static <C extends Comparable<?>> Renderer<Range<C>> range(
      String rangeResourceKey,
      Renderer<C> withUnitValueRenderer,
      Renderer<C> withoutUnitValueRenderer) {
      ...
      if (!value.hasLowerBound() && !value.hasUpperBound()) {
        ...
        mapModel =
            Models.dataMap()
                .put(
                    "end",
                    this.withoutUnitValueRenderer.asModel(
                        Models.transientModel(value.upperEndpoint())))
                .put(
                    "endUnit",
                    this.withUnitValueRenderer.asModel(
                        Models.transientModel(value.upperEndpoint())))
                .build();
        ...
        mapModel =
            Models.dataMap()
                .put(
                    "start",
                    this.withoutUnitValueRenderer.asModel(
                        Models.transientModel(value.lowerEndpoint())))
                .put(
                    "startUnit",
                    this.withUnitValueRenderer.asModel(
                        Models.transientModel(value.lowerEndpoint())))
