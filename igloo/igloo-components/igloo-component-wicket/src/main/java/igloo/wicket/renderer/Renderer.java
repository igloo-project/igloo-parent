...
public abstract class Renderer<T> implements IConverter<T>, IRenderer<T> {
	...
	public static <T extends TemporalAccessor> Renderer<T> fromDateTimeFormatter(DateTimeFormatter dateTimeFormatter) {
		checkNotNull(dateTimeFormatter);
		return fromDateTimeFormatter(dateTimeFormatter::withLocale);
	}
	
	public static <T extends TemporalAccessor> Renderer<T> fromDateTimeFormatter(SerializableFunction2<? super Locale, DateTimeFormatter> dateTimeFormatterFunction) {
	...
	private static class DateTimeFormatterRenderer<T extends TemporalAccessor> extends Renderer<T> {
		...
		private final SerializableFunction2<? super Locale, DateTimeFormatter> dateTimeFormatterFunction;

		public DateTimeFormatterRenderer(SerializableFunction2<? super Locale, DateTimeFormatter> dateTimeFormatterFunction) {
			super();
	...
	public static <T extends TemporalAccessor> Renderer<T> fromDateTimePattern(final IDateTimePattern dateTimePattern) {
		return fromDateTimePattern(dateTimePattern, ZoneId.systemDefault());
	}
	
	public static <T extends TemporalAccessor> Renderer<T> fromDateTimePattern(final IDateTimePattern dateTimePattern, ZoneId zoneId) {
		return fromDateTimeFormatter(
			locale ->
				DateTimeFormatter.ofPattern(Localizer.get().getString(dateTimePattern.getKey(), null, null, locale, null, (IModel<String>) null))
					.withLocale(locale)
					.withZone(zoneId)
		);
	}
