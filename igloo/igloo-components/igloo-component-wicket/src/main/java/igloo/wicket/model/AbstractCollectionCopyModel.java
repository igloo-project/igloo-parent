...
/**
 * An abstract base for implementations of {@link ICollectionModel} whose content is to be "cloned"
 * (i.e. copied to a new collection) each time {@link #setObject(Collection)} is called.
 *
 * <p>This is typically what you want when editing a collection in a form.
 *
 * <p>Content is stored as a list of element models. Thus, subclasses of this one are guaranteed to
 * always return the same model for a given element, up to this element's removal from the
 * collection.
 *
...
abstract class AbstractCollectionCopyModel<T, C extends Collection<T>, M extends IModel<T>>
    extends LoadableDetachableModel<C> implements IItemModelAwareCollectionModel<T, C, M> {
