...
public class DelegatedMarkupPanel extends Panel {
  ...
  public Markup getAssociatedMarkup() {
    return MarkupFactory.get()
        .getMarkup((MarkupContainer) findParent(markupClazz), markupClazz, false);
  }
      ...
      public IMarkupFragment getMarkup(MarkupContainer container, Component child) {
        ...
        if (markup == null) {
          throw new MarkupException(
              "The fragments markup provider has no associated markup. "
                  + "No markup to search for fragment markup with id: "
                  + markupId);
        }
        ...
        //					if ((fragmentTag instanceof WicketTag && ((WicketTag)fragmentTag).isFragementTag())
        // == false)
        //					{
