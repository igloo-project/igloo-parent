...
public class ReadOnlyModel<F, T> implements IComponentAssignedModel<T> {
  ...
  public static final <F, T> IDetachableFactory<IModel<F>, IModel<T>> factory(
      final SerializableFunction2<? super F, T> function) {
  ...
  public static <F, T> ReadOnlyModel<F, T> of(
      IModel<F> model, SerializableFunction2<? super F, ? extends T> function) {
    return new ReadOnlyModel<>(model, function);
  }

  public static <F extends Serializable, T> ReadOnlyModel<F, T> of(
      F object, SerializableFunction2<? super F, ? extends T> function) {
    return new ReadOnlyModel<>(Model.of(object), function);
  }

  protected ReadOnlyModel(
      IModel<? extends F> readModel, SerializableFunction2<? super F, ? extends T> function) {
    super();
