 ...
 *
 * <p>This mapper represents exactly the same behavior like the default {@link MountedMapper},
 * except one behavior:<br>
 * The default {@link MountedMapper} implementation prefers the pageId parameter with a higher
 * priority then the rest of the {@link PageParameters}.<br>
 ...
 * </pre>
 *
 * leads to the UserPage with the indexed {@link PageParameters} "4" at index 0. So the User with id
 * 4 is shown. When a user manually changes the URL to something like this:
 *
 * <pre>
 * ...myApplication/User/5?15
 * </pre>
 *
 * The {@link MountedMapper} would deliver the same page as before and would show the user with id 4
 * and not - like it is implicated in the url - the user with id 5, because the pageId parameter is
 * preferred during mapping process. <br>
 * <br>
 * Therefore this implementation compares the existing {@link PageParameters} of the cached page and
 * the {@link PageParameters} of the current request. If a difference between this two {@link
 * PageParameters} are recognized, this mapper redirects to a fresh bookmarkable instance of the
 * current requested page. <br>
 * <br>
 * To use this mapper to mount your pages, you must declare the mapper programmatically in your
 * {@link Application} by overwriting the "public void init()" method, like shown below:
 *
 * <pre>
 * &#064;Override
 * public void init() {
...
public class PageParameterAwareMountedMapper extends MountedMapper {
  ...
  public PageParameterAwareMountedMapper(
      String mountPath,
  ...
  public PageParameterAwareMountedMapper(
      String mountPath, Class<? extends IRequestablePage> pageClass) {
  ...
  @Override
  protected IRequestHandler processHybrid(
      PageInfo pageInfo,
      Class<? extends IRequestablePage> pageClass,
    ...
    if (handler instanceof RenderPageRequestHandler) {
      ...
      if (renderPageHandler.getPageProvider() instanceof PageProvider) {
        ...
        if (provider.hasPageInstance()) {
          ...
          PageParameters oldPageParameters =
              renderPageHandler.getPageProvider().getPageInstance().getPageParameters();
