 ...
 * @see <a
 *     href="https://cwiki.apache.org/confluence/display/WICKET/AJAX+update+and+file+download+in+one+blow">AJAX
...
public abstract class AbstractDeferredDownloadBehavior extends Behavior {
  ...
  private class ResourceDownloadBehavior extends Behavior implements IRequestListener {
    ...
    public void onRequest() {
      ResourceStreamRequestHandler handler =
          new ResourceStreamRequestHandler(getResourceStream(), getFileDisplayName());
  ...
  public final void bind(Component component) {
    checkState(
        this.component == null,
        "Cannot add a " + Classes.simpleName(getClass()) + " to multiple components");
  ...
  public Component getComponent() {
    checkState(
        this.component != null,
        "A " + Classes.simpleName(getClass()) + " must be added to a component before use");
    return component;
  }
  ...
  private String getUrl(boolean fullUrl) {
    String url =
        getComponent()
            .getPage()
            .urlForListener(resourceDownloadBehavior, new PageParameters())
            .toString();
    if (fullUrl) {
  ...
  private JsStatement createDownloadJsStatement(boolean fullUrl) {
    ...
    return new JsStatement()
        ...
        .chain(
            "setTimeout",
            JsUtils.doubleQuotes("window.location.href=" + JsUtils.quotes(url, true), true),
            String.valueOf(100));
  }

  /**
   * Call this method to redirect to a page ({@link Component#setResponsePage(Page)}) and then
