...
public abstract class AbstractOptGroupDropDownChoice<E> extends DropDownChoice<E> {
  ...
  protected AbstractOptGroupDropDownChoice(
  ...
  protected AbstractOptGroupDropDownChoice(
      String id,
      IModel<E> model,
      final IModel<? extends List<? extends E>> choicesModel,
      IChoiceRenderer<? super E> renderer,
      final Comparator<E> sorter) {
    super(id, model, choicesModel, renderer);
    setChoices(
        new LoadableDetachableModel<List<? extends E>>() {
  ...
  protected void appendOptionHtml(
      AppendingStringBuffer buffer, E choice, int index, String selected) {
    if (grouped) {
