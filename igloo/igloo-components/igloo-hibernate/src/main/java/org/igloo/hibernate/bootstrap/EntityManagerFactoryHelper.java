...
public class EntityManagerFactoryHelper {
  ...
  /**
   * @param settings provided as key/values pairs: Object[] must be even-sized, with alternating
   *     key/value objects.
  ...
  public static EntityManagerFactory generateEntityManagerFactory(Object... settings) {
    return Bootstrap.getEntityManagerFactoryBuilder(
            new PersistenceUnitDescriptorAdapter(), generateSettings(settings))
        .build();
  }
    ...
    if (keysAndValues != null) {
      if (keysAndValues.length % 2 != 0) {
        throw new IllegalStateException(
            "Varargs to create settings should contain even number of entries");
      }
