...
public final class SqlUpdateScript {
  ...
  public static void writeSqlDiffScript(
      ConfigurableApplicationContext context, String fileName, String action) {
  ...
  public static void writeSqlDiffScript(
      ConfigurableApplicationContext context, SqlOutput output, String action) {
    EntityManagerFactory entityManagerFactory = context.getBean(EntityManagerFactory.class);
    Metadata metadata = context.getBean(MetadataRegistryIntegrator.class).getMetadata();
  ...
  public static void writeSqlDiffScript(
    ...
    if (!exceptionHandler.getExceptions().isEmpty()) {
      IllegalStateException e =
          new IllegalStateException("One or multiple exception during execution");
    ...
    String schema =
        Optional.ofNullable(
                entityManagerFactory
                    .getProperties()
                    .getOrDefault(AvailableSettings.DEFAULT_SCHEMA, null))
    ...
    if (output.isFile()) {
      try (Writer outputWriter = new FileWriter(output.getTarget(), false)) {
        ...
        throw new RuntimeException(
            String.format("File %s cannot be created/emptied.", output.getTarget()), e);
      }
    } else {
