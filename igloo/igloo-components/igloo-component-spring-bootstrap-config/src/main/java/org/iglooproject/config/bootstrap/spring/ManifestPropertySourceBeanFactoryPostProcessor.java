...
/**
 * Process {@link Configuration} beans annotated {@link ManifestPropertySource} to extract MANIFEST
 * informations in properties. A prefix is used before each property.
 */
public class ManifestPropertySourceBeanFactoryPostProcessor
    implements ApplicationContextAware, BeanFactoryPostProcessor, PriorityOrdered {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(ManifestPropertySourceBeanFactoryPostProcessor.class);
  ...
  public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
    ...
    // use entry as a pair
    List<Entry<Class<?>, ManifestPropertySource>> propertySources =
        Arrays.stream(beanFactory.getBeanDefinitionNames())
            ...
            .filter(
                ManifestPropertySourceBeanFactoryPostProcessor::isManifestPropertySourceBeanType)
    ...
    for (Entry<Class<?>, ManifestPropertySource> manifestPropertySource : propertySources) {
      ...
      if (className != null && clazz == null) {
        LOGGER.warn(
            "{} className not found on @{} on {} : skipped",
            className,
            ManifestPropertySource.class.getSimpleName(),
            manifestPropertySource.getKey().getSimpleName());
        continue;
      }
      if (clazz == null || clazz == void.class) {
      ...
      for (Entry<Object, Object> attributeEntry : manifest.getMainAttributes().entrySet()) {
        properties.put(
            String.format("%s.%s", prefix, attributeEntry.getKey()), attributeEntry.getValue());
      }
      for (Entry<String, Attributes> manifestEntry : manifest.getEntries().entrySet()) {
        for (Entry<Object, Object> attributeEntry : manifestEntry.getValue().entrySet()) {
          properties.put(
              String.format("%s.%s.%s", prefix, manifestEntry.getKey(), attributeEntry.getKey()),
      ...
      PropertySource<?> existingPropertySource =
          applicationContext.getEnvironment().getPropertySources().remove(propertySourceName);
      ...
      applicationContext
          .getEnvironment()
          .getPropertySources()
          .addLast(new MapPropertySource(propertySourceName, properties));
    }
  }
  ...
  private Manifest findManifest(Class<?> clazz) {
    ...
    if (clazzUrl.getProtocol().equals("jar")) {
      ...
      throw new RuntimeException(
          String.format(
              "Protocol %s not supported in URL %s", clazzUrl.getProtocol(), clazzUrl.toString()));
    }
    ...
    try (InputStream is = new URL(manifestPath).openStream()) {
      ...
      throw new RuntimeException(
          String.format("Failure to read MANIFEST at url: %s", manifestPath, e));
    }
  }
  ...
  private static boolean isManifestPropertySourceBeanType(Class<?> beanType) {
    return isConfigurationBeanType(beanType)
        && beanType.isAnnotationPresent(ManifestPropertySource.class);
  }
  ...
  private static Entry<Class<?>, ManifestPropertySource> toManifestPropertySource(
      Class<?> beanType) {
    return Iterables.getOnlyElement(
        ImmutableMap.<Class<?>, ManifestPropertySource>builder()
            .put(beanType, beanType.getAnnotation(ManifestPropertySource.class))
            .build()
            .entrySet(),
        null);
  }
