 ...
 *
 * <p>This class allows to use a placeholder for a @{@link PropertySource} annotation value field
 * that resolves to a list of resources. It requires some setup be used:
 *
 * <ul>
 *   <li>A custom {@link ProtocolResolver} {@link CompositeProtocolResolver}, registered on the
 *       {@link ResourceLoader} of {@link ConfigurableApplicationContext}.
 *   <li>@{@link org.springframework.context.annotation.PropertySource#factory()} must be set with
 *       {@link CompositePropertySourceFactory}.
 *   <li>@{@link org.springframework.context.annotation.PropertySource#value()} must start with
 *       composite:, followed by a comma-separated list of resources paths ; each resource path is
 *       resolved.
 * </ul>
 *
 * <pre class="code">
  ...
  public PropertySource<?> createPropertySource(String name, EncodedResource resource)
    ...
    if (resource.getResource() instanceof CompositeResource) {
      ...
      for (Resource r : ((CompositeResource) resource.getResource()).getResources()) {
        try {
          PropertySource<?> wrappedPropertySource =
              DEFAULT_PROPERTY_SOURCE_FACTORY.createPropertySource(
                  name, new EncodedResource(r, resource.getEncoding()));
          wrappedPropertySource =
              (wrappedPropertySource instanceof ResourcePropertySource
                  ? ((ResourcePropertySource) wrappedPropertySource).withResourceName()
                  : wrappedPropertySource);
