...
public class ConsoleMaintenanceTaskTaskManagerPanel extends Panel {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ConsoleMaintenanceTaskTaskManagerPanel.class);
	...
	public ConsoleMaintenanceTaskTaskManagerPanel(String id) {
		...
		IModel<Boolean> managerActiveModel = new LoadableDetachableModel<Boolean>() {
		...
		IModel<Integer> queueSizeModel = new LoadableDetachableModel<Integer>() {
			private static final long serialVersionUID = 1L;
			@Override
		...
		add(
			new TransparentWebMarkupContainer("card")
				.add(new ClassAttributeAppender(managerActiveCondition.then("card-success").otherwise("card-danger")))
		);
		
		add(
			new CoreLabel("active", new StringResourceModel("console.maintenance.task.taskManager.active.${}").setModel(managerActiveModel))
				.hideIfEmpty()
		);
		
		add(
			new CountLabel("queueSize", "console.maintenance.task.taskManager.queueSize", queueSizeModel)
		);
					...
					public void execute(AjaxRequestTarget target) {
						try {
							if (queuedTaskHolderManager.isAvailableForAction()) {
								...
								Session.get().error(getString("console.maintenance.task.taskManager.action.unavailable"));
							}
