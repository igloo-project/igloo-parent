...
public class BootstrapCollapseBehavior extends Behavior {
  ...
  public BootstrapCollapseBehavior(Component target, IModel<Boolean> visibleModel) {
    ...
    this.targetClassShowBehavior =
        new ClassAttributeAppender(
            Condition.isTrue(visibleModel).then("show").otherwise((String) null));

    this.collapseShowAjaxEventBehavior =
        new AjaxEventBehavior(BootstrapCollapseEventType.SHOW.getLabel()) {
    ...
    this.collapseHideAjaxEventBehavior =
        new AjaxEventBehavior(BootstrapCollapseEventType.HIDE.getLabel()) {
          private static final long serialVersionUID = 1L;
