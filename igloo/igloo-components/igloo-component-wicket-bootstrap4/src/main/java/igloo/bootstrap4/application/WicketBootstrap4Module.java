...
public class WicketBootstrap4Module implements IWicketModule, IBootstrapProvider {
  ...
  public void modalRenderHead(Component component, IHeaderResponse response) {
    response.render(
        JavaScriptHeaderItem.forReference(Bootstrap4ModalJavaScriptResourceReference.get()));
  }
  ...
  public void tooltipRenderHead(
    ...
    response.render(
        JavaScriptHeaderItem.forReference(BootstrapTooltipJavaScriptResourceReference.get()));
    ...
    response.render(
        OnDomReadyHeaderItem.forScript(
            "$(document).tooltip(" + options.getJavaScriptOptions() + ");"));
  }

  @Override
  public void tabRenderHead(Component component, IHeaderResponse response) {
    response.render(
        JavaScriptHeaderItem.forReference(BootstrapTabJavaScriptResourceReference.get()));
    response.render(
        JavaScriptHeaderItem.forReference(BootstrapTabMoreJavaScriptResourceReference.get()));
    response.render(
        OnDomReadyHeaderItem.forScript(new JsStatement().$(component).chain("tab").render(true)));
  }
  ...
  public void popoverRenderHead(
    ...
    response.render(
        JavaScriptHeaderItem.forReference(BootstrapPopoverJavaScriptResourceReference.get()));
    response.render(
        OnDomReadyHeaderItem.forScript(
            new JsStatement().$(component).chain("popover", options.get()).render()));
  }
}
