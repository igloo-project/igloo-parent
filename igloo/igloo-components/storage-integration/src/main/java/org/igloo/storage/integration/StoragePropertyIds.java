...
public class StoragePropertyIds extends AbstractPropertyIds {
	 ...
	 * and perform effective removal of delete file on commit event. Keep default should be fine.
	 */
	 ...
	 */
	public static final ImmutablePropertyId<Boolean> JOB_CONSISTENCY_DISABLED = immutable("storage.job.consistency.disabled");

	/**
	 * Disable {@link StorageUnit} split job.
	 */
	public static final ImmutablePropertyId<Boolean> JOB_SPLIT_STORAGE_UNIT_DISABLED = immutable("storage.job.splitStorageUnit.disabled");
	...
	/**
	 * Maximum number of {@link StorageUnit} to check for a job. This allows to smooth {@link StorageUnit} check
	 * (not checked {@link StorageUnit} will be processed in a later run). 0 for unlimited.
	 */
	public static final ImmutablePropertyId<Integer> JOB_CONSISTENCY_STORAGE_UNIT_LIMIT = immutable("storage.job.consistency.storageUnitLimit");
	
	/**
	 * Check delay to use if {@link StorageUnit#getCheckDelay()} is null. This controls consistency trigger.
	 */
	public static final ImmutablePropertyId<Duration> JOB_CHECK_DEFAULT_DELAY = immutable("storage.job.check.defaultDelay");

	/**
	 * Check delay to use if {@link StorageUnit#getCheckChecksumDelay()} is null. This controls checksum consistency trigger.
	 */
	...
	/**
	 * Delay (from creation date) before transient cleaning. It ensures that transient files are removed after a minimal
	 * delay.
	 */
