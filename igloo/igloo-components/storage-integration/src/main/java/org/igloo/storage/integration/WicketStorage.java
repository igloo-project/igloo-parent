...
public class WicketStorage {
  ...
  private static final MetaDataKey<IWicketStorageSettings> STORAGE_SETTINGS_METADATA_KEY =
      new MetaDataKey<IWicketStorageSettings>() {
  ...
  public static void install(WebApplication app, IWicketStorageSettings settings) {
    ...
    if (existingSettings == null) {
      ...
      app.mountResource(
          settings.getDownloadMountPath(), FichierFileStorageWebResource.attachment());
      ...
      if (settings.isSupervisionPagesEnabled()) {
        try {
          ...
          throw new IllegalStateException(
              "Error mounting storage wicket supervision pages, check storage-micrometer and igloo-supervision-page dependencies",
              e);
        }
      }
    }
  ...
  private static void mountSupervision(WebApplication app) {
    app.mountResource(
        "/monitoring/storage/failures/",
        SpringBackedMonitoringResource.get(StorageAutoConfiguration.HEALTH_FAILURES));
    app.mountResource(
        "/monitoring/storage/size/",
        SpringBackedMonitoringResource.get(StorageAutoConfiguration.HEALTH_SIZE));
  }
