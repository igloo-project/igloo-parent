...
class TestOrphanRemoval {
   ...
   * (@OneToMany).
   *
   * <p>Check that unset orphanItem.owner (owning-side of the relation) delete OrphanItem entity.
   *
   * <p>Parameters are injected with {@link EntityManagerFactoryExtension}.
   */
  @Test
  ...
  void testOrphanRemoval_keepOrphan(EntityManager entityManager, EntityTransaction transaction) {
    ...
    item = entityManager.find(OrphanItem.class, entityId);
    // when child entity only is modified, it is not removed as removal is only triggered by
    // collection
    item.owner = null;
    entityManager.flush();
    entityManager.clear();
    ...
    assertThat(list(entityManager, OrphanOwner.class)).hasSize(1);
    assertThat(list(entityManager, OrphanOwner.class))
        .first()
        .satisfies(o -> assertThat(o.items).isEmpty());
    ...
    assertThat(list(entityManager, OrphanItem.class))
        .first()
        .satisfies(o -> assertThat(o.owner).isNull());
    entityManager.flush();
    entityManager.clear();
  }
