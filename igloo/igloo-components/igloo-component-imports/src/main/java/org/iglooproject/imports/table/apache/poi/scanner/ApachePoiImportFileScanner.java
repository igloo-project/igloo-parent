...
public class ApachePoiImportFileScanner
    implements IExcelImportFileScanner<Workbook, Sheet, Row, Cell, CellReference> {

  private static final String HIDDEN_FILE_PREFIX = ".";
  private static final String THUMBS_DB_NAME = "Thumbs.db";

  // Default is to scan all files except: hidden files, files in hidden directories and files named
  // "Thumbs.db".
  public static final FileFilter DEFAULT_DIRECTORY_CHILD_FILTER =
      notFileFilter(
          or(
              prefixFileFilter(HIDDEN_FILE_PREFIX),
              and(fileFileFilter(), nameFileFilter(THUMBS_DB_NAME))));

  protected FileFilter getDirectoryChildFilter() {
    return DEFAULT_DIRECTORY_CHILD_FILTER;
  }

  protected static final Map<SheetSelection, ? extends Predicate2<? super Sheet>>
      SELECTIONS_PREDICATES =
          ImmutableMap.of(
              SheetSelection.ALL,
              Predicates2.<Sheet>alwaysTrue(),
              SheetSelection.NON_HIDDEN_ONLY,
              sheet -> {
                if (sheet == null) {
                  return false;
                }
  ...
  public void scanRecursively(
      ...
      IExcelImportFileVisitor<Workbook, Sheet, Row, Cell, CellReference> visitor)
