...
public abstract class AbstractRecordingTableImportEventHandler implements ITableImportEventHandler {
	...
	public AbstractRecordingTableImportEventHandler(TableImportNonFatalErrorHandling errorHandling, ITableImportEventMessageFormatter formatter) {
		this.errorHandling = errorHandling;
		this.formatter = formatter;
	}

	protected abstract void record(ExcelImportErrorEvent event, TableImportLocation location, String formattedMessage);

	protected abstract void record(ExcelImportInfoEvent event, TableImportLocation location, String formattedMessage);

	@Override
	public void headerLabelMappingError(String expectedHeaderLabel, int indexAmongMatchedColumns, TableImportLocation location) throws TableImportHeaderLabelMappingException {
		this.hasError = true;
		...
		if (TableImportNonFatalErrorHandling.THROW_IMMEDIATELY.equals(errorHandling)) {
			throw new TableImportHeaderLabelMappingException(formattedMessage, expectedHeaderLabel, location);
		}
	}
	...
	public void checkNoMappingErrorOccurred() throws TableImportMappingException {
		if (hasError) {
			resetErrors();
			throw new TableImportMappingException("An error occurred; see the error report for more details");
		}
	}
	
	@Override
	public void event(ExcelImportErrorEvent event, TableImportLocation location, String message, Object ... args)
		...
		if (event.isFatal() || TableImportNonFatalErrorHandling.THROW_IMMEDIATELY.equals(errorHandling)) {
			throw new TableImportContentException(formattedMessage);
		}
	}
	
	@Override
	public void event(ExcelImportInfoEvent event, TableImportLocation location, String message, Object ... args) {
		String formattedMessage = formatter.formatErrorMessage(location, message, args);
		record(event, location, formattedMessage);
	}
	...
	public void checkNoErrorOccurred() throws TableImportContentException {
		if (hasError) {
			resetErrors();
			throw new TableImportContentException("An error occurred; see the error report for more details");
		}
	}
