...
public class OpenCsvImportFileScanner
    implements ICsvImportFileScanner<CsvTable, CsvRow, CsvCell, CsvCellReference> {
  ...
  public static final FileFilter DEFAULT_DIRECTORY_CHILD_FILTER =
      FileFilterUtils.notFileFilter(FileFilterUtils.prefixFileFilter(HIDDEN_FILE_PREFIX));

  /**
   * A shorthand for implementing {@code ICsvImportFileVisitor<CsvSheet, CsvRow, CsvCell,
   * CsvCellReference>} without writing down the generics (which is painful).
   */
  public interface IOpenCsvImportFileVisitor
      extends ICsvImportFileVisitor<CsvTable, CsvRow, CsvCell, CsvCellReference> {}
  ...
  public void scanRecursively(
      File file,
      String filename,
      ICsvImportFileVisitor<CsvTable, CsvRow, CsvCell, CsvCellReference> visitor)
  ...
  public void scan(
    ...
    try (Reader reader = createReader(stream, filename);
        CSVReader csvReader = createCsvReader(reader, filename)) {
      CsvTable sheet = new CsvTable(csvReader.readAll());
      visitor.visitTable(navigator, sheet);
    } catch (IOException e) {
