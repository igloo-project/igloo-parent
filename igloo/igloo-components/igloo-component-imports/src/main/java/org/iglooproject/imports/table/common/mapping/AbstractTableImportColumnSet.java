...
/**
 * The central class of this Excel import framework. See TestApachePoiExcelImporter for an example
...
public abstract class AbstractTableImportColumnSet<TTable, TRow, TCell, TCellReference> {
  ...
  public AbstractTableImportColumnSet(
      AbstractTableImportColumnBuilder<TTable, TRow, TCell, TCellReference> builder) {
  ...
  public final TypeState<TTable, TRow, TCell, TCellReference> withHeader(
      String headerLabel, Comparator<? super String> collator) {
    return withHeader(headerLabel, collator, 0, MappingConstraint.REQUIRED);
  }

  public final TypeState<TTable, TRow, TCell, TCellReference> withHeader(
      String headerLabel, MappingConstraint mappingConstraint) {
    return withHeader(headerLabel, 0, mappingConstraint);
  }

  public final TypeState<TTable, TRow, TCell, TCellReference> withHeader(
      String headerLabel, int indexAmongMatchedColumns, MappingConstraint mappingConstraint) {
    return withHeader(
        headerLabel, defaultHeaderLabelCollator, indexAmongMatchedColumns, mappingConstraint);
  }

  public final TypeState<TTable, TRow, TCell, TCellReference> withHeader(
      String headerLabel,
      Equivalence<? super String> headerEquivalence,
      int indexAmongMatchedColumns,
      MappingConstraint mappingConstraint) {
    return builder.withHeader(
        this,
        headerLabel,
        Predicates2.from(headerEquivalence.equivalentTo(headerLabel)),
  ...
  public final TypeState<TTable, TRow, TCell, TCellReference> withHeader(
    ...
    return builder.withHeader(
        this,
        headerLabel,
        Predicates2.comparesEqualTo(headerLabel, collator),
        indexAmongMatchedColumns,
        mappingConstraint);
  }
   ...
   *
   * <p>This class is implemented as an inner class in order to get rid of the <TSheet, TRow,
   * TCellReference, TCell, TValue> generic parameters when the client references columns.
   */
  public class Column<TValue>
      implements ITableImportColumnDefinition<TTable, TRow, TCell, TCellReference, TValue> {
    ...
    public Column(
        ITableImportColumnMapper<TTable, TRow, TCell, TCellReference> mapper,
    ...
    @Override
    public IMappedExcelImportColumnDefinition<TTable, TRow, TCell, TCellReference, TValue> map(
        TTable sheet,
      ...
      Function2<? super TRow, ? extends TCellReference> rowToCellReferenceFunction =
          mapper.tryMap(sheet, navigator, eventHandler);
      return new MappedTableImportColumnDefinitionImpl<>(
          sheet,
          rowToCellReferenceFunction,
          navigator,
          cellToValueFunction,
          mandatoryValuePredicate);
    }
  }

  public final TableContext map(
      TTable sheet,
      ITableImportNavigator<TTable, TRow, TCell, TCellReference> navigator,
      ITableImportEventHandler eventHandler)
      throws TableImportMappingException {
  ...
  public class TableContext extends TableImportLocationContext implements Iterable<RowContext> {
    ...
    private final Map<
            Column<?>, IMappedExcelImportColumnDefinition<TTable, TRow, TCell, TCellReference, ?>>
        mappings;
    ...
    private TableContext(
      ...
      Map<Column<?>, IMappedExcelImportColumnDefinition<TTable, TRow, TCell, TCellReference, ?>>
          mutableMappings = Maps.newHashMap();
    ...
    protected Iterator<RowContext> toRowContexts(Iterator<TRow> rows) {
      return Streams.stream(rows).map(input -> row(input)).iterator();
    }
    ...
    private <TValue>
        IMappedExcelImportColumnDefinition<TTable, TRow, TCell, TCellReference, TValue>
            getMappedColumn(
                ITableImportColumnDefinition<TTable, TRow, TCell, TCellReference, TValue>
                    columnDefinition) {
      IMappedExcelImportColumnDefinition<TTable, TRow, TCell, TCellReference, TValue> mappedColumn =
          (IMappedExcelImportColumnDefinition<TTable, TRow, TCell, TCellReference, TValue>)
              mappings.get(columnDefinition);
    ...
    /**
     * @see TableImportLocationContext The event recording methods error(), info(), etc. defined in
     *     the superclass.
     */
    @Override
    ...
    public void event(ExcelImportErrorEvent event, String message, TRow row, Object... args)
    ...
    public void event(
        ExcelImportErrorEvent event,
        String message,
        TRow row,
        TCellReference cellReference,
        Object... args)
        throws TableImportContentException {
      eventHandler.event(
          event, navigator.getLocation(table, row, cellReference), message, (Object[]) args);
    }

    public void event(
        ExcelImportInfoEvent event,
        String message,
        TRow row,
        TCellReference cellReference,
        Object... args) {
  ...
  public final class RowContext extends TableImportLocationContext {
    ...
    public <TValue> CellContext<TValue> cell(Column<TValue> columnDefinition) {
      return new CellContext<>(
          this, this.eventHandler, tableContext.getMappedColumn(columnDefinition));
    }
    ...
    public void event(
        ExcelImportErrorEvent event, String message, TCellReference cellReference, Object... args)
        throws TableImportContentException {
      tableContext.event(event, message, row, cellReference, (Object[]) args);
    }

    public void event(
        ExcelImportInfoEvent event, String message, TCellReference cellReference, Object... args) {
      tableContext.event(event, message, row, cellReference, (Object[]) args);
    }
  }
  ...
  public final class CellContext<T> extends TableImportLocationContext {
    ...
    private final IMappedExcelImportColumnDefinition<TTable, TRow, TCell, TCellReference, T>
        mappedColumn;
    ...
    public T getMandatory(ExcelImportErrorEvent event, String message, Object... args)
        throws TableImportContentException {
    ...
    @Override
    public TableImportLocation getLocation() {
      return rowContext.tableContext.navigator.getLocation(
          rowContext.tableContext.table,
          rowContext.row,
          mappedColumn.getCellReference(rowContext.row));
    }
  }
}
