...
public class ConsoleNotificationDemoDetailPage extends ConsoleNotificationDemoTemplate {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ConsoleNotificationDemoDetailPage.class);
	...
	public ConsoleNotificationDemoDetailPage(PageParameters parameters, IModel<INotificationContentDescriptor> descriptorModel) {
		super(parameters);
		...
		descriptorWithContextModel = LoadableDetachableModel.of(() ->
			descriptorModel.getObject().withContext(new SimpleRecipient(localeModel.getObject(), null, null))
		);
		
		subjectModel = LoadableDetachableModel.of(() -> {
			try {
		...
		bodyModel = LoadableDetachableModel.of(() -> {
			try {
		...
		add(
			...
			new WebMarkupContainer("body")
				.add(new AttributeModifier("src", () -> urlForListener(iFrameContentBehavior, new PageParameters()).toString()))
		);
	}
