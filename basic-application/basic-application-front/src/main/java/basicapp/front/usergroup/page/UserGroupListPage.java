...
import basicapp.front.usergroup.popup.UserGroupPopup;
...
public class UserGroupListPage extends UserGroupTemplate {

  private static final long serialVersionUID = 2733071974944289365L;

  private static final Logger LOGGER = LoggerFactory.getLogger(UserGroupListPage.class);

  public static final IPageLinkDescriptor linkDescriptor() {
    return LinkDescriptorBuilder.start().page(UserGroupListPage.class);
  }
  ...
  public UserGroupListPage(PageParameters parameters) {
    super(parameters);

    IModel<List<UserGroup>> userGroupListModel =
        new LoadableDetachableModel<List<UserGroup>>() {
    ...
    add(
        DataTableBuilder.start(
                ReadOnlyCollectionModel.of(userGroupListModel, GenericEntityModel.factory()))
            .addLabelColumn(
                new ResourceModel("business.userGroup.name"), Bindings.userGroup().name())
            .withLink(UserGroupDetailPage.MAPPER_SOURCE.setParameter2(new ComponentPageModel(this)))
