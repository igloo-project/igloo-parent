...
public class TechnicalSearchQueryImpl implements ITechnicalUserSearchQuery {
	...
	@Override
	public Collection<TechnicalUser> list(TechnicalUserSearchQueryData data, Map<TechnicalUserSort, ISort.SortOrder> sorts, Integer offset, Integer limit) {
		if (!checkLimit(limit)) {
			return List.of();
		}
	...
	private BiConsumer<? super SearchPredicateFactory, ? super SimpleBooleanPredicateClausesCollector<?>> predicateContributor(TechnicalUserSearchQueryData data) {
		return (f, root) -> {
			root.add(f.matchAll());
