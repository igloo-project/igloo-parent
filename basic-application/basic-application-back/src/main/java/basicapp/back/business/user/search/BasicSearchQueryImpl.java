...
public class BasicSearchQueryImpl implements IBasicUserSearchQuery {
  ...
  @Override
  public Collection<BasicUser> list(
      BasicUserSearchQueryData data,
      Map<BasicUserSort, ISort.SortOrder> sorts,
      Integer offset,
      Integer limit) {
    if (!checkLimit(limit)) {
      return List.of();
  ...
  private BiConsumer<
      ...
      predicateContributor(BasicUserSearchQueryData data) {
    return (f, root) -> {
      root.add(f.matchAll());
