...
public class BasicApplicationWebappSecurityConfig extends AbstractWebappSecurityConfig {
		...
		protected void configure(HttpSecurity http) throws Exception {
			http.regexMatcher("^/console/.*")
				...
				.authorizeRequests()
					.regexMatchers(
						"/console/login/.*",
						"/console/access-denied/.*").permitAll()
					.regexMatchers("/console/.*").hasAnyAuthority(CoreAuthorityConstants.ROLE_ADMIN);
		}
	}
	...
	@Configuration
	@Order(3)
	public static class BasicApplicationWebSecurityConfigurerAdapter extends WebSecurityConfigurerAdapter {
		@Autowired
		private CoreWebSecurityExpressionHandler webSecurityExpressionHandler;
		...
		protected void configure(HttpSecurity http) throws Exception {
			http
				...
				.authorizeRequests()
					...
					.antMatchers("/**").hasAnyAuthority(CoreAuthorityConstants.ROLE_AUTHENTICATED);
		}
