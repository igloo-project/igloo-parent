...
public class ExampleHtmlNotificationPanel extends AbstractHtmlNotificationPanel<User> {
  ...
  public ExampleHtmlNotificationPanel(String id, IModel<User> userModel, IModel<Date> dateModel) {
    ...
    add(
        new WebMarkupContainer("intro")
            .add(
                ...
                new DateLabel("date", dateModel, DatePattern.SHORT_DATE).showPlaceholder(),
                new DateLabel("time", dateModel, DatePattern.TIME).showPlaceholder()));
    ...
    add(new ExternalLink("mainLink", urlModel), new ExternalLink("url", urlModel, urlModel));
