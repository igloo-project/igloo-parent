...
public abstract class AbstractReferenceDataSimpleListPanel<
  ...
  public AbstractReferenceDataSimpleListPanel(
      String id, D dataProvider, CompositeSortModel<S> sortModel) {
    super(id);
  ...
  protected void onInitialize() {
    ...
    results =
        decorate(
                addActionColumn(addColumns(DataTableBuilder.start(dataProvider, sortModel)))
                    .rows()
                    .withClass(
                        itemModel ->
  ...
  protected abstract Component createSearchForm(
      String wicketId, D dataProvider, DecoratedCoreDataTablePanel<T, S> table);
