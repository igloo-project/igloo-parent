...
@Component
@Scope("prototype")
public class HistoryLogSearchQueryImpl
    extends AbstractHibernateSearchSearchQuery<HistoryLog, HistoryLogSort>
  ...
  public IHistoryLogSearchQuery date(Date dateMin, Date dateMax) {
    must(matchRange(AbstractHistoryLog.DATE, dateMin, dateMax));
    return this;
  }
  ...
  @Override
  public IHistoryLogSearchQuery mandatoryDifferencesEventTypes(
      Collection<HistoryEventType> mandatoryDifferencesEventTypes) {
    if (!mandatoryDifferencesEventTypes.isEmpty()) {
      List<HistoryEventType> allowedWithoutDifferencesEventTypes =
