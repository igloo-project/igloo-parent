...
@Service
public class UserGroupPermissionEvaluatorImpl extends AbstractGenericPermissionEvaluator<UserGroup>
    implements IUserGroupPermissionEvaluator {
  ...
  public boolean hasPermission(User user, UserGroup userGroup, Permission permission) {
    if (is(permission, READ)) {
      ...
      return hasRole(user, ROLE_ADMIN) && UserGroupPredicates.unlocked().apply(userGroup);
