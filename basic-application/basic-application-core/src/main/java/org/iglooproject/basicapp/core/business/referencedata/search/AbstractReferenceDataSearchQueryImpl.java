...
public abstract class AbstractReferenceDataSearchQueryImpl<T extends ReferenceData<? super T>, S extends ISort<Function<SearchSortFactory, SortFinalStep>>, D extends AbstractReferenceDataSearchQueryData<T>> implements IAbstractReferenceDataSearchQuery<T, S, D> {
	...
	public AbstractReferenceDataSearchQueryImpl(Class<T> clazz) {
	...
	protected BiConsumer<SearchPredicateFactory, SimpleBooleanPredicateClausesCollector<?>> predicateContributor(D data) {
		return (f, root) -> {
			root.add(f.matchAll());
			if (data.getLabel() != null) {
				root.add(f.simpleQueryString().field(ReferenceData.LABEL_FR_AUTOCOMPLETE).matching(wildcardTokensOr(data.getLabel())));
			}
