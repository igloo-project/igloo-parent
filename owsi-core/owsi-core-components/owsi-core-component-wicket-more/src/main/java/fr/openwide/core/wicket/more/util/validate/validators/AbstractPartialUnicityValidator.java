...
public abstract class AbstractPartialUnicityValidator<T, V> extends Behavior implements IValidator<V> {
	...
	public void validate(IValidatable<V> validatable) {
		...
		if (matchingObjects != null && !matchingObjects.isEmpty() && !contains(matchingObjects, mainObject)) {
			validatable.error(decorate(
					new ValidationError(this).addKey(errorKey)
