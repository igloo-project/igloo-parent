...
public interface IMapProvider<K, V> extends ISequenceProvider<K> {
	...
	/**
	 * @param keyModel A model that (on contrary to {@link #getValueModelForProvidedKeyModel(IModel)}) may not have been
	 ...
	 * On contrary to {@link #getValueModelForProvidedKeyModel(IModel)}, the behavior of this method is unchanged
	 * if the given keyModel has not been obtained through this {@link IMapProvider} or if it has been wrapped.
	 */
	IModel<V> getValueModel(IModel<? extends K> keyModel);
	 ...
	 */
	IModel<V> getValueModelForProvidedKeyModel(IModel<K> keyModel);
