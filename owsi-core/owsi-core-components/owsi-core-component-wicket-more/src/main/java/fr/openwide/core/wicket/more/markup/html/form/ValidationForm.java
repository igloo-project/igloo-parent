...
public class ValidationForm<T> extends ModelValidatingForm<T> {

	private static final long serialVersionUID = 2978133799490179228L;

	private final IModel<Class<?>[]> groupsModel;

	public ValidationForm(String id, IModel<T> model) {
		this(id, model, new GroupsModel());
	}

	public ValidationForm(String id, IModel<T> model, IModel<Class<?>[]> groupsModel) {
		super(id, model);
	...
	protected void onValidateModelObjects() {
		...
		final ImmutableListMultimap<FieldPath, ConstraintViolation<T>> violationsByFieldPath = Multimaps.index(violations, new Function<ConstraintViolation<T>, FieldPath>() {
			...
			public FieldPath apply(ConstraintViolation<T> input) {
				return FieldPath.fromString(input.getPropertyPath().toString().replaceAll("\\[[0-9]*\\]", "\\[\\*\\]"));
			}
		});
			...
			public void component(FormComponent<?> object, IVisit<Void> visit) {
				...
				if (violationsByFieldPath.containsKey(completeFieldPath)) {
					for (ConstraintViolation<T> constraintViolation : violationsByFieldPath.get(completeFieldPath)) {
						if (isMatchingPath(model, constraintViolation, completeFieldPath)) {
	...
	private boolean isMatchingPath(IModel<?> model, ConstraintViolation<T> constraintViolation, FieldPath completeFieldPath) {
		PathImpl pathImpl = ((PathImpl) constraintViolation.getPropertyPath());
		
		FieldPath fieldPath = FieldPath.of(completeFieldPath);
		
		List<KeyValue<FieldPath, Object>> nodes = Lists.newArrayList(Lists.transform(
				Lists.reverse(Lists.newArrayList(pathImpl)),
				new Function<Node, KeyValue<FieldPath, Object>>() {
					...
					public KeyValue<FieldPath, Object> apply(Node input) {
						...
						return KeyValue.with(toFieldPath(nodeImpl), nodeImpl.getValue());
					}
				}
		));
		nodes.add(KeyValue.<FieldPath, Object>with(FieldPath.ROOT, getModelObject()));
		...
		while (currentModel != null) {
			...
			if (currentModelAsPropertyModel != null) {
				...
				expressionFieldPath = getCurrentFieldPath(fieldPath);
			}
			...
			// consistency check
			if (!node.getKey().equals(getCurrentFieldPath(expressionFieldPath))) {
				return false;
			}
			...
			if (currentModelAsPropertyModel != null) {
				...
				while (expressionFieldPath != null && !expressionFieldPath.isRoot() && !nodes.isEmpty() && fieldPath !=  null) {
					...
					// consistency check
					if (!currentNode.getKey().equals(getCurrentFieldPath(expressionFieldPath))) {
						return false;
					}
	...
	private FieldPath toFieldPath(NodeImpl nodeImpl) {
		if (nodeImpl == null) {
			return null;
		}
	...
	private FieldPath getCurrentFieldPath(FieldPath fieldPath) {
		if (fieldPath == null) {
			return null;
		}
