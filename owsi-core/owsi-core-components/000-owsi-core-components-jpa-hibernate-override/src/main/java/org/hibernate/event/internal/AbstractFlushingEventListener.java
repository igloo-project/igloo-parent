...
public abstract class AbstractFlushingEventListener implements Serializable {
	...
	private void logFlushResults(FlushEvent event) {
		...
		if (session.getActionQueue().numberOfInsertions() != 0
				|| session.getActionQueue().numberOfDeletions() != 0
				|| session.getActionQueue().numberOfUpdates() != 0) {
			LOG.infof(
					"Flushed: %s insertions, %s updates, %s deletions to %s objects",
					session.getActionQueue().numberOfInsertions(),
					session.getActionQueue().numberOfUpdates(),
		...
		if (session.getActionQueue().numberOfCollectionCreations() != 0
				|| session.getActionQueue().numberOfCollectionUpdates() != 0
				|| session.getActionQueue().numberOfCollectionRemovals() != 0) {
			LOG.infof(
					"Flushed: %s (re)creations, %s updates, %s removals to %s collections",
					session.getActionQueue().numberOfCollectionCreations(),
					session.getActionQueue().numberOfCollectionUpdates(),
...
//		for ( Map.Entry me : IdentityMap.concurrentEntries( persistenceContext.getEntityEntries() ) ) {
	...
	private int flushCollections(final EventSource session, final PersistenceContext persistenceContext) throws HibernateException {
		...
		for ( Map.Entry<PersistentCollection,CollectionEntry> me :
			...
			if ( ce.isDorecreate() ) {
				LOG.infof("onCollectionRecreate %s", ce.getRole());
			...
			if ( ce.isDoremove() ) {
				LOG.infof("onCollectionRemove %s", ce.getRole());
			...
			if ( ce.isDoupdate() ) {
				LOG.infof("onCollectionUpdate %s", ce.getRole());
