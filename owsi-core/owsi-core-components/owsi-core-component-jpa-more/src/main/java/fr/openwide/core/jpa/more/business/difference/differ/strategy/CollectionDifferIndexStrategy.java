...
import fr.openwide.core.jpa.more.business.difference.access.CollectionItemByIndexAccessor;

public final class CollectionDifferIndexStrategy<T> extends AbstractCollectionDifferStrategy<T, Integer> {
	public CollectionDifferIndexStrategy(ItemContentComparisonStrategy itemContentComparisonStrategy) {
	...
	public Iterable<Integer> intersection(Collection<T> source, Collection<T> filter) {
		int sourceSize = source == null ? 0 : Iterables.size(source);
		int filterSize = filter == null ? 0 : Iterables.size(filter);
		return ContiguousSet.create(Range.closedOpen(0, Math.min(sourceSize, filterSize)), DiscreteDomain.integers());
	}
}