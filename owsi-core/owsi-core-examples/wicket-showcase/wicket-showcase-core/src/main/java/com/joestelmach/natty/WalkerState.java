...
/**
   ...
   */
	public void seekBySpan(String direction, String seekAmount, String span) {
		...
		int sign = direction.equals(DIR_RIGHT) ? 1 : -1;
		int field = span.equals(DAY) ? Calendar.DAY_OF_YEAR : span.equals(WEEK) ? Calendar.WEEK_OF_YEAR : span
				.equals(MONTH) ? Calendar.MONTH : span.equals(YEAR) ? Calendar.YEAR : span.equals(HOUR) ? Calendar.HOUR
				: span.equals(MINUTE) ? Calendar.MINUTE : span.equals(SECOND) ? Calendar.SECOND : null;
		if (field > 0)
	...
	public void setExplicitDate(String month, String dayOfMonth, String dayOfWeek, String year) {
		...
		// if no year is given, but a day of week is, we ensure that the
		// resulting
	...
	public void setExplicitTime(String hours, String minutes, String seconds, String amPm, String zoneString) {
		...
		if (hoursInt <= 12) {
			int amPmInt = amPm == null ? (hoursInt >= 12 ? Calendar.PM : Calendar.AM) : amPm.equals(PM) ? Calendar.PM
					: Calendar.AM;
	 ...
	 */
	// public void seekToHoliday(String holidayString, String direction, String
	// seekAmount) {
	//
	...
	// Map<Integer, Date> dates = getDatesForHoliday(startYear, endYear,
	// holiday);
	//
	// // grab the right one
	...
	// Date date = dates.get(year - (holiday.equals(Holiday.NEW_YEARS_EVE) ? 1 :
	// 0));
	//
	// if(date != null) {
	// Calendar cal = getCalendar();
	...
	*/
	...
	// private Map<Integer, Date> getDatesForHoliday(int startYear, int endYear,
	// Holiday holiday) {
	// Map<Integer, Date> holidays = new HashMap<Integer, Date>();
	//
	// if(_holidayCalendar == null) {
	// InputStream fin =
	// WalkerState.class.getResourceAsStream(HOLIDAY_ICS_FILE);
	// try {
	...
	// _logger.log(Level.SEVERE, "Invalid start or end year: " + startYear +
	// ", " + endYear, e);
	// return holidays;
	// }
	//
	...
	// utcCal.setTime(date);
	//
	// // use the year, month and day components of our UTC date to form a new
	// local date
