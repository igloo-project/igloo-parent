...
public class SimpleMandatoryLinkParameterValidator implements ILinkParameterValidator {
	...
	public SimpleMandatoryLinkParameterValidator(Collection<String> parameterNames, Collection<? extends IModel<?>> parameterModels) {
	...
	public void validateModel(LinkParameterValidationErrorCollector collector) {
		for (IModel<?> parameterModel : parameterModels) {
			...
			if (object == null) {
				collector.addError(String.format("A mandatory parameter model object for parameter names '%s' was null.", parameterNames));
			}
		}
	}
